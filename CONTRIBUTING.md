...
5. **Choose `scope` honestly.** Use `portable` for a skill with no repo-specific claims, `workspace` for cross-repo workspace knowledge, `project` for a skill grounded in a specific repo. `scope: project` requires a populated `grounding` block — this is machine-enforced by the schema. For a decision table, see `docs/field-decision-guide.md § 1. Which scope do I use?`.
