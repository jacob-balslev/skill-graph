...
Start with `README.md`, `SKILL_GRAPH.md`, `skill-metadata-protocol/design-rationale.md`, and `skill-metadata-protocol/field-reference.md` before opening a pull request. `SKILL_GRAPH.md` explains the repo's five-tier organisation — every contribution should be classifiable into exactly one tier.

## What you can contribute

**Welcome:**

- Fixes to broken cross-references, stale examples, or drift between the schemas and the Tier 2 docs (`skill-metadata-protocol/design-rationale.md`, `skill-metadata-protocol/field-reference.md`, `docs/manifest-field-mapping.md`)
...
2. **Rewrite the identity.** Change `name:` to your skill's identifier (lowercase, hyphens, matches the parent directory). Rewrite `description:` as a short about-statement of what the skill is. Activation signals belong to `keywords` / `triggers` / `examples` / `anti_examples`; boundary semantics belong to `relations.boundary`. Rewrite every other field to match your subject.
3. **Choose `subject` honestly.** Pick from the closed 12-value enum (3 bands, ADR-0020): `backend-engineering`, `frontend-engineering`, `software-architecture`, `data-engineering`, `agent-ops`, `ai-engineering`, `quality-assurance`, `design`, `reasoning-strategy`, `software-engineering-method`, `knowledge-organization`, `product-domain`. If the skill genuinely spans two shelves, add a secondary in `subjects[]` (max 2 entries, primary first). For field-level guidance, see `skill-metadata-protocol/field-reference.md § subject`.
...
5. **Choose `public` honestly.** Set `public: true` for a skill safe for the public release (no private API keys / personal / customer / internal-operational data); `public: false` for a skill that carries private data and must not be published (the fail-safe default when unsure). Project anchoring is a separate axis: a non-empty `project[]` requires a populated `grounding` block (including `grounding.subject_matter`) — machine-enforced by the schema. The `scope` field is a free-text PRD-style description, not an enum. For the decision tables, see `skill-metadata-protocol/field-decision-guide.md § 1. What public value do I use?`.
6. **Point `relations.*` at real skills.** Every `related`, `boundary`, `verify_with`, and `depends_on` target must be the `name` of another skill that exists in `skills/`. `scripts/skill-lint.js` will reject dangling targets.
7. **Match the 3 eval fields to reality.** `eval_artifacts: none | planned | present` (artifact state), `eval_state: unverified | passing | monitored` (runtime state), `routing_eval: absent | present` (routing coverage). The schema's M5 coherence rule enforces that `eval_state: passing` or `monitored` requires `eval_artifacts: present`. See `skill-metadata-protocol/field-decision-guide.md § 3. What state do I choose for evals?` for the full decision table.
...
If you touched a Tier 2 doc (`skill-metadata-protocol/design-rationale.md`, `skill-metadata-protocol/field-reference.md`, etc.), also update the other Tier 2 docs so they remain in lockstep. Skill Metadata Protocol is the overview; `skill-metadata-protocol/field-reference.md` is the per-field semantics authority; the schema is Tier 1 — source of truth for machine enforcement. Drift between them is a bug.
...
- The PR description states what changed and why, references the relevant `skill-metadata-protocol/design-rationale.md` or `skill-metadata-protocol/field-reference.md` sections, and includes the `node scripts/skill-lint.js` + `node scripts/check-protocol-consistency.js` output.
...
When auditing an existing skill, follow `skill-audit-loop/SKILL_AUDIT_LOOP.md` for the 12-step process and `skill-audit-loop/SKILL_AUDIT_LOOP.md` § Part 2 for the per-skill checklist. See `skill-audit-loop/SKILL_AUDIT_LOOP.md § Recommended Artifact Layout` for the authoritative two-tier artifact root convention (`examples/audits/<skill-name>/` for shipped worked examples; `audits/<skill-name>/` for downstream consumer output). The `examples/audits/documentation/` directory is the canonical worked example.
...
See `skill-audit-loop/SKILL_AUDIT_LOOP.md § Stub Generator` for the full reference.
