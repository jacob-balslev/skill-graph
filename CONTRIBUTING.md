...
5. **Choose `deployment_target` honestly.** Use `portable` for a skill with no repo-specific claims, `project` for a skill grounded in a specific repo. `deployment_target: project` requires a populated `grounding` block (including `grounding.subject_matter`) — this is machine-enforced by the schema. The `scope` field is now a free-text PRD-style description and is not an enum. For a decision table, see `skill-metadata-protocol/field-decision-guide.md § 1. Which deployment_target do I use?`.
