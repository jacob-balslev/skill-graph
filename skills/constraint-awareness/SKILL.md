...
description: "Use when prioritizing work in an AI-assisted codebase, designing agent autonomy levels, deciding what to automate vs what to keep manual, or evaluating whether a proposed process or tool actually adds value. Applies Eliyahu Goldratt's Theory of Constraints to the AI-coding era's specific cost-structure inversion: code production has become cheap (agents generate it in minutes), so the system bottleneck has moved to the human activities that cannot be delegated — review, validation, decision-making, and learning. Covers the Five Focusing Steps (Identify / Exploit / Subordinate / Elevate / Repeat), the pre-AI vs post-AI cost structure, the constraint decision framework, four constraint-aware process-design rules (front-load agent work, batch constraint touches, present options not questions, eliminate false dependencies), five anti-patterns that waste the constraint, attention-audit ratios for measuring constraint health, and the constraint-shift model (when the bottleneck moves from review → strategic decisions → user research → market timing). Do NOT use for task-effort estimation (that is a sizing question), for backlog scoring with RICE / WSJF / ICE (that is a prioritization framework), or for routing a specific task to a specific model (that is an operational dispatch decision)."
version: 1.0.0
type: capability
