...
relations:
  ...
  boundary:
    - skill: api-design
      reason: "api-design owns the surface shape (routes, request/response schemas, pagination); http-semantics owns the protocol-level method/status/header contract that any HTTP API builds on."
    - skill: webhook-integration
      reason: "webhook-integration owns inbound provider mechanics (signing, retry, vendor-specific topics); http-semantics owns vendor-neutral HTTP method and status semantics."
    - skill: system-interface-contracts
      reason: "system-interface-contracts owns cross-boundary contract shapes regardless of transport; http-semantics is HTTP-specific."
  verify_with:
    - api-design
    - code-review
concept:
  definition: "HTTP semantics is the IETF-standardized contract layer (RFC 9110, 9111, 9112) that defines method meanings, status code families, request/response metadata, conditional requests, content negotiation, and caching — independent of any specific framework or language. It is the wire-level meaning that every HTTP API inherits."
  mental_model: |
    Six layers compose HTTP behavior. Most framework choices are local to one layer; mistakes that look like framework bugs usually live in the layer below.

    1. **Method** — what the request asks the server to do. RFC 9110 § 9 defines the canonical eight: GET, HEAD, POST, PUT, DELETE, CONNECT, OPTIONS, TRACE, plus PATCH (RFC 5789). Each has two semantic properties: **safe** (does not modify resource state — GET, HEAD, OPTIONS, TRACE) and **idempotent** (repeated identical requests have the same effect as one — GET, HEAD, PUT, DELETE, OPTIONS, TRACE). PATCH is neither safe nor idempotent by default; POST is neither.

    2. **Target** — the resource the method addresses. URI semantics (RFC 3986) plus the server's routing table. The target identifies *what* the method acts on.

    3. **Representation** — the bytes carried in the request or response body. Has metadata headers: Content-Type (media type per RFC 6838), Content-Encoding, Content-Language, Content-Location, Content-Length, and ETag/Last-Modified for caching.

    4. **Status** — the server's response code, in three-digit form with semantically loaded first digit (RFC 9110 § 15):
       - **1xx** — informational, processing continues.
       - **2xx** — success (200 OK, 201 Created, 202 Accepted, 204 No Content, 206 Partial Content).
       - **3xx** — redirect; client must take further action (301 Moved Permanently, 302 Found, 303 See Other, 304 Not Modified, 307 Temporary Redirect, 308 Permanent Redirect).
       - **4xx** — client error (400 Bad Request, 401 Unauthorized, 403 Forbidden, 404 Not Found, 405 Method Not Allowed, 409 Conflict, 410 Gone, 412 Precondition Failed, 413 Content Too Large, 415 Unsupported Media Type, 422 Unprocessable Content, 428 Precondition Required, 429 Too Many Requests).
    ...
    5. **Conditional / Caching** — headers that turn a request into a conditional or a cacheable resource (RFC 9111). The four conditional headers: If-Match, If-None-Match, If-Modified-Since, If-Unmodified-Since. The freshness model: Cache-Control directives (max-age, s-maxage, no-store, no-cache, private, public, must-revalidate, stale-while-revalidate, stale-if-error), Expires, Age, ETag, Last-Modified, Vary. Vary tells caches which request headers participate in cache-key construction.

    6. **Negotiation** — the Accept-* / Content-* dance. Accept (media types), Accept-Encoding (compression), Accept-Language, Accept-Charset on the request; Content-Type, Content-Encoding, Content-Language on the response. Vary on the response tells caches which request headers it varies on.

    Idempotency, the most frequently violated property, is about the **resource state on the server**, not about the response code. A DELETE that returns 204 the first time and 404 the second time is still idempotent — the post-condition (resource is gone) is the same after one or after many invocations.
  purpose: |
    HTTP is the lingua franca of distributed systems. Every API, every webhook, every browser request, every fetch call rides on the contract layer defined by RFC 9110/9111/9112. When developers treat HTTP as "whatever the framework gives me," subtle but load-bearing semantics break: idempotency violations cause double-charges on retry, cache-control mistakes serve stale data, conditional requests get bypassed, status codes become decorative rather than informative.

    The discipline of HTTP semantics is the discipline of reading the spec, not the framework documentation. Frameworks make convenient choices but they don't always make spec-conformant choices. A team that grounds method, status, and caching decisions in RFC 9110 builds APIs that interoperate cleanly with proxies, CDNs, retry middleware, and client libraries written in any language. A team that grounds them in "what the framework defaults to" builds APIs that work today and stop working when the proxy in front changes.
  boundary: |
    **HTTP semantics is not API design.** API design (use api-design) is the shape of the surface: routes, request schemas, response schemas, pagination, filtering, error envelopes. HTTP semantics is the contract layer below that: which method, which status, which header, by spec.

    **HTTP semantics is not transport.** The TCP/TLS/QUIC layer is below HTTP. RFC 9112 (HTTP/1.1 syntax), RFC 9113 (HTTP/2), and RFC 9114 (HTTP/3) are different syntax/wire layers; HTTP semantics is the consistent layer above them that all three preserve.

    **HTTP semantics is not webhooks.** Webhooks ride on HTTP but the relevant concerns (signing, retry, idempotency keys, vendor-specific topics) are inbound integration mechanics. Use webhook-integration.

    **HTTP semantics is not REST.** REST is one architectural style on top of HTTP (Fielding 2000). HTTP semantics is the underlying contract; REST is a set of constraints — uniform interface, statelessness, cacheability, layered system, code-on-demand, client-server. Many APIs that call themselves REST violate the spec; many APIs that don't call themselves REST are spec-conformant.

    **HTTP semantics is not WebSocket or SSE.** Those are different transport patterns. WebSocket is a separate protocol (RFC 6455) that uses HTTP only for the handshake. SSE (Server-Sent Events) reuses HTTP but is a streaming pattern; use a streaming-architecture skill for that.
  taxonomy: |
    - **REST** (specialization) — an architectural style constrained by HTTP semantics; not all HTTP APIs are RESTful, and not all RESTful APIs use HTTP correctly. Roy Fielding's 2000 dissertation defines REST's six constraints.
    - **GraphQL over HTTP** (alternative use) — a different query model that rides on HTTP but mostly uses POST + status 200. The HTTP semantics layer still applies to caching, content negotiation, and error reporting.
    - **gRPC over HTTP/2** (alternative use) — uses HTTP/2 as transport but has its own status code mapping. HTTP semantics for headers and method (POST) still apply.
    - **CONNECT and proxy semantics** (subset) — RFC 9110 § 9.3.6 defines CONNECT for tunneling; relevant when designing proxies, not direct APIs.
    - **Range requests** (composition) — RFC 9110 § 14 + RFC 7233 (now consolidated) define partial content with Range and Content-Range; relevant for large file downloads.
    - **Caching strategy** (composition with caching skill) — the freshness model (Cache-Control, ETag, Vary) is in scope; the cache architecture (CDN, reverse proxy, browser cache) is downstream.
  analogy: |
    HTTP semantics is to web systems what the standard library is to a programming language. Frameworks are like third-party libraries built on top: they make common operations ergonomic, but they don't define what those operations *mean*. The meaning is in the standard library — and for HTTP, that standard is RFC 9110/9111/9112.

    A developer who only knows their framework's HTTP helpers is like a developer who only knows their ORM's query builder: they can build working systems for the cases their tools cover, but they cannot reason about edge cases (retries, caching, conditional requests, content negotiation) because those live in the layer the tool abstracts away.
  misconception: |
    The most common misconception is that **idempotency is about response codes**. It is not. Idempotency is about the **post-condition on server state**. DELETE /orders/42 is idempotent because the post-condition (order 42 does not exist) is the same whether the request runs once or one hundred times — even though the response is 204 the first time and 404 every subsequent time.

    The second common misconception is that **PATCH is idempotent**. RFC 5789 says PATCH is neither safe nor idempotent by default. You can author idempotent PATCHes (e.g., "set status to 'shipped'"), but you can also author non-idempotent PATCHes (e.g., "increment quantity by 1"). The verb does not constrain the operation; the operation's design does.

    The third misconception is that **status codes are about UX**. 404 is not "show the user a 404 page" — it is "the target does not exist or the server will not disclose its existence." Choosing 404 vs 403 vs 401 is a security-relevant disclosure decision (RFC 9110 § 15.5.4). UX framing comes after the spec-correct choice.

    The fourth misconception is that **POST is for "create" and PUT is for "update."** RFC 9110 § 9.3.4 and § 9.3.5 define PUT as "the request payload is the new state of the target resource" (idempotent replacement) and POST as "perform a resource-specific operation that does not fit other methods" (catch-all, not idempotent). Either can create or update depending on whether the target exists. The "POST=create, PUT=update" shorthand collapses two different distinctions.
---
