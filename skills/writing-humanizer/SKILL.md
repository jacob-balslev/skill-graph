...
description: "Use when writing or editing prose that humans will read — documentation, PR descriptions, issue bodies, release notes, error messages, UI copy, commit messages, tooltips, support responses — or when text sounds robotic, padded, monotonous, or overly formal. Covers AI-tell detection and removal (Tier 1 zero-tolerance words like 'delve', 'testament', 'comprehensive', 'leverage'; Tier 2 conditional removals like 'utilize', 'facilitate'), passive-to-active voice conversion with the actor-known decision tree, hedging-pattern removal, readability scoring (Flesch-Kincaid, Gunning Fog, Flesch Reading Ease) used as diagnosis not target, sentence variety and the 3-beat short-long-medium rhythm, vocabulary-diversity rules, tone mapping across context types, AI-detection-avoidance techniques (perplexity, burstiness), paragraph rhythm with the hook-body-landing pattern, the bullets-vs-prose decision tree, anti-patterns (over-qualification, repetitive transitions, the enumeration trap, hollow intensifiers), and the 5-step humanization workflow. Provides the prose-quality humanization layer for any human-readable text — not the doc structure itself, not the casing convention, and not the in-product UI-text pattern catalog. Do NOT use for documentation routing or doc-type selection (use documentation), code-identifier naming (use semantics or naming-conventions), or the in-product UI-text pattern catalog (use microcopy)."
version: 1.0.0
type: capability
