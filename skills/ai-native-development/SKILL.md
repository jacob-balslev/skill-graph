...
keywords:
  ...
  - autonomy levels coding
  - ai code security regression
examples:
...
anti_examples:
  - "improve this specific prompt for the grader"             # → prompt-craft
  - "review this AI-generated PR for correctness"             # → code-review
  - "design the checkpoint state machine for our loop"        # → agent-engineering
  - "scaffold a new skill that codifies our coding doctrine"  # → skill-scaffold
  - "the autonomous loop is stalling — debug it"              # → debugging
relations:
  boundary:
    - skill: prompt-craft
      reason: "prompt-craft is the per-prompt authoring discipline; ai-native-development is the meta-frame that explains why prompts are source code"
    - skill: agent-engineering
      reason: "agent-engineering owns the production reliability discipline (orchestration, error budgets, observability); ai-native-development owns the conceptual model that motivates those concerns"
    - skill: code-review
      reason: "code-review evaluates the AI-generated output; ai-native-development frames why that output exists and how to size the gates around it"
    - skill: tool-call-strategy
      reason: "tool-call-strategy is the tactical layer for an agent's tool dispatch; ai-native-development is the conceptual layer above it"
  related:
    - prompt-craft
    - agent-engineering
    - code-review
...
The gates are also the *justification* for higher autonomy. Without the gates, level 4 is reckless. With the gates, level 4 is responsible.
...
| `code-review` | Reviewing the AI-generated code that comes out of a Software 3.0 workflow — this skill frames *why* the review is needed; code-review is *how* the review is done |
| `tool-call-strategy` | The tactical layer of which tool an agent should call when, in what order, with what fallback |
| `skill-router` | The cross-skill dispatch decision (which skill activates for a query) — this skill is meta about *why* a skill library exists at all |
