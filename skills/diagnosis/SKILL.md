...
schema_version: 3
name: diagnosis
description: "Use when facing an unknown software failure and unsure which debugging methodology to apply, when multiple symptoms suggest different root causes, or when an initial debugging attempt has not converged after a few iterations. Provides the triage-first diagnostic routing framework that classifies a failure into a problem class and routes it to the right specialist methodology before root-cause analysis begins. Covers nine symptom classes (Logic Error / Runtime Crash / Data Integrity / Timing & Race / Performance / Configuration / Security / Integration / Tooling-Build-Script-path) with a classification decision tree; the universal evidence-collection protocol plus class-specific evidence checklists; the technique-selection matrix (stack-trace reading, data-flow tracing, git bisect, differential comparison, instrumentation, MRE isolation, profiling, boundary probing) with time-cost and evidence prerequisites; the diagnostic confidence ladder (level 0 Symptom → 1 Classified → 2 Localized → 3 Root Cause → 4 Verified Fix) with stuck-state checkpoints; escalation criteria for switching approach or escalating to a human; cross-domain patterns (Cascade, Coincidence, Environment Ghost); diagnostic anti-patterns; and a structured diagnostic-session template. Do NOT use for actually executing scientific debugging (use `debugging` once triage has classified the failure), for code-quality review (use `code-review`), or for proactive observability setup."
version: 1.0.0
type: capability
