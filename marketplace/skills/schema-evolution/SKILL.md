...
metadata:
  ...
  relations: "{\"related\":[\"data-modeling\",\"database-migration\",\"indexing-strategy\",\"acid-fundamentals\"],\"boundary\":[{\"skill\":\"database-migration\",\"reason\":\"database-migration owns the mechanics of applying one concrete migration (ALTER TABLE statement, lock profile, batched backfill, CONCURRENTLY indexes, NOT VALID constraints, lock_timeout/retry on a single statement, ghost-table cutover, unpooled connections, rollback SQL); this skill owns the multi-release sequence of migrations and the deploy-coordination discipline that keeps every intermediate state compatible.\"},{\"skill\":\"indexing-strategy\",\"reason\":\"indexing-strategy owns which indexes the database should have; this skill owns how the index set evolves over time. Adding, validating, or removing an index is one type of schema change governed by this skill's discipline.\"}],\"verify_with\":[\"data-modeling\",\"database-migration\"]}"
...
- Owned by `indexing-strategy`: which indexes the database should have
