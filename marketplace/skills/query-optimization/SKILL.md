...
8. **Hand off durable changes.** Index portfolio changes go through indexing-strategy; schema/materialized-view changes go through data-modeling and database-migration; isolation/lock correctness goes through transaction-isolation.
...
| Designing schema, entity relationships, denormalization, or materialized views | `data-modeling` | Query optimization may diagnose the need, but data-modeling owns the model |
...
- choose the database schema and keys (use data-modeling)
...
**Related skills**
- Verify with: `data-modeling`, `indexing-strategy`, `transaction-isolation`, `performance-testing`
- Related: `schema-evolution`, `indexing-strategy`, `data-modeling`, `transaction-isolation`, `replication-patterns`, `performance-testing`, `database-migration`
