...
metadata:
  relations: "{\"related\":[\"naming-conventions\",\"refactor\",\"best-practice\",\"methodical\",\"no-cutting-corners\"],\"suppresses\":[\"skill-router\",\"skill-infrastructure\"]}"
  subject: agent-ops
...
Skill scaffolding is protocol-backed authoring, not free-form prompt writing. The scaffold separates the routing contract, field contract, concept model, grounding evidence, body sections, and verification gates so a new skill becomes a routable graph node rather than a decorative Markdown file.
...
2. **Rename** identity fields: `name`, `description`, `subject`, `public`, `scope`, `keywords`, `examples`, `anti_examples`, `paths` (if applicable), and the body title. Every reference to "skill-metadata-template" should be gone.
3. **Classify** the skill. Choose the primary `subject`, optional `subjects[]` when the skill genuinely spans two shelves, `public`, free-text `scope`, and `taxonomy_domain` only when the subject shelf is crowded enough to need a sub-path.
...
| Retired archetype language | New skill guidance asks for `type: capability` / `workflow` / `router` / `overlay` | Replace with v8 `subject`, `public`, free-text `scope`, and body structure chosen by intent |
...
**Related skills**
- Related: `naming-conventions`, `refactor`, `best-practice`, `methodical`, `no-cutting-corners`
