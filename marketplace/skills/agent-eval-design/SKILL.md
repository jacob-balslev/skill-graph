...
- Purpose: Agent behavior changes when prompts, models, tools, context, retrieval, policies, or orchestration change. Agent eval design prevents teams from discovering those regressions in production by making expected behavior, failure boundaries, and release thresholds executable before changes ship.
- Boundary: This skill designs reusable eval suites, graders, thresholds, hard negatives, and regression cases. It does not score one completed deliverable against evidence (evaluation), run eval suites as an iterative release process (eval-driven-development), choose product test levels (testing-strategy), operate skill-library health tooling (skill-infrastructure), investigate one failed run (debugging), extract project knowledge (project-knowledge-extraction), or design prompt-injection mitigations (prompt-injection-defense).
- Analogy: Agent eval design is a flight simulator: it creates repeatable weather, failures, and landing criteria before trusting the pilot in production airspace.
