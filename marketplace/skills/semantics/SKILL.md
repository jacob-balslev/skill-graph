...
- Owned by `semiotics`: multi-channel sign systems such as icons, badges, shapes, colors, visual metaphors, and signifier/signified coherence
...
- Owned by `api-design`
- Owned by `http-semantics`: the RFC 9110/9111/9112 method/status/header/cache contract in detail
- Owned by `type-safety`: compile-time and runtime type guarantees, unsoundness, narrowing, exhaustiveness, and validation boundaries
- Owned by `design-system-architecture`: token taxonomy, component APIs, theming, governance, and migration
- Owned by `theme-system-design`: the runtime theme contract and CSS-variable strategy
- Owned by `tool-call-flow`: the declaration/request/execution/continuation protocol
- Owned by `system-interface-contracts`: full boundary contracts across modules, agents, services, jobs, and APIs
- Owned by `observability-modeling`: telemetry strategy, trace/span/event/metric design, signal selection, dashboards, SLOs, and instrumentation boundaries
...
- Related: `linguistics`, `naming-conventions`, `semantic-relations`, `semiotics`, `microcopy`, `semantic-center`, `conceptual-modeling`, `taxonomy-design`, `api-design`, `http-semantics`, `type-safety`, `design-system-architecture`, `theme-system-design`, `tool-call-flow`, `system-interface-contracts`, `observability-modeling`
