...
**Known reconciliation (flagged, not yet resolved).** Pre-2026-06-05 artifacts violate this rule and are migration debt: the audit slash-commands in `.claude/commands/audit/*` and `.opencode/commands/*`, and several Skill Graph scripts under `~/Development/scripts/skill/*` (claim system, worklist builder, census, router). These were placed there by [ADR-0015](docs/adr/0015-audit-loop-spec-ownership.md) / [ADR-0016](docs/adr/0016-operational-data-ownership.md) on the rationale that slash-command *resolution* is a workspace-runtime concern (Claude Code resolves user-typed `/commands` only from `.claude/commands/`; OpenCode only from `.opencode/command/`). Reconciling the new directive with that mechanical constraint (full move vs. thin pointer-shims in `.claude/` that carry no logic and defer to `skill-graph/`) is an open SYSTEM decision requiring an ADR amendment — it is NOT done silently. Until then: author all new code in `skill-graph/`; do not add to the violation set.
