...
2. **Leave ONLY a thin pointer-shim where a runtime mechanically requires one.** Claude Code resolves user-typed `/commands` only from `.claude/commands/`; OpenCode only from `.opencode/command/`. So a slash command keeps a *minimal* file there that carries **no logic** — only frontmatter + a one-line pointer that defers to the `skill-graph/` implementation. A workspace consumer (hook, loop) that calls a moved script keeps a thin shim that `require`s/execs the `skill-graph/` canonical path. Shims are registered per `docs/reference/implementation-ownership.md`.
