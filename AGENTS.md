...
- `relations.suppresses` — score-aware exclusion guard: when this skill wins a query, listed skills are suppressed from co-routing. The verb matches the mechanic ("skill-A `suppresses` skill-B" = "when skill-A wins, skill-B is suppressed from the result set"). Canonical name as of [ADR-0018](docs/adr/0018-relations-boundary-semantic-inversion.md), which renamed it from `boundary` so the name no longer reads as deference. Use ownership reason-text ("I own this exclusively over X"), not deference ("use X instead").
...
- `description` — short description of what the skill is about. Activation signals belong to `keywords`/`triggers`/`examples`/`anti_examples`; boundary semantics belong to `relations.suppresses`.
...
- `public` — publishability / private-data gate (boolean). `true` = safe for the public skills.sh release; `false` = carries private API keys / personal / customer / internal-operational data and must not be published (the fail-safe default; the single switch the marketplace exporter filters on). Replaced the `deployment_target` enum (ADR-0017 amendment). Project anchoring (and the `grounding` requirement) is a separate axis carried by `project[]`. The `scope` field is a required free-text PRD-style statement (the v8 `scope` enum's `workspace` value was removed 2026-05-27; see ADR-0017 amendment).
...
- Requires the **five flat top-level Understanding fields**: `mental_model`, `purpose`, `concept_boundary`, `analogy`, `misconception`. The legacy top-level `boundary` alias and nested `concept` block (`definition / mental_model / purpose / boundary / taxonomy / analogy / misconception`) are deprecated and accepted only for back-compat; flat fields win when both are present.
...
| **Edges / relations semantics** (`related`, `suppresses`, `verify_with`, `depends_on`) — the field-level mechanic | `skill-metadata-protocol/SKILL_METADATA_PROTOCOL.md` § Relations | The deprecated `relations.boundary` alias exists only for unmigrated skills; `AGENTS.md § What the Skill Graph Is` summarizes and links. |
...
- **Project-grounded** when the skill is project-anchored (non-empty `project[]`, anchored to specific repo truth; or a legacy unmigrated skill still carrying `scope: codebase`); **principle-grounded** when the skill is ambient/portable (no `project[]`, repo-agnostic patterns). A skill that blends both records `grounding_mode: hybrid`.
...
8. **Understanding-fields gate (when `comprehension_state: present`)** — the five flat fields (`mental_model`, `purpose`, `concept_boundary`, `analogy`, `misconception`) populated (legacy top-level `boundary` alias and nested `concept` block accepted for back-compat only), no repo-specific nouns in the Understanding-field content.
...
> **Read the rationale first:** [`docs/skill-audit-loop-philosophy.md`](docs/skill-audit-loop-philosophy.md) — WHY the loop curates (never strips to a delta), WHY two fully-tooled frontier models research and curate, WHY the eval generator is a frontier model, and HOW agents use the curate→eval cycle. Do not change how the loop builds or scores skills without reading it.
...
- **Default eval generator (the measured agent) = a FRONTIER, deployment-matched model** (the `opus` alias for the single-direction default; each direction's frontier from `FRONTIER_PAIR` in the two-frontier bidirectional eval). Skills are deployed to and used by frontier agents, so a skill's value is whether it helps THAT agent; measuring lift on a weaker stand-in measures the wrong thing and the "ceiling effect" it avoids is the correct redundancy signal. This is the canonical decision in [`docs/skill-audit-loop-philosophy.md` § "WHY the generator (the measured agent) is a frontier model"](docs/skill-audit-loop-philosophy.md) and is enforced by `DEFAULT_COMPREHENSION_GENERATOR_MODEL = 'opus'`. It is **not** a `no-lesser-models` violation: that rule governs the GRADER (the judge), and a frontier generator is not a lesser model. The `representative-generator` role (Sonnet) remains in the registry for measurements that explicitly want an average-agent subject, but it is no longer the audit-loop eval default.
...
- **Core (certifying) vs advisory tier.** The certifying CORE is the two-frontier pair (`FRONTIER_PAIR` = Opus 4.8 ⇄ GPT-5.5). On top of it, an **opt-in advisory tier** (`ADVISORY_MODELS` = Gemini 3.1 Pro + the free OpenCode Zen models + Gemini 3 Flash) runs each advisory model as a **measured generator graded by a CORE frontier** (`runAdvisoryPanel`, `--advisory` / `AUDIT_ADVISORY_PANEL=1`). Advisory output is recorded in `advisory_panel` and **NEVER feeds reconciliation or sets a verdict** — it is breadth/novelty only. This honors `no-lesser-models-for-quality` (the GRADER is always a top-tier frontier; the advisory model is only the measured subject). Certification stays with the core. See [`docs/skill-audit-loop-philosophy.md` § "The advisory tier"](docs/skill-audit-loop-philosophy.md).
