...
- **New Skill Graph system code MUST be authored in `skill-graph/`.** The panel-enrich loop authored 2026-06-05 complies: `lib/audit/run-panel-enrich.js`, `lib/audit/panel-enrich-live-deps.js`, `prompts/skill-audit-loop-*.md` — all in `skill-graph/`. It deliberately has **no workspace `scripts/loop/dispatch-solver.js` dependency**; advisory dispatch is self-contained via the models' own CLIs.
