...
- **`application_verdict: APPLICABLE` requires honest model provenance.** If the grader or generator identity is unresolved, the run is not certifying and stays PROVISIONAL unless the eval contract explicitly records stronger evidence. A single same-family frontier judgment is also not enough: the representative-generator path certifies only when both frontier judges agree.
