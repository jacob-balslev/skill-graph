...
| **Audit loop procedure** (`Read → Verify → Evaluate → Research → Improve → Use → Evaluate → Grade`; the four operations; the two gates) | `skill-audit-loop/SKILL_AUDIT_LOOP.md` | The four-verdict Audit Status lives here. `AGENTS.md § Skill Audit Loop` summarizes and links. |
...
- **Default eval generator (the measured agent) = the `representative-generator` role** (currently Sonnet). The bidirectional eval keeps that measured subject fixed and asks both frontier judges (`FRONTIER_PAIR`) to grade the same with/without-skill evidence. This is the canonical decision in [`docs/skill-audit-loop-philosophy.md` § "WHY the generator (the measured agent) is representative"](docs/skill-audit-loop-philosophy.md) and is enforced by `DEFAULT_COMPREHENSION_GENERATOR_MODEL = REPRESENTATIVE_GENERATOR_MODEL` plus `APPLICATION_GENERATOR_DEFAULT_MODEL = REPRESENTATIVE_GENERATOR_MODEL`. It is **not** a `no-lesser-models` violation: that rule governs the GRADER (the judge) and AUTHOR (quality creator), while the generator here is the measured subject.
...
- **`application_verdict: HARMFUL` is deleted from the active tree, not archived.** A certifying-clean `HARMFUL` verdict means the skill makes agents worse. Resolve it by `git rm`-ing the active skill files so recovery lives in git history, and explain the harmful eval/verdict evidence in the commit message and task report. Do not move the harmful skill to a quarantine, deprecated, hidden, or marketplace path. A replacement must be a newly evaluated skill with a non-HARMFUL application verdict. Canonical details: [`docs/verdict-semantics.md`](docs/verdict-semantics.md).
