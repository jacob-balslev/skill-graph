...
| **Per-skill audit checklist + runbook** | `skill-audit-loop/AGENT_CONTEXT.yaml` first, then `skill-audit-loop/WORKFLOW_CONTRACT.md`, then `skill-audit-loop/SKILL_AUDIT_LOOP.md` § Part 2 (checklist) + § Part 3 (binding runbook) |
| **Delegating or scripting the Skill Audit Loop** | `skill-audit-loop/AGENT_CONTEXT.yaml` + `skill-audit-loop/WORKFLOW_CONTRACT.md` + `audits/workflow-conformance/spec.yaml` before any script or shell delegation |
...
- **Triangulate** — which related, dependent, suppressing, and verification skills should be loaded with it.
...
1. **Skill Metadata Protocol — the per-skill contract (the substrate).**
   *What it is supposed to do:* make each skill's relevance **explicit**. Every `SKILL.md` declares typed frontmatter — scope, area/category, domain, file surfaces (`paths` / `grounding`), dependencies, related skills, and exclusion boundaries (what it is *not* for). This is the contract that turns "name + description" into something a machine can route, group, and check. It is the substrate: without typed fields there is no graph to query and no deterministic gate to audit against. It turns a skill from "a Markdown instruction file" into a **machine-readable relevance contract** that can be linted, compiled into a manifest, routed against, drift-checked, and audited. **When you author or edit a skill, your job is to make its relevance and boundaries true and explicit.** Binding docs: `skill-metadata-protocol/SKILL_METADATA_PROTOCOL.md`, `skill-metadata-protocol/field-reference.md`.

2. **Skill Graph — the library-level system (what reasons over the metadata).**
   *What it is supposed to do:* operate on that metadata across the **whole library**. Route a query to the best-matching skill; show which skills are related, which domains/groups they belong to, and where boundaries lie; detect overlap and boundary violations; drift-check grounding against source-of-truth; and export back to portable `SKILL.md`. It turns a folder of skills into a typed graph an agent, router, or human can reason over. Each node is a `SKILL.md`; each edge is a typed relation such as `relations.related`, `relations.suppresses`, `relations.verify_with`, or `relations.depends_on`. It supplies schema validation, manifest compilation, routing, clustering, overlap detection, drift sentinel checks, audit artifacts, evals, and export back to plain `SKILL.md`. It is **build-time / authoring-time tooling — not** an agent runtime, hosted marketplace, or memory system; consumers read from the graph, they do not redefine it. Authority map: `SKILL_GRAPH.md`.

3. **Skill Audit Loop — the discipline that keeps it honest (what the structure enables).**
   *What it is supposed to do:* answer **one question per skill** — *does it still teach an agent to do the thing it claims to teach?* The Skill Audit Loop is the umbrella lifecycle: `Read → Verify → Evaluate → Research → Improve → Use → Evaluate → Grade`. The lowercase `audit` operation is the report-only Integrity-Gate command inside that lifecycle. Candidate changes are kept or reverted by Karpathy discipline: revert only on genuine regression, never just because a narrow eval did not reward a useful curation. Its four operations are `audit`, `improve`, `evaluate`, and `evolve`. It has two gates: the **Integrity Gate** proves the skill is structurally valid, grounded, routable, and export-safe; the **Behavior Gate** (comprehension) proves the skill deepens agent understanding on realistic scenarios, hard negatives, and boundary cases. The three-verdict Audit Status records the result on the skill itself: `structural_verdict`, `truth_verdict`, and `comprehension_verdict` (the behavior-gate quality signal). Behavior verdicts are evidence-gated and receipt-backed; see [`docs/verdict-semantics.md`](docs/verdict-semantics.md) for the canonical enum definitions, the eligibility-vs-assessment distinction, and confidence ordering. It is **not a lint-test factory**; an empty findings report on a genuinely good skill is a PASS. Procedure: `skill-audit-loop/SKILL_AUDIT_LOOP.md`; execution context: `skill-audit-loop/AGENT_CONTEXT.yaml`; workflow/BDD/script wiring: `skill-audit-loop/WORKFLOW_CONTRACT.md`.
...
| `audit --graded` | + grader CLI | (no new sidecar verdicts — same Integrity-Gate fields as `audit`) | graded qualitative scorecard rows for the seven dimensions. Does NOT stamp `comprehension_verdict` — that comes from `evaluate --mode comprehension` (row below). |
...
**CONTENT mode** changes individual skills *through* the contract. Edits to specific `SKILL.md` files, their `comprehension.json` eval artifacts, their `references/`, and their Audit Status stamps. CONTENT commits are the *output* of running the audit loop against the CURRENT contract — never an ad-hoc batch edit.
...
| Designing a "compatibility window" / "sunset phase" / "migration mode" that forces every new author to write BOTH old and new fields until the corpus migrates | CONTENT state leaked into SYSTEM thinking. The schema's correctness is independent of how many skills currently comply. Holding the schema hostage to corpus migration produces phrases like "must author BOTH" in normative docs and traps the schema in a perpetual phase state. | Make the schema correct in one edit: move new fields into `required`, delete prior-version field definitions and redundant conditional rules, and record the historical cut in git/CHANGELOG/ADR notes. Skills that fail validation route to ONE CONTENT-mode follow-up ticket the audit loop drains per-skill. See `~/Development/.claude/rules/version-schema-contract.md § Companion rule — Do not hold the schema hostage to corpus migration progress`. |
...
## Canonical Location — Skill Graph Code Lives in `skill-graph/`

> **User directive (2026-06-05):** *"All work related to the Skill Graph project is supposed to be in the `skill-graph/` repo — besides the individual skill work in `skills/skills/`. All scripts and commands belonging to the Skill Graph project should live in the `skill-graph/` repo and not in `.claude/` or `.opencode/`."*

**The rule.** Every Skill Graph script, library module, prompt, and command implementation lives in `skill-graph/` (`lib/`, `scripts/`, `bin/`, `prompts/`, `skill-audit-loop/`, `schemas/`, `docs/`). The ONLY Skill Graph content outside `skill-graph/` is the individual `SKILL.md` files (+ their per-skill artifacts) in `~/Development/skills/skills/**`, which are CONTENT, not system code.

- **New Skill Graph system code MUST be authored in `skill-graph/`.** The Skill Audit Loop (multi-model panel) authored 2026-06-05 complies: `lib/audit/run-skill-audit-loop.js`, `lib/audit/skill-audit-loop-live-deps.js`, `prompts/skill-audit-loop-*.md` — all in `skill-graph/`. It deliberately has **no workspace `scripts/loop/dispatch-solver.js` dependency**; advisory dispatch is self-contained via the models' own CLIs.
- **Do not add Skill Graph scripts to `~/Development/scripts/` or commands to `~/Development/.claude/commands/` or `~/Development/.opencode/commands/`.** If you reach for one of those, the code belongs in `skill-graph/` instead.

**Migration of existing violations — DECIDED 2026-06-05: move with thin pointer-shims.** Pre-2026-06-05 artifacts violate this rule and are migration debt: the audit slash-commands in `.claude/commands/audit/*` and `.opencode/commands/*`, and the Skill-Graph-project scripts under `~/Development/scripts/skill/*` (claim system, worklist builder, census, router, etc.). These were placed there by [ADR-0015](docs/adr/0015-project-owned-operational-prompts.md) / [ADR-0016](docs/adr/0016-operational-data-ownership.md). The user's directive supersedes that placement. The migration:

1. **Move the implementation into `skill-graph/`** (`git mv` — history preserved): script bodies → `skill-graph/scripts/` (or `lib/`/`bin/` as fits); command bodies → `skill-graph/` (the canonical contract already lives in `skill-audit-loop/SKILL_AUDIT_LOOP.md`).
2. **Leave ONLY a thin pointer-shim where a runtime mechanically requires one.** Claude Code resolves user-typed `/commands` only from `.claude/commands/`; OpenCode only from `.opencode/commands/` (plural — verified 2026-06-05 against [opencode.ai/docs/commands](https://opencode.ai/docs/commands/) + the live dir, OpenCode 1.15.10). So a slash command keeps a *minimal* file there that carries **no logic** — only frontmatter + a one-line pointer that defers to the `skill-graph/` implementation. **Exception — verbatim-consumed loop prompts:** an `.opencode/commands/*-loop.md` registered as a `"prompt"`-type loop in `scripts/loop/run-loop-sweep.sh` is read **verbatim** as the autonomous dispatcher's prompt; replacing its body with a pointer-shim guts the loop. Such files are workspace loop-orchestration (they drive `.opencode/progress/` queue data per ADR-0016, and may be Sales-Hub-grounded) and stay in place — they are NOT a canonical-location violation. A workspace consumer (hook, loop) that calls a moved script keeps a thin shim that `require`s/execs the `skill-graph/` canonical path. Shims are registered per `docs/reference/implementation-ownership.md`.
3. **Update every reference** across `*.md`, `*.sh`, `*.json`, `package.json` to the new `skill-graph/` path in the same change, and **amend ADR-0015/0016** with the supersession block.

**Deprecated / dead code is DELETED to git history, never archived (user directive 2026-06-05; see [`~/Development/.claude/rules/delete-dont-archive.md`](../.claude/rules/delete-dont-archive.md)).** As part of enforcing the canonical structure: any version-controlled script or code that is deprecated, superseded, or dead is **flagged deprecated, then removed with `git rm`** — git history is the archive. Do NOT leave `_archive/` copies, `.legacy` renames, commented-out blocks, or "kept just in case" duplicates in the tree. A duplicate implementation created by the migration (e.g. an old workspace copy left behind after the body moved to `skill-graph/`) is dead code → delete it (or convert exactly one entrypoint into a registered shim). The deletion commit records what + why + the recovering `git` command.

**When you flag/remove something deprecated, write a short explanation in `CHANGELOG.md` (user directive 2026-06-05).** A deprecation or deletion is not self-explanatory from the diff alone — a future agent finding the artifact gone (or a shim in its place) needs to know what happened and why. Add a one-paragraph `### Deprecated` / `### Removed` entry to `skill-graph/CHANGELOG.md § [Unreleased]` naming the artifact, why it was deprecated/removed, its replacement (or shim target), and the recovering `git` command. The CHANGELOG is the durable record agents read; the commit message alone is not enough.

**Version-controlled timestamps carry TIME-OF-DAY, not just the date (user directive 2026-06-05).** We often do several work sessions in one day, so a date-only stamp (`2026-06-05`) collides and cannot order intra-day work. Every timestamp written into a version-controlled artifact — CHANGELOG entries, `### Deprecated`/`### Removed` notes, doc "Updated …" stamps, deprecation flags, ADR amendment blocks, audit run records — uses a date **plus** time-of-day in a sortable form: ISO-8601 like `2026-06-05T14:32Z` (UTC) or `2026-06-05T14:32+02:00`. Run-dir naming already does this (`2026-06-05T0855--…`); extend the same precision to the prose stamps. (Pass timestamps in explicitly — do not rely on a tool that only emits a date.)

Until a given artifact is migrated: author all NEW code in `skill-graph/`; never add to the violation set.
...
- Current in-repo version: `0.5.10` on `2026-05-25` (the "canonical-shape sweep" release; ADR-0014/0015/0016 fully landed and the 2026-05-25 multi-model restructure review backlog closed; see [`CHANGELOG.md`](CHANGELOG.md#0510--2026-05-25)). **npm-published `latest` is `0.5.8`** — `0.5.9`/`0.5.10` are committed bumps not yet pushed through the manually-gated CI publish workflow (`.github/workflows/publish.yml`, `workflow_dispatch`). Publishing `v0.5.10` is a maintainer CI action; until then `npm install -g @skill-graph/cli` resolves `0.5.8` (recorded 2026-06-08, SKI-16).
- Current skill contract: see [`SKILL_GRAPH.md § Current State`](SKILL_GRAPH.md#current-state--single-source-of-truth) — single source of truth for schema version, axes required, audit-loop maturity, and corpus counts. The three-verdict Audit Status (`structural_verdict` / `truth_verdict` / `comprehension_verdict`) is the current audit surface; behavior claims require real artifacts and receipt-backed evaluation.
- This repo is the **canonical consolidated implementation** post-2026-05-18 (commit `654b4df`; see [ADR 0009](docs/adr/0009-sibling-repo-deprecation.md) and SH-6137). Schemas, audit scripts, graders, eval fixtures, examples, and the protocol/audit canonical docs all live here. The canonical skill library lives at `/Users/jacobbalslev/Development/skills/`; the current corpus count is generated in `docs/status.generated.md` + the live `find` command (never hand-stamped — see § Doc Ownership Map "Current corpus state"). The previously separate `skill-metadata-protocol` and `skill-audit-loop` mirrors are preserved as docs-only deprecation mirrors — they were **archived (read-only) on GitHub on 2026-05-20** ([ADR 0009 § Update](docs/adr/0009-sibling-repo-deprecation.md)) and no longer carry active source code. `examples/` still holds specimen projects and per-skill comprehension evals.
- Workspace config at `.skill-graph/config.json` points lint, manifest, drift, route, and truth_source resolution at the canonical sibling skills repo by default. `SKILL_GRAPH_WORKSPACE` env-var still overrides — useful when developers clone the canonical repo elsewhere. Truth_source paths starting with `skills/<name>/SKILL.md` are resolved via `scripts/lib/roots.js::resolveTruthSourcePath()`: skill-library-aware first, with REPO_ROOT fallback. Non-`skills/` truth_sources resolve against REPO_ROOT (the tooling repo) by default, EXCEPT for workspace-anchored skills (whose `project[]` includes the surrounding workspace, e.g. `development`): for those, `skill-graph-drift.js` passes the workspace root (`path.dirname(REPO_ROOT)`) as an existence-checked `extraRoots` candidate, so workspace-relative sources (`AGENTS.md`, `docs/reference/...`, `.claude/rules/...`, `skill-graph/...`) resolve correctly instead of reporting BROKEN (2026-06-11T06:07Z, see CHANGELOG).
...
- `relations.suppresses` — score-aware exclusion guard: when this skill wins a query, listed skills are suppressed from co-routing. The verb matches the mechanic ("skill-A `suppresses` skill-B" = "when skill-A wins, skill-B is suppressed from the result set"). Use ownership reason-text ("I own this exclusively over X"), not deference ("use X instead").
...
- `relations.disjoint_with` — formal class-disjointness assertion; rare, and distinct from the routing-layer exclusion mechanic in `relations.suppresses`.

The graph is not a documentation pile. It is queried by routers, traversed by injection hooks, evaluated against retrieval baselines, and drift-checked against source-of-truth files. Two skills are "the same kind" iff they share a `subject × public` pair plus a head noun (per [ADR-0017](docs/adr/0017-five-axis-classification-model.md)). The routing layer uses that pair as a first-pass discriminator before walking edges. Note: `public` is a binary gate (most skills are `public: true`), so the pair provides only broad stratum separation, not fine-grained routing precision — keyword score, `project[]` project-fit, and relation edges carry the discriminating load. See `skill-metadata-protocol/SKILL_METADATA_PROTOCOL.md` § Classification for the current distribution analysis and authoring implications.
...
**Two files per skill (ADR-0019).** A skill is `SKILL.md` (31 agent-facing frontmatter fields; `required`: `name`, `description`, `subject`, `public`, `scope`) **plus** a sibling `audit-state.json` sidecar (29 audit/eval/provenance fields the Skill Audit Loop owns; schema `schemas/skill-audit-state.schema.json`; `required`: `schema_version`, `owner`, `freshness`, `drift_check`, `eval_artifacts`, `eval_state`, `routing_eval`). `schema_version`, `version`, `owner`, the three Audit Status verdicts, `eval_*`, `drift_check`, `lifecycle`, `portability`, `runtime_telemetry`, `comprehension_state`, `marketplace_tier` all live in the sidecar, NOT frontmatter. The manifest join recombines them. Author the sidecar from `examples/skill-audit-state-template.md`.
...
- `description` — short description of what the skill is about. Activation signals belong to `keywords`/`triggers`/`examples`/`anti_examples`; exclusion semantics belong to `relations.suppresses`.
- `subject` — primary browse shelf (the competency the skill teaches), ONE of twelve closed values in 3 bands: **engineering** `backend-engineering`, `frontend-engineering`, `software-architecture`, `data-engineering`; **AI-agentic** `agent-ops`, `ai-engineering`; **cross-cutting** `quality-assurance`, `design`, `reasoning-strategy`, `software-engineering-method`, `knowledge-organization`, `product-domain`. Shelf-size guidance is advisory (no hard cap — relaxed 2026-06-15): there is no maximum number of skills per subject; subdivide a large shelf via `taxonomy_domain` only when routing precision actually degrades, never at a fixed count, and a small shelf is fine when its members are genuine content with a growth path. To propose a 13th value: ADR + ≥5 primary-fit skills. See [ADR-0020](docs/adr/0020-twelve-shelf-competency-reaxis.md) for the current shelf rationale.
- `public` — publishability / private-data gate (boolean). `true` = safe for the public skills.sh release; `false` = carries private API keys / personal / customer / internal-operational data and must not be published (the fail-safe default; the single switch the marketplace exporter filters on). Project anchoring (and the `grounding` requirement) is a separate axis carried by `project[]`. The `scope` field is a required free-text PRD-style statement.
- `subjects[]` (optional, max 2, primary first) covers polyhierarchy when a skill genuinely spans two browse shelves.

Activation surfaces: `keywords` (≤10), `triggers` (exact), `examples` / `anti_examples` (positive/negative prompts), `paths`, `dependencies`, `codebase_layer`, `applicable_tasks`, `environment`, and `internal_tools`. Routing edges live in `relations` (`related` / `suppresses` / `verify_with` / `depends_on` / `broader` / `narrower` / `disjoint_with`).
...
- Requires the **five flat top-level Understanding fields**: `mental_model`, `purpose`, `concept_boundary`, `analogy`, `misconception`.
...
A version number on a skill — `schema_version`, `skill_graph_protocol`, and any `vN` label — asserts that the skill's **content** meets that version's bar. Advancing the number is honest only after the substantive migration the version represents has actually been performed. Bumping a label without doing that work is fake-conformance: the same class of doc-lie as `eval_state: passing` without an `eval_last_run` receipt, or a positive behavior verdict without the required eval receipt.

- **`schema_version` is the mechanical shape; the content label is the substantive bar.** Migration codemods bump `schema_version` corpus-wide — a shape migration a script *can* do. The deeper content v8 carries — `subject` + `public` classification, polyhierarchy via `subjects[]` (max 2), free-text `scope`, hierarchical `taxonomy_domain`, `project[]` and `repo[]` belonging-entity references, capped `keywords`, the typed `relations` graph, the three-verdict Audit Status, and the five flat Understanding fields with `comprehension_state` — is **not** something a codemod can author. A skill only earns the v8 label when that content is actually present and reviewed.
...
## The Latest Canonical Schema IS the Product. Corpus State Is Backlog, Never a Design Finding.

**The product is defined by the latest canonical `schemas/SKILL_METADATA_PROTOCOL_schema.json` (Tier 1, which wins over all prose).** The on-disk corpus is at varying stages of migrating *toward* that schema. That gap is normal, in-progress work — it is not, and must never be reported as, a defect in how the product is designed.

- **Judge design against the schema; judge progress against corpus conformance; never conflate the two.** When an agent assesses whether the skill system is well-designed, it judges the latest schema (and the protocol/audit docs that describe it). When it assesses how far migration has come, it counts how many skills conform. A finding like "113 skills lack the v8 fields" / "two-thirds have no `audit-state.json`" / "this skill's `SKILL.md` looks different from the v8 shape" / "subject X has only 3 skills" is a **statement about migration progress**, not a design problem — file it (if at all) as audit-loop backlog, never as a protocol/system defect.
- **A skill↔schema mismatch is honest drift, not wrongness.** Per § Version Labels Are Earned, a skill still on an older shape (or carrying empty/older version fields) is correctly recording that the migration has not reached it yet. The fix is to run the skill through `/audit:*`, never to read the mismatch as the schema being wrong.
- **Greenness is gated PER-SKILL.** `verify:system` (the SYSTEM gate — schema, scripts, docs self-consistency) stays green while the corpus is mid-migration. Corpus completeness ("are all N skills migrated?") is a separate `verify:corpus` concern and is **never folded into `verify:system`** — a library-level red from unmigrated skills is CONTENT backlog, not a SYSTEM integrity failure. Companion memory: `skill-graph-per-skill-greenness-not-corpus-completeness`.
- **Why this section exists:** a 2026-06-08 design review repeatedly reported in-progress corpus state (un-migrated skills, empty version fields, partial sidecar coverage, subject-count imbalance) as if those were defects in the product's design. They are not. The schema is the product; the corpus is catching up to it.
...
- It fills **mechanical** fields only (those with a single honest default: `eval_artifacts`/`eval_state`/`routing_eval`, the four UNVERIFIED Health verdicts) and **never authors a semantic field** (`scope`, `subject`, `public`, Understanding-field prose) — those route to `/audit:*`.
...
- **A skill is two schemas (ADR-0019).** `schemas/SKILL_METADATA_PROTOCOL_schema.json` governs agent-facing `SKILL.md` frontmatter; `schemas/skill-audit-state.schema.json` governs audit/eval/provenance fields in the `audit-state.json` sidecar. A field-placement change (frontmatter ⟷ sidecar) touches BOTH. `check-schema-constants.js` asserts the frontmatter `required` (5 core) AND the sidecar `required` (7) and reads `schema_version`/the three verdicts from the sidecar; `check-protocol-consistency.js` C1 unions both schemas; `build-field-reference.js` covers both; `skill.context.jsonld` covers frontmatter only (the sidecar is never exported/RDF'd). The cross-file gates (`comprehension_state`↔Understanding, `stability`↔`superseded_by`) live in `skill-lint.js`, not in either schema's `allOf`. The manifest join (`generate-manifest.js`) merges the sidecar under the frontmatter so manifest `health`/`eval`/`lifecycle` projections are byte-identical.
...
- If you change `schemas/manifest.schema.json`, also update `docs/manifest-field-mapping.md`, and `scripts/generate-manifest.js` if projection logic changes.
...
- If you change a gate script (`scripts/skill-lint.js`, `scripts/skill-graph-drift.js`) or add/modify a structural, truth, or protocol-field rule, add or update the matching Given/When/Then scenario (and its one-rule-broken negative fixture) in `audits/gate-conformance/spec.yaml`, then run `node scripts/__tests__/test-gate-conformance.js`. That suite is the executable statement of the per-skill gate criteria — a new rule without a conformance scenario is an untested rule. The fixtures live under `audits/gate-conformance/fixtures/invalid/<rule>/`, outside every corpus sweep; never put invalid fixtures under `examples/fixture-skills/` (the `verify:system` lint pass would fail on them). See `audits/gate-conformance/README.md` for the gate→coverage map and the "add a scenario" steps.
- If you change the Skill Audit Loop agent entry context, prompt read order, workflow docs, or a script that depends on audit-loop context, update `skill-audit-loop/AGENT_CONTEXT.yaml`, `skill-audit-loop/WORKFLOW_CONTRACT.md`, `audits/workflow-conformance/spec.yaml`, and the affected prompt/command docs in the same change, then run `node scripts/__tests__/test-workflow-conformance.js`. That suite is the executable statement of the docs → ADRs → BDD → scripts context chain agents must understand before execution.
...
For any single Skill Metadata Protocol field (`name`, `description`, `subject`, `public`, `scope`, `stability`, `freshness`, `eval_state`, `routing_eval`, `comprehension_state`, `relations.*`, `grounding.*`, etc.):
...
| **Decision logic between values** — "when should `public` be true?", "when is `stability: stable` earned?", branching decision trees | `skill-metadata-protocol/field-decision-guide.md` | Decision tables only. Do not duplicate decision logic from `SKILL_METADATA_PROTOCOL_field-reference.md`; link to it. |
...
| **Current corpus state** — verified counts (canonical skills, exported skills, worklist), build/health snapshot | `docs/status.generated.md` (generated SoT) + the live `find … -name SKILL.md \| wc -l` command | **Counts are generated, never hand-stamped.** The single source is the generated `docs/status.generated.md` (owned by `scripts/build-status-doc.js`) plus the live command — so a corpus change never forces a doc sweep. `SKILL_GRAPH.md § Current State` and every other doc **link** to them; they never restate the number. Enforced by `scripts/check-inline-skill-counts.js` (wired into `verify` + `verify:system`); historical records (CHANGELOG, ADRs, plans, research, audits) carry dated as-of counts that must NOT be re-stamped. |
| **Edges / relations semantics** (`related`, `suppresses`, `verify_with`, `depends_on`) — the field-level mechanic | `skill-metadata-protocol/SKILL_METADATA_PROTOCOL.md` § Relations | `AGENTS.md § What the Skill Graph Is` summarizes and links. |
| **Audit loop procedure** (`Read → Verify → Evaluate → Research → Improve → Use → Evaluate → Grade`; the four operations; the two gates) | `skill-audit-loop/SKILL_AUDIT_LOOP.md` | The three-verdict Audit Status lives here. `AGENTS.md § Skill Audit Loop` summarizes and links. |
| **Agent context packet for Skill Audit Loop work** (mission, vision, goal, rules, required docs, BDD suites, scripts, display contract) | `skill-audit-loop/AGENT_CONTEXT.yaml` | Machine-readable context bundle agents and delegators must read before audit-loop scripts. Kept aligned with `skill-audit-loop/WORKFLOW_CONTRACT.md` and `audits/workflow-conformance/spec.yaml`. |
| **Workflow-conformance BDD** (agent entrypoint, lifecycle step, metrics, prompt read order, script context wiring) | `audits/workflow-conformance/spec.yaml` (+ runner `scripts/__tests__/test-workflow-conformance.js`, contract `audits/workflow-conformance/README.md`) | The declarative source of truth for "entrypoint X must point agents to docs/ADRs/BDD/scripts/context Y." Covers agent understanding and workflow wiring; it does not replace judged behavior evaluation. |
...
| **Verdict semantics** (enums for all three Audit Status fields, confidence tier ordering) | `docs/verdict-semantics.md` | Canonical for verdict enums. AGENTS.md / skill-audit-loop/SKILL_AUDIT_LOOP.md / pipeline doc carry one-line summaries that link here. ADR-0011 is the rationale; this doc is the spec. |
| **Gate pass/fail/relevance criteria** (executable Given/When/Then for the per-skill structural + truth gates and the protocol field rules) | `audits/gate-conformance/spec.yaml` (+ runner `scripts/__tests__/test-gate-conformance.js`, contract `audits/gate-conformance/README.md`) | The declarative source of truth for "input X → gate yields verdict/error Y", executed against the live gate scripts so the spec cannot drift from them. Covers the per-skill-drivable structural (`skill-lint.js`) + truth (`skill-graph-drift.js`) gates; corpus-scoped gates (verdict-honesty) are referenced there and covered by their own unit tests, not re-driven. `docs/verdict-semantics.md` (row above) owns the verdict *enum definitions*; this owns the *input→outcome* scenarios. |
...
| **Run-dir layout** (per-skill `runs/<run-id>/` structure, artifact filenames, claim atomicity) | `skill-graph/lib/audit/run-layout.js` (project-owned CONTRACT — the canonical naming/structure, **MIGRATED 2026-06-02 per ADR-0016 surface #3**) | `lib/audit/run-layout.js` is the pure, fs-free layout contract (run-dir naming, tier helpers, sub-path names). The workspace `scripts/skill/skill-audit-paths.js` is the runtime adapter: it `require`s the project module, binds it to the absolute run-root `<ws>/skill-graph/skill-audit-loop/progress/skill-audits` (relocated 2026-06-07T from `.opencode/progress/skill-audits` per the ADR-0016 surface #3 supersession; single source of truth = `AUDIT_ROOT`/`AUDIT_ROOT_SEGMENTS` there), and owns the fs I/O (mkdir, atomic symlink, readlink) — so the naming contract is delegated, never duplicated. The run-root is gitignored transient scratch (`skill-graph/.gitignore` `skill-audit-loop/progress/`); it must never be committed to this public repo. Edit the contract in `run-layout.js`; edit only root/I/O in `skill-audit-paths.js`. |
| **Finding schema** (severity, evidence, scope, recommended action) | `skill-graph/audits/finding.schema.json` (project-owned machine shape — **MIGRATED 2026-06-02 per ADR-0016 surface #5, SH-6608**) | `audits/finding.schema.json` is the binding shape for a `skill-audit.finding/v1` finding (draft 2020-12). The workspace `docs/reference/skill-audit-pipeline.md § Unified Finding Schema` keeps the prose source-of-truth + mapping rules and now defers to this schema for the field shape. |
| **Grader prompts** (concept grader for gate 8) | `lib/audit/graders/*-prompt.md` (**canonical, sole copy** since SH-6603) | The grader prompts are now single-canonical here. The divergent workspace `scripts/skill/graders/` copies were collapsed by **SH-6603 (2026-05-28)**: the package `concept-grader-prompt.md` absorbed the workspace's "Anti-Compression Mandate" + "Calibration Reference Set" block (the workspace had been AHEAD there), the workspace `scripts/skill/graders/` directory was removed, and `scripts/skill/evaluate-skill.js` became a thin shim that loads these package prompts. The workspace `Development/scripts/docs/doc-verification-gates.js` COMPRESSION_TRIGGER_FILES set was repointed to these package paths in the same change (caveat: skill-graph is a nested git repo, so a Development-side `git diff` does not surface edits to them — the cross-repo compression-trigger gap is a follow-up). |
...
| **Skill-injector routing config — shape** | `schemas/routing-config.schema.json` (project-owned per ADR-0016 surface #4 — **schema AUTHORED 2026-05-27; WIRED 2026-06-02**) | The binding shape for `agent-orchestration/references/skill-routing-config.json` (instance file, cross-project orchestration). Type lives here; instance stays at the consumer location. Enforced by `scripts/check-routing-config.js` (`npm run routing-config:check`, wired into `verify` + `verify:system`): a Node-built-in structural validator (required-keys + top-level type — no ajv dep, per repo policy) that resolves the instance via `$SKILL_ROUTING_CONFIG` or the default sibling path and SKIPS gracefully (exit 0) for standalone clones where the instance is not reachable. |
...
| **Quality doctrine** (improve = curate; preserve scope; organize-over-trim) | `docs/quality-doctrine.md` | `AGENTS.md § Quality Doctrine` is a summary; deeper doctrine lives in the docs file. |
...
| Hand-writing a corpus count (e.g. "N canonical skills", a bolded marketplace total) into `SKILL_GRAPH.md` / `README.md` / `AGENTS.md` / topic docs | Counts are generated in `docs/status.generated.md` + the live command. Everywhere else links; never restate the number. `scripts/check-inline-skill-counts.js` fails the build on any inline count. Inline counts go stale within days and force token-expensive sweeps. |
...
| **`marketplace/skills/<name>/SKILL.md`** (the mirror) | _generated_ — do not hand-edit. Edit the canonical source at `~/Development/skills/skills/<subject>/<name>/SKILL.md` and re-run `node scripts/export-marketplace-skills.js` |
...
| **Skill audit loop or checklist** (`scripts/skill-audit.js`, `lib/audit/*`) | `skill-audit-loop/SKILL_AUDIT_LOOP.md`, `skill-audit-loop/SKILL_AUDIT_LOOP.md` § Part 2, `skill-audit-loop/WORKFLOW_CONTRACT.md`, `skill-audit-loop/AGENT_CONTEXT.yaml`, `AGENTS.md` § Skill Audit Loop |
| **Skill Audit Loop TUI / operator surface** (`tui/`, `schemas/session.schema.json`, `lib/audit/panel-liveness.js`) | `skill-audit-loop/SKILL_AUDIT_LOOP.md` § Surface ordering, `CHANGELOG.md`, `schemas/session.schema.json` if session events/snapshots change |
| **Audit-loop agent context, workflow contract, prompt read order, or script context dependency** (`skill-audit-loop/AGENT_CONTEXT.yaml`, `skill-audit-loop/WORKFLOW_CONTRACT.md`, `prompts/skill-audit-loop-*.md`, `.claude/commands/audit/*`, context-dependent scripts) | `skill-audit-loop/AGENT_CONTEXT.yaml`, `skill-audit-loop/WORKFLOW_CONTRACT.md`, `audits/workflow-conformance/spec.yaml`, affected prompt/command docs. Run `node scripts/__tests__/test-workflow-conformance.js`. |
| **Gate script or new structural/truth/protocol rule** (`scripts/skill-lint.js`, `scripts/skill-graph-drift.js`, schema field rules) | `audits/gate-conformance/spec.yaml` (add/update the scenario), `audits/gate-conformance/fixtures/invalid/<rule>/` (the negative fixture), `audits/gate-conformance/README.md` if the coverage map changes. Run `node scripts/__tests__/test-gate-conformance.js`. See also `## Coupled Changes`. |
...
- **Project-grounded** when the skill is project-anchored (non-empty `project[]`, anchored to specific repo truth); **principle-grounded** when the skill is project-agnostic (no `project[]`, repo-agnostic patterns). A skill that blends both records `grounding_mode: hybrid`.
...
The audit loop is the disciplined sweep that validates a skill (or a batch of skills) against the Skill Metadata Protocol, drift baselines, retrieval evals, and the quality doctrine. Full contract: `skill-audit-loop/SKILL_AUDIT_LOOP.md` plus `skill-audit-loop/SKILL_AUDIT_LOOP.md` § Part 2. Agent entry context lives in `skill-audit-loop/AGENT_CONTEXT.yaml`; workflow metrics, BDD, ADR, prompt, script, and display wiring lives in `skill-audit-loop/WORKFLOW_CONTRACT.md` and `audits/workflow-conformance/spec.yaml`. This section is the operational summary.
...
4. **Eval coverage** — `evals/comprehension.json` exists when a graded verdict is claimed; comprehension has at least 5 realistic scenarios; `eval_state` reflects reality.
...
7. **Description hygiene** — positive trigger phrases present, explicit negative examples present, marketplace description (if exported) ≤ the marketplace limit (currently 1024 chars).
8. **Understanding-fields gate (when `comprehension_state: present`)** — the five flat fields (`mental_model`, `purpose`, `concept_boundary`, `analogy`, `misconception`) populated, no repo-specific nouns in the Understanding-field content.
...
> **Read the rationale first:** [`docs/skill-audit-loop-philosophy.md`](docs/skill-audit-loop-philosophy.md) — WHY the loop curates (never strips to a delta), WHY two fully-tooled frontier models research and curate, WHY the eval generator is representative while frontier models judge, and HOW agents use the curate→eval cycle. Do not change how the loop builds or scores skills without reading it.
...
- **Default eval generator (the measured agent) = the `representative-generator` role** (currently Sonnet). The bidirectional eval keeps that measured subject fixed and asks both frontier judges (`FRONTIER_PAIR`) to grade the same with/without-skill evidence. This is the canonical decision in [`docs/skill-audit-loop-philosophy.md` § "WHY the generator (the measured agent) is representative"](docs/skill-audit-loop-philosophy.md) and is enforced by `DEFAULT_COMPREHENSION_GENERATOR_MODEL = REPRESENTATIVE_GENERATOR_MODEL`. It is **not** a `no-lesser-models` violation: that rule governs the GRADER (the judge) and AUTHOR (quality creator), while the generator here is the measured subject.
...
- **Grader prompts and audit docs must not name dated model versions.** Say "quality judge", "the `strongest-reasoning-grader` role", or the registry alias being requested — never "Opus 4.6", "GPT-5.5", `claude-opus-4-8`, or `gpt-5.4`. Mechanically enforced on the gate-scanned surfaces (`prompts/`, `lib/audit/graders/`) by `npm run models:check` (`scripts/check-no-restated-model-versions.js`), wired into both `verify` and `verify:system`; the same prose rule applies by convention to audit docs outside that scan (see the surface-scope note below).
- **A certifying behavior verdict requires honest model provenance.** If the grader or generator identity is unresolved, the run is not certifying and stays at a lower-confidence state unless the eval contract explicitly records stronger evidence. A single same-family frontier judgment is also not enough: the representative-generator path certifies only when both frontier judges agree.
- **Core (certifying) vs advisory tier.** The certifying CORE is the `representative-generator` measured subject plus both mandatory frontier judges (`FRONTIER_PAIR`). On top of it, an **opt-in advisory tier** (`ADVISORY_MODELS` = Gemini 3.1 Pro + the free OpenCode Zen models + Gemini 3 Flash) runs each advisory model as a **measured generator graded by a CORE frontier** (`runAdvisoryPanel`, `--advisory` / `AUDIT_ADVISORY_PANEL=1`). Advisory output is recorded in `advisory_panel` and **NEVER feeds reconciliation or sets a verdict** — it is breadth/novelty only. This honors `no-lesser-models-for-quality` (the GRADER is always a top-tier frontier; the advisory model is only the measured subject). Certification stays with the representative generator judged by both frontier judges. See [`docs/skill-audit-loop-philosophy.md` § "The advisory tier"](docs/skill-audit-loop-philosophy.md).
- **Output names models in full; dispatch names them by alias.** The dated-version ban above governs ONLY the gate-scanned surfaces (`prompts/`, `lib/audit/graders/`) — keep the alias there so the newest model auto-resolves. In ALL human-facing OUTPUT (audit receipts, scorecards, board minutes, reports) name each model by its full display name via `resolveDisplayName()` / the registry `DISPLAY_NAMES` map (`Opus 4.8`, `GPT-5.5`, `Gemini 3.1 Pro`, `MiniMax M3`, …) — never the bare alias. `DISPLAY_NAMES` lives in the registry (exempt from `models:check`); the CLI-resolving entries (opus/codex-current) are display PINS hand-bumped when a new model ships.
- **Eval receipts record the model epoch AND the resolved model.** Comprehension receipts carry `registry_version` (from `REGISTRY_VERSION` in the registry) **and** `resolved_grader_model` / `resolved_generator_model`. `registry_version` alone is NOT sufficient for cross-date comparability: it changes only on a registry *edit*, never on CLI resolution, so a bare claude alias (`opus`) that the CLI resolves to the newest installed Opus yields a byte-identical receipt across Opus 4.8 and 4.9 (SKI-41). The resolved fields therefore record the concrete pinned id when one is known (`gemini-3.1-pro-preview`, `gpt-5.4`, or a codex id captured from the output header per SH-6680) and the honest `latest-alias-unresolved` sentinel otherwise — a sentinel-tagged score is not strictly comparable across dates even within the same `registry_version`.
...
npm run verify:corpus    # corpus-completeness gate — manifest:fresh/validate, marketplace:verify, status:check, overlap, test:marketplace-corpus
npm run release:ready   # public-release wrapper — verify:system + marketplace superset shape + malicious-content scan + description-budget check
...
**Which to run depends on your work mode.** `npm run verify:system` is the gate for **SYSTEM work** (schema, scripts, protocol docs, fixtures): it excludes the corpus-completeness gates (`lint`, `manifest:fresh`, `manifest:validate`, `marketplace:verify`, `status:check`, `test:marketplace-corpus`, `routing-eval`, `export:verify-skill-md`, `overlap`) that go red purely because individual `skills/skills/**/SKILL.md` files have not yet been migrated to the current contract — those live in `npm run verify:corpus` (per SKI-319, so a parallel CONTENT session's corpus drift can never redden the SYSTEM gate; `overlap` joined that set per SKI-359 — a corpus-wide cross-skill duplicate-trigger/path check is CONTENT-dependent, so a future content defect must not redden the SYSTEM gate). A SYSTEM change is shippable when `verify:system` is green. `npm run verify` (the full gate) only goes green once the corpus has been migrated through the audit loop (CONTENT work) — do **not** relax the schema or skip a gate to force the full `verify` green while CONTENT migration is paused.

> **`verify:system` green does NOT mean publishable.** It deliberately omits the release-critical corpus gates, so a green `verify:system` can coexist with a red `manifest:validate` / `routing-eval` (e.g. skills missing the required `scope` field). Before claiming the library is publishable, run **`npm run release:ready`** for the combined public-release wrapper and **`npm run release:check`** for the corpus-heavy release gate; these surface corpus reds distinctly, so SYSTEM-green can never be mistaken for release-green. (Added 2026-06-06 — board-meeting finding SKI-222; `release:ready` wrapper added 2026-06-13.)
...
node scripts/__tests__/test-gate-conformance.js   # executable gate/protocol conformance — runs audits/gate-conformance/spec.yaml scenarios against the live gate scripts (structural + truth gates). In test:unit → verify + verify:system.
node scripts/__tests__/test-workflow-conformance.js # executable workflow conformance — runs audits/workflow-conformance/spec.yaml scenarios against agent entrypoints, prompts, command docs, and context wiring. In test:unit → verify + verify:system.
...
`lib/audit/` is the audit-loop runtime bundled with `@skill-graph/cli`. `bin/skill-graph.js` calls into it for the `audit`, `evaluate`, `status`, and `evolve` operations; `batch-eval.js` runs the comprehension grader across the corpus; `eval-staleness-checker.js` and `eval-linter.js` keep eval artifacts honest; `run-skill-improvement-loop.js` plus the `skill-improvement-helpers.js` / `skill-test-runner.js` / `research-feedback.js` / `log-paths.js` cluster are the inner loop for `improve`/`evolve`; `graders/*-prompt.md` are externalised grader prompts. The two-frontier Skill Audit Loop runner is `run-skill-audit-loop-lite.js` (pure orchestration: sequencing + anti-loss + keep-or-revert, deps injected) with its LIVE production deps in `skill-audit-loop-lite-deps.js` (shells the workspace claim system + claude/codex CLIs; a `--dry-run` exercises the whole path offline); its EVAL guardrail is `run-bidirectional-eval.js` + `eval-execution-profile.js`, with the per-direction live runner `evaluate-skill.runEvalDirection`. The **multi-agent Skill Audit Loop (multi-model panel)** (2026-06-05) generalizes the two-frontier path: `run-skill-audit-loop.js` (pure orchestrator — Phase 1 parallel propose by Opus+GPT MANDATORY + every free model ADVISORY; Phase 2 cross-review iterate-to-convergence; Phase 3 frontier-curated anti-loss synthesis; Phase 4 eval guardrail; Phase 5 apply-on-keep) with its self-contained live deps in `skill-audit-loop-live-deps.js` (advisory dispatched via the models' OWN CLIs — `gemini --yolo`, `opencode run` — NOT the workspace dispatch-solver). **Two-tier proposal delivery (2026-06-06):** frontier (Opus/GPT) WRITES its proposal/revision to a verified path; advisory TEXT-CAPTURES (research under the source-read-only kernel Seatbelt, emit the curated `SKILL.md` as reply text, captured from stdout via `extractSkillDocument`/`looksLikeSkillDoc` + written by us, model-written file as fallback). The fence makes the skill source READ-ONLY to advisory CLIs (a write EPERMs) and honors per-CLI quirks (Gemini's workspace-root lstat allow; codex's no-nested-`sandbox-exec` via `osFenceActive`). Full contract + the four SYSTEM fixes (`b680f11`/`ecd7df6`/`431a34f`/`7c6becf`): `skill-audit-loop/SKILL_AUDIT_LOOP.md § delivery contract` + § "Kernel-fence interactions". Its prompts are `prompts/skill-audit-loop-cross-review-pass.md` + `prompts/skill-audit-loop-revise-pass.md`; advisory arg builders `buildGeminiAuditArgs` / `buildOpencodeAuditArgs` live in `skill-audit-loop-lite-deps.js`. Full contract: `skill-audit-loop/SKILL_AUDIT_LOOP.md § Multi-agent panel — the Skill Audit Loop’s multi-model implementation`. `lib/audit-shared/` is cross-grader plumbing reused by the standalone CLI graders (`certification.js`, `synthesize-bidirectional.js`, `model-provider.js`).
...
1. **Generate the marketplace surface** in this repo: `node scripts/export-marketplace-skills.js`. Verify: `node scripts/export-marketplace-skills.js --check`. The exporter writes plain Agent Skills `SKILL.md` files to `marketplace/skills/`. **Encoding doctrine: the canonical source is authored in the flat protocol-native shape (every field a top-level key); the nested Agent-Skills `metadata:` encoding is a GENERATED build artifact** — the exporter (`export-skill.js::buildOutputFrontmatter`) JSON-encodes the structured fields under `metadata:` for the public release, so authors never hand-write the nested form. `RELEASE_TARGET_REPO` in that script is `jacob-balslev/skills` — do not change without coordinating both repos.
