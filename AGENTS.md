...
| **Grader prompts** (concept grader for gate 8, application grader for gate 9) | `lib/audit/graders/*-prompt.md` (intended canonical) | The workspace `scripts/skill/graders/` copies are **NOT shims — they are divergent forks** (verified 2026-05-28): `application-grader-prompt.md` differs only in cross-reference link style, but `concept-grader-prompt.md` carries an extra "Anti-Compression Mandate" + "Calibration Reference Set" block (~lines 13-63) the package copy LACKS — the workspace grader is AHEAD there. They are loaded by the forked `scripts/skill/evaluate-skill.js`, so their collapse is coupled to the evaluate-skill fork cluster (**SH-6603**); the package graders must absorb the calibration block before the workspace path is removed. `doc-verification-gates.js:544-545` gates the workspace paths' existence — update it in the same change. |
