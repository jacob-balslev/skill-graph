...
| **Audit-ledger consistency** | **PASS as of 2026-06-01** — `npm run audit-manifest:check` checked 59 verdicts across 161 skills and reported that every graded verdict has its gradeable artifact on disk. The gate runs inside full `npm run verify`; it is intentionally omitted from `npm run verify:system`, which stays the SYSTEM-only blocking gate. | `scripts/check-audit-manifest.js`; `package.json` scripts |
