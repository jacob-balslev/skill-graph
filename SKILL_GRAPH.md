...
|---|---|---|
| **Schema version enforced** | **v8 (v7→v8 phase ended 2026-05-26)**. The schema's global `required` array now mandates `subject` + `operation` (v8 axes). The v7 fields (`type`, `category`) remain DEFINED as optional properties for back-compat with skills that still carry them, but are no longer required. `schema_version: 7\|8` both still validate the field itself — but the SHAPE requirements are v8 across the board. Any skill that still lacks v8 axes is CONTENT-mode migration work for the audit loop. | `schemas/skill.schema.json` + [ADR-0017](docs/adr/0017-five-axis-classification-model.md) |
