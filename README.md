...
| [`AGENTS.md`](AGENTS.md) | Agent-facing repo guide (mission, doctrine, doc routing, validation commands, internal `lib/` layout, public-distribution contract). Imported into `CLAUDE.md`. |
...
| Path | Purpose |
|---|---|
| [`bin/`](bin/) | Public CLI entrypoint (`bin/skill-graph.js`). Subcommands: `init`, `add`, `lint`, `audit`, `route`, `drift`, `export`, `evolve`, `doctor` (umbrella). |
| [`lib/audit/`](lib/audit/) | Audit-loop runtime bundled in the published CLI: the four operations (`audit`, `evaluate`, `status`, `evolve`), application/comprehension graders, eval-staleness checker, eval linter, and thin shims that re-export `scripts/lib/` helpers without crossing the `lib/audit/` boundary. |
| [`lib/audit-shared/`](lib/audit-shared/) | Cross-grader plumbing shared between standalone graders and the in-process audit runner. |
| [`schemas/`](schemas/) | Skill and manifest JSON Schemas, including pinned v2 through v7; the unversioned mirror tracks v7. Also `skill.context.jsonld` and a `vocabulary/` namespace. |
| [`scripts/`](scripts/) | Authoring + maintenance tooling: lint, manifest generator, drift sentinel, router, routing-eval, overlap, exporter, migration codemods, mirror/doc sentinels, status builder. Helpers live in `scripts/lib/` and `scripts/lint/`; tests in `scripts/__tests__/`. Inventory of historical/on-demand scripts: see [`AGENTS.md § Maintenance, historical, and on-demand scripts`](AGENTS.md#maintenance-historical-and-on-demand-scripts). |

### Evidence and outputs

| Path | Purpose |
|---|---|
| [`marketplace/`](marketplace/) | Staging buffer for the public `jacob-balslev/skills` release. Generated by `scripts/export-marketplace-skills.js` from the canonical sibling library at `~/Development/skills/`. **Never hand-edit** — see [`AGENTS.md § Public Distribution`](AGENTS.md#public-distribution--canonical-url-contract). |
| [`audits/`](audits/) | Per-skill audit artifacts (`audits/<skill-name>/findings.md`, `verdict.md`, `scorecard.md`) emitted by `bin/skill-graph.js audit`. Evidence, not state — the state of truth is the Health Block on each skill's frontmatter. |
| [`evals/`](evals/) | Routing-eval baselines (currently `retrieval-baseline-v2.json`). Per-skill comprehension/application evals live alongside each skill in the canonical library or under `examples/evals/`. |
| [`data/`](data/) | Hand-edited classification data feeding upstream tooling. Today: `publication-classification.json` (per-skill OSS publication tier + market-state, consumed by the parent Development repo's audit worklist builder; seeded by `scripts/seed-publication-classification.js`). |
| [`examples/`](examples/) | Authoring template (`examples/skill-metadata-template.md`), specimen projects (`examples/projects/markdown-static-site/`), sample manifest, fixture skills, audit/eval examples. |
