...
The single `audit_verdict` field is retired (ADR 0011). The three verdicts are independent: `structural_verdict` (form), `truth_verdict` (drift), and `comprehension_verdict` (the behavior-gate quality signal). `audit` owns the first two; `evaluate` owns comprehension. Most skills sit at `UNVERIFIED` on the comprehension field until eval artifacts are authored and run.
