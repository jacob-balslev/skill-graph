...
> **Purpose:** Explain the 32-field frontmatter contract at a conceptual level without inventing structure that the schema does not actually enforce. If this file disagrees with the schema, the schema wins — fix this file.

## What kind of graph is this?

Skill Graph is a **property graph with a controlled-vocabulary set of typed predicates**, not an RDF knowledge graph. Nodes are skills. Edges are the keys inside `relations.*`. Node attributes are the 32 authored frontmatter fields. A JSON-LD `@context` (`schemas/skill.context.jsonld`) projects the property graph into SKOS / Dublin Core / PROV-O triples for consumers that want RDF semantics, but authoring stays in flat YAML.
...
## The 32 authored fields — grouped by role

The fields split into nine conceptual groups. This grouping is a teaching aid only — the schema groups by *requiredness* (always required / required-for-archetype / required-for-scope / optional). See `docs/metadata-contract.md § Requiredness Groups` for the authoritative grouping.

The exact field count:

- **32 authored fields** — the number authors write in YAML frontmatter.
...
- **14 optional enrichment fields** — including the full nested sub-fields inside `relations`, `grounding`, `portability`, `compatibility`, `lifecycle`, `runtime_telemetry`.

When you see "52 possible fields" anywhere, that is the count including nested sub-fields (e.g. `drift_check.last_verified`, `drift_check.truth_source_hashes`) counted individually. The 32 count refers to top-level authored keys only.
...
### Relations (one object, up to 4 predicate keys, each optional)
...
| `boundary` *(deprecated alias)* / `disjoint_with` | many; string or `{skill, reason}` | Anti-ownership exclusion | `skos:related` + `owl:disjointWith` |
...
`adjacent` and `boundary` remain valid — they are the v3.0 names, kept as aliases through v3.x. The v4 bump removes them in favour of the SKOS-aligned names. ADR 0001 records the decision.
...
5. Stated the field count as 32 without distinguishing authored-vs-possible — clarified: 32 authored, 52 when counting nested sub-fields individually.
...
- `docs/metadata-contract.md` — requiredness groups, archetype section map, schema strictness
