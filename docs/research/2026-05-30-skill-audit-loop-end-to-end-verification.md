...
| **SH-6639 — does `improve` AUTHOR v8 fields?** | ✅ **ANSWERED (real run)** — `improve --skill inversion --apply --grader claude` on a genuinely sub-v8 skill (schema_version 7, no scope, zero Understanding fields). Candidate captured from the worktree: authored `schema_version: 8` + `scope` + `subject` + `deployment_target` + **all five Understanding fields** + `comprehension_state` (original had none). So improve authors the FULL v8 content (this REFUTES my code-read prediction that Understanding fields wouldn't be authored — real-run > code-read). **BUT the gate DISCARDED it** (`kept:0, discarded:1`; inversion stays v7): the full-rewrite trimmed the skill 332→262 lines and removed 64% of domain anchors (the `code-preservation` "improve must enrich, not trim" guard fired) and regressed comprehension. **The capability exists; the acceptance criterion blocks it.** The full-rewrite improve conflates "upgrade schema" with "rewrite body"; the rewrite fails the enrichment/comprehension gate and takes the v8 upgrade with it. | **The real gap** posted to SH-6639. Fix direction: a SCOPED frontmatter-only v8-upgrade path (body untouched → gate passes on schema conformance, not comprehension delta), extending the existing `--field` hard-scope. |
