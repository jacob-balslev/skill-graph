...
`stability: stable` signals that a skill's content is settled and suitable for production dependence. The promotion criteria below are lint-enforced at **WARN level** (never ERROR) via `scripts/lint/check-stability-promotion.js`. The check was briefly removed in commit `2bd8e64` (2026-05-19) per the audit-doctrine cleanup that scoped `skill-lint.js` itself to external Anthropic/OpenAI mandates only, then restored as a standalone WARN-only check in commit `92978fb` (2026-05-20) once it was clear the gate carries useful author signal without violating the "lint = external mandates" doctrine. The criteria define what `stable` means; the lint surfaces a warning when a skill claims `stability: stable` without meeting them. Final promotion remains author judgment, audited by the Health Block fields and the application-eval pipeline (gate 9).
...
**Implementation:** enforced at WARN level by `scripts/lint/check-stability-promotion.js`. Tests: `scripts/__tests__/test-stability-promotion.js` (8 cases). Final promotion is author judgment, audited by reading the Health Block fields directly; the lint surfaces premature-stable claims rather than blocking them.
