...
2. **both directions cross-family certifying** — generator and grader families differ in each direction (self-preference guard, `certification.js`). This is the model-contract that makes the `PROVISIONAL → APPLICABLE` step honest: a same-family judge inflates its own family's outputs by ~+10–25pp (LLM self-preference bias, [arXiv 2410.21819](https://arxiv.org/abs/2410.21819)), so a single same-family top-tier run — even Opus-generates → Opus-grades — can support **at most `PROVISIONAL`**; certifying `APPLICABLE` requires an independent cross-family top-tier grader (Opus generates → GPT grades, or the swap). This does **not** conflict with `no-lesser-models-for-quality` (`~/Development/.claude/rules/`): both the generator and the grader remain top-tier; the additional requirement is only that their *families differ* on the certifying run.
