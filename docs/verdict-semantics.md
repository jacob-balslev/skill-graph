...
| `HARMFUL` | Negative delta — agent makes worse decisions with the skill loaded. Active skills with this verdict must be removed from the corpus with `git rm` so recovery lives in git history, or replaced by a newly evaluated non-HARMFUL version. The removal commit/report must explain the harmful evidence. SkillsBench arXiv 2602.12670 found 19% of evaluated skills exhibit this. | Grader. |
...
**HARMFUL is removal, not quarantine.** An active skill with `application_verdict: HARMFUL` is a corpus violation because the best available behavior evidence says the skill makes agents worse. The correct resolution is to `git rm` the active skill files so the old content is recoverable from git history, with the commit message and completion report explaining which eval/verdict evidence made the skill harmful. Do not move it to an active archive, deprecated folder, quarantine shelf, marketplace export, or hidden alternate path. A replacement is allowed only as a newly evaluated skill that earns a fresh non-HARMFUL application verdict. The `check-audit-manifest.js` gate fails while any active manifest entry still carries `HARMFUL`.
