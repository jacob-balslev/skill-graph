...
**Remaining (CONTENT-heavy):** item 4 (SH-6591 `scope` drain, 152 skills — best run as a dedicated `/evolve`/grind session now that the worklist queue is reliable post-SH-6650); item 5 (field-purpose-comment backfill — `--apply` writes SKILL.md, so CONTENT via the loop, not a SYSTEM commit); the deferred **certifying cross-family** application run to lift `debugging` (and other skills) from single-model to grader-confirmed.
