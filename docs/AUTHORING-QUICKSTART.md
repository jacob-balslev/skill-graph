...
|---|---|---|
| **Protocol-native** | You author skills in a protocol-aware repo (this one's `skill-graph/`). Authoring tooling can read structured types natively. | Every agent-facing `SKILL.md` protocol field is a top-level YAML key. `relations` and `grounding` are native YAML objects/arrays. |
| **Agent-Skills-compatible** | You author skills in the canonical library at `~/Development/skills/`, which double-duties as the public Agent-Skills release repo. | Only `name`, `description`, `license`, `compatibility`, `allowed-tools` at top level. **Agent-facing protocol fields are nested under a `metadata:` map, structured fields JSON-string-encoded.** |

Both `SKILL.md` encodings are normalized to the same in-memory frontmatter shape by `scripts/lib/parse-frontmatter.js::normalizeFrontmatter()` before lint, manifest, routing, and drift tooling reads them. The sibling `audit-state.json` sidecar is separate for both encodings. If you author Agent-Skills-compatible and later need a protocol-native copy, the round-trip is lossy for rich types (`docs/SKILL-MD-FORMAT-COMPATIBILITY.md`). Keep your authored copy as the canonical source.
...
cp examples/skill-audit-state-template.md skills/<your-skill-name>/audit-state.json
```

The template at `examples/skill-metadata-template.md` is a real schema-conformant skill whose subject is skill authoring. The sidecar template carries the audit/eval/provenance fields the Skill Audit Loop owns. Adapt by renaming identity, rewriting body sections, filling the sidecar values, and stripping every `# TEMPLATE NOTE:` YAML comment and `> **TEMPLATE NOTE:**` body blockquote from the skill file.

## Step 3, fill the required v8 fields

These are enforced by `skill-lint.js` against `schemas/SKILL_METADATA_PROTOCOL_schema.json` plus `schemas/skill-audit-state.schema.json`. A skill missing any required field in either file fails the structural gate.
...
| 5 | `eval_artifacts` | enum | `none` / `planned` / `present`. Default `planned` for a new skill. Flip to `present` when an eval file exists. |
| 6 | `eval_state` | enum | `unverified` / `passing` / `monitored`. Default `unverified` for a new skill. Flip to `passing` only after a real grader run. |
| 7 | `routing_eval` | enum | `absent` / `present`. Default `absent`. Setting `present` requires populated `examples` + `anti_examples` AND a passing routing eval run. |
...
`skills/example-protocol-native/audit-state.json`:
...
The `audit-state.json` sidecar is the same as in example A; the two physical encodings differ only in how the agent-facing `SKILL.md` fields are written.
...
```bash
git commit --only -F /tmp/msg -- skills/<your-skill-name>/SKILL.md skills/<your-skill-name>/audit-state.json
```
