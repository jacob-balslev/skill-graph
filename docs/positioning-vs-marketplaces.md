...
| **Project scoping** | Folder structure or naming hacks | `public: false` + `project[]` belonging references, no folder gymnastics |
...
| Share some skills across two projects but not all | **Skill Graph** (`public: true` for shared skills, `public: false` + `project[]` for anchored skills) |
