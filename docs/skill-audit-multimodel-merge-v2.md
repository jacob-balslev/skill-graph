...
>
> **Operationalized (HOW).** The two-channel auditor output (structured proposal + novelty memo), the rotated curator, and the anti-loss merge-ledger v2 fields (`surfaced_by` / `corroborated_by` / `evidence_strength` / `format_loss` / `reopen_trigger`) defined here are produced programmatically by the two-frontier ENRICH orchestrator `lib/audit/run-bidirectional-enrich.js` + its live deps `lib/audit/enrich-live-deps.js` (per-model enrich-pass prompt: `prompts/skill-audit-loop-enrich-pass.md`). The orchestrator's `validateAntiLoss` enforces the anti-loss rule on the ledger this doc specifies. The **WHY** is canonical in [`audit-loop-enrich-philosophy.md`](./audit-loop-enrich-philosophy.md); this doc owns the merge mechanics + attribution schema.
