...
>
> **Operationalized (HOW).** The two-channel auditor output (structured proposal + novelty memo), the rotated curator, and the anti-loss merge-ledger v2 fields (`surfaced_by` / `corroborated_by` / `evidence_strength` / `format_loss` / `reopen_trigger`) defined here are produced programmatically by the two-frontier Skill Audit Loop orchestrator `lib/audit/run-skill-audit-loop-lite.js` + its live deps `lib/audit/skill-audit-loop-lite-deps.js` (per-model improve-pass prompt: `prompts/skill-audit-loop-improve-pass.md`). The orchestrator's `validateAntiLoss` enforces the anti-loss rule on the ledger this doc specifies. The **WHY** is canonical in [`skill-audit-loop-philosophy.md`](./skill-audit-loop-philosophy.md); this doc owns the merge mechanics + attribution schema.
