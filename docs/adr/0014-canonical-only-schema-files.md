...
- `schemas/SKILL_METADATA_PROTOCOL_schema.json` (the "canonical" file edited directly)
...
1. `schemas/SKILL_METADATA_PROTOCOL_schema.json` and `schemas/manifest.schema.json` are the only schema files on disk. Editing them is editing the contract.
...
5. Per-version migration narrative (e.g., `docs/migrations/v6-to-v7.md`) is not on disk. The migration story for any historical version is `git log -- schemas/SKILL_METADATA_PROTOCOL_schema.json` plus the commit that introduced that version's `schema_version` constant.
...
- A future external consumer that needs to pin against "the v7 contract specifically" has to derive it from git history (`git show <commit>:schemas/SKILL_METADATA_PROTOCOL_schema.json`) rather than read a pinned file. We accept this because no such consumer currently exists; if one appears, the right answer is a `git tag schema-v7` plus a `$ref` URL that resolves against the tag, not a duplicate file in `main`.
- ADRs 0011 and 0013 reference `skill.v7.schema.json` by filename in their body text. Per ADR convention (broken-link-fix exception), the references have been updated in-place to `SKILL_METADATA_PROTOCOL_schema.json`. The substantive decisions in those ADRs are unchanged; only the file reference was repointed.
...
If a downstream consumer materializes that genuinely needs the pinned mirror, the backout is mechanical: re-run `cp schemas/SKILL_METADATA_PROTOCOL_schema.json schemas/skill.v7.schema.json` plus a `sed` to set the `$id`/`title` back to the v-prefixed form, and reinstate the C6 check. Two lines of code. We do not pre-pay for that contingency.
...
- `node scripts/skill-lint.js` → still validates against `schemas/SKILL_METADATA_PROTOCOL_schema.json`.
- `ls schemas/` → contains exactly: `manifest.schema.json`, `SKILL_METADATA_PROTOCOL_schema.json`, `skill.context.jsonld`, `vocabulary/`.
