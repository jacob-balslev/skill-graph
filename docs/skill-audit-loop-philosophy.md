...
2. **Permission parity = equal full tool access for the GENERATORS.** The parity invariant binds the two *generators* (the measured agents that research + answer the eval task): both get the *same* tools, the *same* repo scope, the *same* web allowance. Parity means *equal access*, not equal-zero. If the two directions' generators did not run under matched permissions, the run is **invalid** (`parity_ok: false`) and may never certify a skill — different permissions per provider would mean you measured "who got to peek," not the model. **WEB allowance specifically defaults to OFF (2026-06-11, owner directive):** the eval measures a skill's *deployment value* (search-elimination + curation), not web-findability — a web-enabled baseline can search its way to a passable answer and hide the skill's lift. Parity still binds it: both directions get the SAME (now no-web) allowance — *equal-no-web is valid parity*; what is forbidden is UNEQUAL per-provider access. `tools` stay `full`, so `research: 'repo'` remains a certifying config (`SKILL_EVAL_WEB=on` restores web). The skill **authoring/curation** step is separate and keeps full web research — "research IS the curation mechanism" (§ above). See `lib/audit/eval-execution-profile.js` header + `docs/application-eval-spec.md § Web access`. The **graders** are a separate role: each grades a *fixed, closed evidence packet* (the prompt + the candidate response) with **tools OFF** — a judge that browses mid-grade is non-deterministic. Grader parity is "both graders closed and identical," which is the default. (Clarified 2026-06-03 per the GPT-5.5 cross-review, finding F3.)
