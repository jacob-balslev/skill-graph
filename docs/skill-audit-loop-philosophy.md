# Skill Audit Loop — Curation Philosophy (Why + How Agents Use It)
...
1. **CURATE (the objective):** continuously research and curate the strongest available knowledge for each skill's topic, and fold it into the skill — so the skill keeps getting closer to "the best solution we and the best available models can assemble for this topic."
2. **EVAL (the guardrail):** confirm the curated skill actually helps a real, tool-enabled agent solve better, and didn't regress or turn harmful.
...
## WHY curate, never strip to a delta
...
- **The objective is knowledge and quality — curate.** Each iteration folds in the best current solution for the topic, toward the *fullest, strongest* curated knowledge. This is the project's [`quality-doctrine`](quality-doctrine.md) and [`code-preservation`](../../.claude/rules/code-preservation.md) direction: *improve = curate; organize, don't trim.*
- **The eval is a guardrail, not the optimizer.** It answers "does the curated skill genuinely help, and did it regress or turn harmful?" — a *floor and a safety check*, never a pressure to shrink.
- **Keep-or-revert reverts genuine regressions only.** Revert a curation commit if the eval shows the skill became *harmful* or *measurably worse*. **Never** revert (or prune) knowledge merely because a narrow eval delta failed to credit it. "Didn't move the score" is **not** a reason to remove curated knowledge — the eval is too narrow to see all the value, and absence of measured lift is not evidence of absence of value.
...
"Research the repo and online" is scoped to the **public skill-graph repo + the skills tree + the open web.** Eval and curation agents must **NEVER** pull Sales Hub / Sales Channels / Printify / Shopify / personal-API / bank / customer data from the private workspace into prompts, proposals, references, or receipts. The skills library is public; private operational data must not leak into it. See memory `skill-graph-private-content-boundary`.

## WHY the generator (the measured agent) is representative

When the eval measures "does this skill help," the agent that *answers the eval task* (the generator) is the `representative-generator` role. Today that role resolves to Sonnet. The frontier models still decide quality, but as judges: Opus and GPT independently judge the same representative-generator evidence and both must agree before a verdict certifies.

- The eval needs headroom. A frontier model used as the measured subject often already knows the broad concept, which can produce a no-lift ceiling even when the skill would help the agents that actually consume skills during routine work. The representative-generator role measures whether the skill improves a capable deployment-representative agent without turning the frontier judges into the measured subject.
- The quality bar still sits with frontier models. Sonnet is not creating the skill and is not judging whether the skill is good; it is the measured subject whose baseline and with-skill behavior are compared. `no-lesser-models-for-quality` remains intact because the judges and curators are the strongest available models.
- A no-lift result still matters. `NOT_DISCRIMINATED_CEILING` and `EQUIVALENT_ON_FRONTIER` remain scoped evidence about the measured generator and case set; they are not removal instructions and do not license stripping unscored knowledge.

## The advisory tier (breadth — NEVER certifies)

The CORE of the loop is the `representative-generator` measured subject plus the two mandatory frontier judges. On top of it, an **opt-in advisory tier** widens the search: **Gemini 3.1 Pro** + the enabled free OpenCode Zen models (**MiniMax M3, DeepSeek V4 Flash, MiMo V2.5**) + **Gemini 3 Flash** (`ADVISORY_MODELS` in `lib/audit-shared/model-provider.js`). Nemotron and Big Pickle descriptors remain available for explicit probes, but are excluded from automatic advisory dispatch until their bounded runs reliably return usable output. Each advisory model runs as a **measured generator graded by a CORE frontier** (Opus — top-tier), via `runAdvisoryPanel` (`lib/audit/run-bidirectional-eval.js`), enabled with `--advisory` / `AUDIT_ADVISORY_PANEL=1`.

The advisory tier asks a *different, non-certifying* question — does the skill *also* help a cheaper/other-family agent? — and is recorded separately (`advisory_panel`). **It never feeds the conservative reconciliation and never sets a verdict.** `no-lesser-models-for-quality` is honored because the GRADER is always a top-tier frontier; the advisory model is only the measured subject (measurement, not quality-judging). Certification stays with the representative generator judged by both frontier judges.

## HOW agents use the loop

Per skill, the cycle is **curate → eval-guardrail → keep-or-revert**:

```
CURATE (primary, reuse the union-curate merge protocol)
...
EVAL (guardrail — bidirectional, tools ON, lockstep parity)
  Run A: representative-generator answers the eval task -> Opus judges it.
  Run B: representative-generator answers the same task  -> GPT judges it.
  Synthesize conservatively: certify only if BOTH frontier judges agree it helps.

KEEP-OR-REVERT (guardrail only)
  Keep the curated skill. Revert ONLY on a genuine quality/knowledge regression
...
Both eval directions must run under **identical conditions** — the only variable is the frontier judge:
...
2. **Permission parity = equal full tool access for the measured generator runs.** The parity invariant binds the two generator runs: both get the *same* tools, the *same* repo scope, and the *same* web allowance. Parity means *equal access*, not equal-zero. If the two directions' generator runs did not use matched permissions, the run is **invalid** (`parity_ok: false`) and may never certify a skill — different permissions per direction would mean you measured "who got to peek," not the skill. **WEB allowance specifically defaults to OFF (2026-06-11, owner directive):** the eval measures a skill's *deployment value* (search-elimination + curation), not web-findability — a web-enabled baseline can search its way to a passable answer and hide the skill's lift. Parity still binds it: both directions get the SAME (now no-web) allowance — *equal-no-web is valid parity*; what is forbidden is UNEQUAL per-direction access. `tools` stay `full`, so `research: 'repo'` remains a certifying config (`SKILL_EVAL_WEB=on` restores web). The skill **authoring/curation** step is separate and keeps full web research — "research IS the curation mechanism" (§ above). See `lib/audit/eval-execution-profile.js` header + `docs/application-eval-spec.md § Web access`. The **graders** are a separate role: each grades a *fixed, closed evidence packet* (the prompt + the candidate response) with **tools OFF** — a judge that browses mid-grade is non-deterministic. Grader parity is "both graders closed and identical," which is the default. (Clarified 2026-06-03 per the GPT-5.5 cross-review, finding F3; revised 2026-06-13 for the representative-generator decision.)
3. **The repo-scope fence is four layers: prompt + claim-filter + in-process path guard + a kernel OS fence.** `cwd: <skill-graph>` and `repoScope: 'skill-graph + skills ONLY'` declare the *intended* public scope, but process cwd alone does not stop an agent from path-traversing into a private sibling product repo or reading a private absolute path. The boundary is enforced by (a) the prompt instruction, (b) the `skill-audit-claim` private-skill filter, (c) an in-process **path-scope guard** (`lib/audit/public-content-fence.js`, SH-6681): the live deps `assertPublicScope` every artifact path, run dir, and skillDir against the public roots and REFUSE a private path before any shell-out (defense-in-depth, catches a private path the *orchestrator* resolves), and (d) the **FULL OS fence** (`lib/audit/isolated-checkout.js`, SH-6681 remainder): every spawned model CLI (claude/codex) runs under a macOS Seatbelt (`sandbox-exec`) profile that is the *policy equivalent of an isolated checkout* — from the process's view the workspace contains ONLY the public roots (skill-graph repo + skills tree + audit-artifacts run dir); every other workspace path is kernel-DENIED for read+write (EPERM), even by absolute path. This is what stops the model *process itself* from reading private sibling repos regardless of what it types — `claude --permission-mode bypassPermissions` has no sandbox and `codex -s workspace-write` restricts only writes, so layer (d) is what makes the boundary OS-hard. It is stronger than a physical checkout: no stale corpus copy, no disk cost, and public cross-tree refs (e.g. `skill-graph/audits/merge-protocol.md`) stay readable while the private trees vanish. Default ON when the OS supports it (`SKILL_AUDIT_OS_FENCE=0` opts out); on non-macOS it degrades to layer (c) and logs the gap. The remaining work is a **container (Docker) fence for Linux/CI** where `sandbox-exec` is absent — the only piece of SH-6681 still open. Do not describe cwd alone as a hard fence. (Clarified 2026-06-03 per GPT-5.5 review finding F5; in-process path guard added 2026-06-03; OS Seatbelt fence added 2026-06-03.)
4. **Private-content boundary** (above) applies to every model, every role, every direction.

### Operator commands

- Curate + eval one skill: the two-frontier Skill Audit Loop runner (`lib/audit/run-skill-audit-loop-lite.js`) drives the per-model research/propose, the union-curate merge, and the eval guardrail. Its LIVE production deps (claim → research/propose per frontier model → curate → revert) live in `lib/audit/skill-audit-loop-lite-deps.js`. Run it: `node lib/audit/run-skill-audit-loop-lite.js --skill <slug> --skill-dir <dir> --cwd <skill-graph-root>` (add `--dry-run` to exercise the whole orchestrator path offline — no LLM dispatch, no SKILL.md mutation — which is the CI-verifiable wiring path; the live pilot, without `--dry-run`, dispatches the two frontier models and writes CONTENT, so run it as a deliberate audit-loop pilot, one skill, committed separately).
- The four classic operations remain (`audit` / `improve` / `evaluate` / `evolve`, see [`SKILL_AUDIT_LOOP.md § Part 3`](../skill-audit-loop/SKILL_AUDIT_LOOP.md)); the two-frontier path is the *curate-the-best-knowledge* path, with `evaluate` as its guardrail.
...
- ❌ Letting the measured generator become the quality judge.
- ❌ Letting one frontier judgment certify by itself.
- ❌ Running the two models with different tools/permissions and comparing them.
- ❌ Pulling private workspace data into eval/curation artifacts.
