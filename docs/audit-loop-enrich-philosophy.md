...
3. **The repo-scope fence is prompt + claim-filter + an in-process path guard, not yet a full OS sandbox.** `cwd: <skill-graph>` and `repoScope: 'skill-graph + skills ONLY'` declare the *intended* public scope, but process cwd does not stop an agent from path-traversing to `../sales-hub`. The boundary is enforced by (a) the prompt instruction, (b) the `skill-audit-claim` private-skill filter, and (c) an in-process **path-scope guard** (`lib/audit/public-content-fence.js`, SH-6681): the enrich live deps `assertPublicScope` every artifact path, run dir, and skillDir against the public roots and REFUSE a private path before any shell-out (defense-in-depth). What remains for the FULL OS fence is sandboxing the model *process itself* — an isolated checkout containing only the public trees, so an agent the CLI spawns cannot traverse at all. That isolated-checkout sandbox is the tracked remainder of SH-6681. Do not describe cwd alone as a hard fence. (Clarified 2026-06-03 per GPT-5.5 review finding F5; path guard added 2026-06-03.)
