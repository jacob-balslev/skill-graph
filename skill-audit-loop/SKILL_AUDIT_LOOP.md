...
| **3 · curate (synthesis)** | A frontier curator union-merges the two MANDATORY proposals under STRICT anti-loss + mandatory-coverage AND **dispositions every advisory cross-review finding** in the merge-ledger as incorporated / deferred-to-eval / rejected — each with a reason (silence ≠ permission to ignore). | one frontier curator (rotated) | A merged curated `SKILL.md` + a merge-ledger where every mandatory contribution AND every advisory finding is recorded (kept/dropped/dispositioned) **with a non-"unscored" reason**. | Anti-loss or coverage violation **throws**; an undispositioned advisory finding is a coverage gap. |
