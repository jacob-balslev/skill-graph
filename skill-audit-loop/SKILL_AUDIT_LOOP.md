...
- **Advisory (gemini / opencode free models) — TEXT-CAPTURE.** The model RESEARCHES (tools ON under the kernel Seatbelt fence) and EMITS the complete enriched SKILL.md as its final reply text; the orchestrator captures it from stdout (`extractEnrichedDoc` / `looksLikeSkillDoc` in `panel-enrich-live-deps.js`) and writes it. A model-written file is a fallback. This recovered the weak free models that otherwise stalled on an agentic write (emitting only a plan). The trade-off: a text-capture reviser re-emits a non-byte-identical document each round, so advisory stability rarely reaches 1.0 — the `maxRounds` budget is the designed terminator (NOT a bug; documented in `ecd7df6`).
