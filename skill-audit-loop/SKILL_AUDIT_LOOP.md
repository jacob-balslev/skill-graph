...
- **Curate, never strip to a delta.** The eval is a non-regression guardrail, not the optimizer. The guardrail grades the curated skill (a temp copy), not the canonical pre-curation version. Keep-or-revert reverts ONLY a genuine regression (HARMFUL / measurably worse than the prior graded verdict); a non-improving or UNVERIFIED result is NEVER grounds to remove curated knowledge. If the canonical active skill itself carries `application_verdict: HARMFUL`, remove it from the active corpus or replace it with a newly evaluated non-HARMFUL version. The anti-loss check (`validateAntiLoss`) refuses any merge that drops a contribution for an "unscored / didn't move the score" reason.
