...
function reconcile(dirClaude, dirCodex, { mode } = {}) {
  ...
  if (mode === 'comprehension') {
    ...
    if (claudeProc) return { ...base, verdict: vCodex, agreement: false, note: `Claude direction procedural (${vClaude}); deferring to Codex direction graded verdict` };
    if (codexProc) return { ...base, verdict: vClaude, agreement: false, note: `Codex direction procedural (${vCodex}); deferring to Claude direction graded verdict` };
  }

  const rClaude = rankOf(table, vClaude);
  const rCodex = rankOf(table, vCodex);
  // Conservative: the lower-ranked (more skeptical) verdict wins.
  const verdict = rClaude <= rCodex ? vClaude : vCodex;
  const agreement = vClaude === vCodex;
