...
// Conservative synthesis of the bidirectional eval guardrail.
//
// Directions are named by the frontier judge: "Claude" = the representative
// generator answers and Opus judges; "Codex" = the representative generator
// answers and GPT judges. This module reconciles the two judge directions into
// ONE verdict using the CONSERVATIVE rule (user decision 2026-06-02): the lower /
// more-skeptical verdict wins. PASS is therefore reachable ONLY when BOTH frontier
// judges independently reach it for the same measured generator. A disagreement
// caps the result below the certifying verdict and records `agreement: false`.
//
// Ranks follow the confidence ordering in docs/verdict-semantics.md
// (PASS > PROVISIONAL > UNVERIFIED), with the negative grader verdicts ranked
// below PROVISIONAL (a real negative signal is worse than "not yet confirmed").
// Procedural non-signals (SKIPPED_BASELINE_HIGH, NA) are handled explicitly: they
// are not graded outcomes, so they neither certify nor drag a graded direction
// down — see reconcile().
...
function rankTable(mode) {
  ...
  throw new Error(`synthesize-bidirectional: unknown mode '${mode}' (expected 'comprehension')`);
}
 ...
 * no-signal floor — so an invalid direction caps the whole reconciliation at "no
 * graded signal" rather than leaking the garbage label. The caller records WHICH input
 * was normalized in the receipt `note` for auditability.
 *
 * @param {object} table  The mode's rank table (COMPREHENSION_RANK).
 ...
 *
 * Doctrine (docs/verdict-semantics.md + skill-audit-loop-philosophy.md): the
 * certifying verdict (PASS) requires BOTH frontier judges to grade and agree — the
 * conservative rule exists precisely so one direction can never certify alone. When
 * one direction is PROCEDURAL (SKIPPED_BASELINE_HIGH / NA) it carries no graded
 * signal, so only ONE direction actually graded. That single graded direction can
 * support at most PROVISIONAL (a real, single-perspective result) — it may NOT
 * certify PASS, because the second corroborating direction is missing. If the lone
 * graded verdict already ranks at or below PROVISIONAL (a genuine negative like
 * SHALLOW/REDUNDANT, or UNVERIFIED), keep it — capping never inflates a weak verdict,
 * only prevents a single direction from certifying.
 *
...
function capAtProvisional(table, verdict) {
  ...
  // No PROVISIONAL in this table would be a programming error; COMPREHENSION_RANK
  // defines it, so this is defensive only.
 ...
 *
 * @param {object} dirClaude  { verdict, model, ...receipt } for the Claude judge direction (representative generator → Opus grade).
 * @param {object} dirCodex   { verdict, model, ...receipt } for the Codex judge direction (representative generator → GPT grade).
 * @param {object} opts
...
function reconcile(dirClaude, dirCodex, { mode } = {}) {
  ...
  // graded direction caps at PROVISIONAL — PASS requires BOTH (the conservative rule
  // forbids single-direction certification). capAtProvisional keeps a genuine negative
  // (SHALLOW/REDUNDANT/UNVERIFIED) as-is and only blocks certification.
