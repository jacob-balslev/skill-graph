...
function insertScalarFieldIntoMetadataBlock(yaml, fieldLine) {
...
function updateFrontmatterField(content, key, value) {
  ...
  // block (e.g. `  comprehension_verdict: UNVERIFIED`) are matched and replaced
  ...
  if (existingMatch) {
    ...
    const withMetadataField = insertScalarFieldIntoMetadataBlock(yaml, fieldLine);
    if (withMetadataField !== null) {
      yaml = withMetadataField;
    } else {
