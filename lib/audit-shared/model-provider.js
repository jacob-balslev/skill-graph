...
// frontier judges EXPLICITLY by their real product names — `opus` (Opus 4.8,
// Anthropic) and `gpt-5.5` (GPT-5.5, OpenAI) — its measured representative
// generator by the explicit `representative-generator` role, and the free advisory
// tier by its real model aliases (`gemini`, `deepseek-flash`, …). The registry keeps
// each model's `provider` so output can print
...
const REPRESENTATIVE_GENERATOR_MODEL = 'representative-generator';
const REPRESENTATIVE_GENERATOR_RECEIPT_ID = 'representative-generator:sonnet';

const MODEL_REGISTRY = {
  // ── The two frontier judges of the audit loop, named explicitly. ──
  // They judge the representative generator's outputs independently; conservative
  // reconciliation requires both frontier judges to agree before PASS/APPLICABLE stands.
  opus: {
    alias: 'opus',
    backend: 'claude',
    modelId: 'opus',
  ...
  [REPRESENTATIVE_GENERATOR_MODEL]: {
    alias: REPRESENTATIVE_GENERATOR_MODEL,
    backend: 'claude',
    modelId: 'sonnet',
    provider: 'anthropic',
    description: 'Representative generator role — Sonnet via Claude; measured subject for eval lift, never a quality judge',
  },
  // ── Other Claude family aliases — bare alias = newest installed model. ──
  // (sonnet is reached through representative-generator in the eval path; haiku is
  // never used by the audit loop but stays defined because run-layout.js tiering +
  // the workspace router reference it.)
  sonnet: {
    alias: 'sonnet',
    backend: 'claude',
    modelId: 'sonnet',
...
const DISPLAY_NAMES = {
  opus: 'Opus 4.8',
  [REPRESENTATIVE_GENERATOR_MODEL]: 'Representative Generator (Sonnet 4.6)',
  sonnet: 'Sonnet 4.6',
  haiku: 'Haiku 4.5',
  'gpt-5.5': 'GPT-5.5',
  codex: 'GPT-5.3 Codex',
...
module.exports = {
  ...
  FRONTIER_PAIR,
  REPRESENTATIVE_GENERATOR_MODEL,
  REPRESENTATIVE_GENERATOR_RECEIPT_ID,
  otherFrontier,
  PANEL_BANNED_MODELS,
