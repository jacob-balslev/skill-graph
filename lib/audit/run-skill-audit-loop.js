 ...
 * @param {'comprehension'} [opts.evalMode]  default 'comprehension'.
...
function runSkillAuditLoop(opts = {}) {
  ...
  // and force eval_certified:false) — even if the eval guardrail reaches PASS. Knowledge
  ...
  if (!convergenceState.converged) {
    ...
    // stamp PASS. A later full run that converges supersedes it.
  ...
  return {
    ...
    // eval_certified: true ONLY when the guardrail produced a certifying-clean PASS.
    eval_status: evalStatus,
    eval_skip_reason: evalSkipReason,
    eval_certified: Boolean(
      evalReceipt && evalReceipt.certifying_clean
      && evalReceipt.synthesized_verdict === 'PASS'
...
//   [--eval-mode comprehension] [--prior-verdict <V>] [--max-rounds N]
...
function recordFullLoop({ skill, skillDir, cwd, result, evalMode, log = () => {}, deps = {} }) {
  ...
  const earnedProtocolLabel = deps.earnedProtocolLabel || sidecar.earnedProtocolLabel;
  ...
  const recorded = {};
  const findings = [];
  // B9: the Integrity-Gate subprocess timeout is env-tunable — a large skill with many truth
  // sources can legitimately exceed the default, and a timeout is reported as a DISTINCT finding
  // (not a generic exit) so it is not mistaken for a clean stamp. Default: 3 min integrity.
  ...
  // Per-model participation ledger. This is deliberately separate from the Audit Status verdicts:
  ...
  const primaryField = 'comprehension_verdict';
  const primaryArtifact = 'comprehension.json';
  // B2/B4: a non-certifying run (non-converged / verify-not-run / degraded) must never STAMP a
  // certifying verdict, even if the guardrail eval reached PASS — cap it at PROVISIONAL.
  ...
  if (result.eval) {
    try {
      ...
      if (v && v !== 'UNVERIFIED') {
        ...
        // (possibly PASS) verdict attached to changed content.
  ...
  // 4. skill_graph_protocol — the EARNED content label (SKI-355). Stamp it ONLY
  //    when this kept content migration produced a real, content-earning behavior
  //    verdict (PASS/PROVISIONAL); UNVERIFIED never earns it. Computed from the
  //    skill's integrity-validated schema_version → `Skill Metadata Protocol vN`.
  //    Per `AGENTS.md § Version Labels Are Earned, Not Bumped`, this is the one
  //    deterministic write path — never a find-replace. Uses the injected sidecar
  //    I/O so the composition stays unit-testable.
  try {
    const sc = readSidecar(skillMd) || {};
    const label = earnedProtocolLabel({ comprehensionVerdict: recorded[primaryField], schemaVersion: sc.schema_version });
    if (label && sc.skill_graph_protocol !== label) {
      writeSidecarFields(skillMd, { skill_graph_protocol: label });
      recorded.skill_graph_protocol = label;
    }
  } catch (e) { findings.push(`skill_graph_protocol stamp failed: ${e.message}`); }

  return { recorded, findings };
}

// SKI-349: terminal-marker safety net. The run is judged by an unambiguous
// `SKILL-AUDIT-LOOP: COMPLETE|FAILED|NON-CONVERGENT` line on stderr (a piped
// `| tee` masks the exit code; a detached launch can print result JSON even when
// killed). The synchronous try/catch in main() covers a THROWN run, but a process
// that DIES another way — killed by the batch watchdog (SIGTERM), an OOM, an
// uncaught async error, or a deep early `process.exit()` — would leave NO terminal
// marker and a `complete:false` heartbeat stuck at the last phase (the exact
// failure observed on project-knowledge-extraction + testing-strategy at
// cross-review r3/3). This installs process-level handlers that emit a terminal
// FAILED marker AND flush the heartbeat to a terminal (complete) state via
// reporter.teardown(), so EVERY exit path a process can still run code on is
// covered. SIGKILL is uncatchable — nothing can fix that — but the watchdog sends
// SIGTERM first, which is caught here. The success/throw paths call markEmitted()
// so the net no-ops for a run that already wrote its own marker (idempotent;
// preserves the exact COMPLETE/FAILED marker strings the watchers parse).
//
// Injectable (proc/stderr) so the regression test exercises it without touching
// the real process or spawning a model run.
function createExitSafetyNet({ skill, reporter, proc = process, stderr = process.stderr } = {}) {
  let emitted = false;
  function emitFailed(reason, exitCode) {
    if (emitted) return false;
    emitted = true;
    // Flush the heartbeat to a terminal (complete) state so a watcher stops
    // showing `complete:false` at a stale phase. Best-effort — never throw here.
    try { if (reporter && typeof reporter.teardown === 'function') reporter.teardown(); } catch (_) { /* best-effort */ }
    try { stderr.write(`SKILL-AUDIT-LOOP: FAILED skill=${skill} exit=${exitCode} reason=${reason}\n`); } catch (_) { /* best-effort */ }
    return true;
  }
  proc.on('exit', (code) => {
    // Last-resort net: any exit that did NOT go through markEmitted() (a deep
    // process.exit, a fallthrough) gets an honest FAILED marker. Synchronous only
    // (teardown/heartbeat use writeFileSync), which is all an 'exit' handler allows.
    emitFailed('process exited without a terminal marker (killed/crashed/early-exit)', code || 1);
  });
  for (const [sig, code] of [['SIGTERM', 143], ['SIGINT', 130]]) {
    proc.on(sig, () => { emitFailed(`signal ${sig} (killed by watchdog/operator)`, code); try { proc.exit(code); } catch (_) { /* test fake */ } });
  }
  proc.on('uncaughtException', (e) => { emitFailed(`uncaughtException: ${(e && e.message) || e}`, 1); try { proc.exit(1); } catch (_) { /* test fake */ } });
  proc.on('unhandledRejection', (e) => { emitFailed(`unhandledRejection: ${(e && e.message) || e}`, 1); try { proc.exit(1); } catch (_) { /* test fake */ } });
  return {
    /** Mark that the run already wrote its own COMPLETE/FAILED marker — the net then no-ops. */
...
function main(argv) {
  ...
  // SKI-349: install the terminal-marker safety net BEFORE the run, so a death
  // that bypasses the try/catch below (SIGTERM from the batch watchdog, OOM,
  // uncaught async, deep process.exit) still emits a FAILED marker + terminal
  // heartbeat. The success/catch paths call exitNet.markEmitted() so the net
  // no-ops once a real marker is written.
  const exitNet = createExitSafetyNet({ skill, reporter });
  ...
  try {
    ...
    exitNet.markEmitted(); // a real terminal marker was written — the safety net must not double-emit on exit
  ...
  if (!dryRun && !args['no-record-loop']) {
    try {
      const loopRecord = recordFullLoop({
        ...
        result, evalMode: args['eval-mode'] || 'comprehension', log,
      });
  ...
  exitNet.markEmitted(); // a real COMPLETE marker was written — the safety net must not emit FAILED on exit
  process.exit(exitCode);
}
