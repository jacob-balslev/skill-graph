...
function runSkillAuditLoop(opts = {}) {
  ...
  if (!convergenceState.converged) {
    throw new NonConvergenceError(
      `skill-audit-loop: mandatory proposals did not converge for ${skill} after ${convergenceState.rounds} round(s) (${convergenceState.reason}); aborting before curate/apply.`,
      { skill, rounds: convergenceState.rounds, reason: convergenceState.reason },
    );
  }
  ...
  if (typeof deps.verifyMerge === 'function') {
    ...
    for (let vRound = 1; vRound <= 2 && !approvedAll; vRound += 1) {
      ...
      if (!approvedAll && vRound < 2) {
        ...
        merge = deps.curate({
          ...
          crossReview: (convergenceState.lastRoundFeedback || []).concat(
            roundGaps.map((g) => ({ reviewerModel: g.verifier, reviewerTier: 'mandatory', targetModel: 'curator', round: `verify-r${g.round}`, items: [{ verdict: 'wrong', item: g.item, evidence: g.evidence, required_action: g.required_action }] })),
          ),
        });
    ...
    log('phase 3.1 · ✓ both mandatory frontier models approved the merge');
  } else {
...
function main(argv) {
  ...
  if (!skill || !skillDir) {
    console.error('Usage: node lib/audit/run-skill-audit-loop.js --skill <slug> --skill-dir <dir> [--cwd <skill-graph-root>] [--eval-mode application|comprehension] [--prior-verdict <V>] [--max-rounds N] [--advisory-timeout-ms N] [--no-advisory] [--dry-run] [--no-eval] [--no-record-loop] [--preflight-only [--auth-probe [--auth-probe-timeout-ms N]]] [--model-cli-home auto|scratch|real] [--public-workspace auto|always|never] [--status-file <path>] [--no-tui] [--quiet]');
    process.exit(1);
  }
  ...
  const usePublicWorkspace = publicWorkspaceMode === 'always' || (publicWorkspaceMode === 'auto' && !osFenceSupported);
  ...
  const deps = createSkillAuditLoopDeps({
    ...
    dryRun,
    modelEnv: modelHome.env,
    modelCwd: publicWorkspace.active ? publicWorkspace.skillGraphRoot : null,
