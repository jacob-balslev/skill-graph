...
function recordFullLoop({ skill, skillDir, cwd, result, evalMode, log = () => {}, deps = {} }) {
  ...
  if (result.eval) {
    try {
      ...
      if (v && v !== 'UNVERIFIED') {
        let stamp = v;
        if (certBlocked && (v === 'APPLICABLE' || v === 'PASS')) {
          stamp = 'PROVISIONAL';
          findings.push(`${primaryField}: run is NON-CERTIFYING (${result.certifying_blocked.join('; ')}) — guardrail reached ${v} but the verdict is capped at PROVISIONAL; a full certifying run is required before ${v}.`);
        }
