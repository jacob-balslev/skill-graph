...
function main(argv) {
  ...
  if (!skill || !skillDir) {
    console.error('Usage: node lib/audit/run-skill-audit-loop.js --skill <slug> --skill-dir <dir> [--cwd <skill-graph-root>] [--eval-mode application|comprehension] [--prior-verdict <V>] [--max-rounds N] [--no-advisory] [--dry-run] [--no-eval] [--no-record-loop] [--preflight-only] [--model-cli-home auto|scratch|real] [--public-workspace auto|always|never] [--status-file <path>] [--no-tui] [--quiet]');
    process.exit(1);
  }
