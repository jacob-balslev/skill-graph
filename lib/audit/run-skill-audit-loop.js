...
// READ FIRST: docs/skill-audit-loop-philosophy.md (enrich-not-strip; eval is a guardrail).
//
...
function runSkillAuditLoop(opts = {}) {
  ...
  if (convergenceState.reason === 'quorum-collapsed') {
    throw new Error(`skill-audit-loop: mandatory quorum lost mid-convergence for ${skill} — a frontier model failed; aborting (the 2-frontier floor is required for a certifiable enrich).`);
  }
...
function main(argv) {
  ...
  if (!skill || !skillDir) {
    console.error('Usage: node lib/audit/run-skill-audit-loop.js --skill <slug> --skill-dir <dir> [--cwd <skill-graph-root>] [--eval-mode application|comprehension] [--prior-verdict <V>] [--max-rounds N] [--no-advisory] [--dry-run] [--no-eval] [--status-file <path>] [--no-tui] [--quiet]');
    process.exit(1);
  }
