...
const { parseRateLimit, RateLimitError } = require('./panel-budget');
...
function runSkillAuditLoop(opts = {}) {
  ...
  for (const model of mandatoryModels) {
    ...
    try {
      ...
      // propagates as-is.) The dispatch text often lives in err.dispatchOutput/stderr, not just
      // err.message — scan all of them.
  ...
  for (const model of advisoryRequested) {
    ...
    if (slot && slot.ok === false) { advisoryFailures.push({ model, phase: 'claim', error: slot.error, failure_reason: 'error' }); log(`phase 1b · ✗ ${model} claim failed: ${slot.error}`); onProgress({ kind: 'agent', model, tier: 'advisory', phase: 'propose', state: 'skip', failure_reason: 'error' }); continue; }
    let p;
    ...
    if (!p || p.ok === false || !p.proposalPath) {
      ...
      onProgress({ kind: 'agent', model, tier: 'advisory', phase: 'propose', state: 'skip', failure_reason });
