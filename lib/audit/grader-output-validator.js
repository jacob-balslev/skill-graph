 ...
 *   - application-grader-output (0/1/2 per axis across 4 axes)
...
function validateApplicationGrader(data) {
  ...
  const weightedScore = weightSum ? weightedNumer / (weightSum * 2) : 0;
  ...
  // passed: primary axis ≥ 1 AND weighted_score ≥ 0.6
  ...
  const computedPassed = typeof primaryScore === 'number' && primaryScore >= 1 && weightedScore >= 0.6;
  if (typeof data.passed === 'boolean' && data.passed !== computedPassed) {
    issues.push({
      severity: 'error',
      code: 'passed_mismatch',
