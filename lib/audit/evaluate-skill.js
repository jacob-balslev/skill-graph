...
function runGraderPrompt(prompt, {
  ...
  // Codex grader branch (two-frontier bidirectional Direction A: opus generates →
  // GPT-5.5 via Codex grades). Without this branch a codex grader silently fell
...
function runEvalDirection({
  ...
  // null model (live pilot 2026-06-03 caught exactly this — Direction B failed with
  ...
  // codex-current resolves to null (USE_CLI_CURRENT) — record the honest sentinel so
  // the receipt never claims a concrete model it cannot prove (caps at PROVISIONAL).
  const resolvedGrader = resolvedGraderModel || 'latest-alias-unresolved';
  const resolvedGenerator = resolvedGeneratorModel || 'latest-alias-unresolved';
