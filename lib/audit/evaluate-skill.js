...
// canonical decision in docs/audit-loop-enrich-philosophy.md § "WHY the generator
...
function runComprehensionEval(evalFile, options = {}) {
  ...
  // IS the curation mechanism (audit-loop-enrich-philosophy.md). Without a profile,
...
function runEvalDirection({
  ...
  const { cli: graderCli, model: resolvedGraderModel } = resolveModelExecutor(graderModel);
  ...
  const { cli: generatorCli, model: resolvedGeneratorModel } = resolveModelExecutor(generatorModel);
  ...
  // Resolve the concrete models AFTER the eval ran. For a Claude alias (opus),
  // resolveModelExecutor already returns a concrete-enough id. For a codex role the
  // executor returns null; prefer the model captured from codex's output header
  // (SH-6680), falling back to the honest sentinel only if no header was seen.
  const SENTINEL = 'latest-alias-unresolved';
  const capturedCodexModel = getResolvedCodexModel();
  const resolvedGenerator = resolvedGeneratorModel
    || (generatorCli === 'codex' ? capturedCodexModel : null)
    || SENTINEL;
  const resolvedGrader = resolvedGraderModel
    || (graderCli === 'codex' ? capturedCodexModel : null)
    || SENTINEL;
