...
// Comprehension evaluation runner for the Skill Audit Loop.
...
const { writeSidecarFields, stampEarnedProtocolLabel } = require('./audit-state-sidecar');
// Bidirectional design: PASS is reachable only from a certifying
...
// file stays portable for `npm install -g @skill-graph/cli`), and the eval
...
//
// See docs/adr/0011-split-audit-verdict.md (and its
...
const BOOLEAN_FLAGS = new Set([
  ...
  // comprehension verdict PASS is downgraded
  // to PROVISIONAL so the Health Block reflects the actual confidence tier.
  'single-model',
  // SH-6624/SKI-306: explicit operator attestation that the run is a
  // certifying configuration. Without it (the default), the run is
  // provisional-capped and PASS is unreachable — the in-code form of
  // "never UNVERIFIED→PASS without evidence". Pair with
...
// which (with parity + cross-family) unblocks PASS. gpt-5.5 is
...
function runGraderPrompt(prompt, {
  ...
  if (grader === 'claude') {
    ...
    // options layer (comprehensionGraderModel(), default the strongest tier).
    // No model => throw, never substitute down.
    if (!model) {
      throw new Error(
...
function runComprehensionEval(evalFile, options = {}) {
  ...
  // SKI-205: true no-call dry-run preview.
  ...
  if (options._baselineSkipTriggered) {
    ...
    // A measured comprehension regression maps to SHALLOW — the skill loaded but
    // did not deepen (and on these cases worsened) the agent's understanding.
  ...
  const summary = {
    ...
    avg_delta: avgDelta,
    classification,
    // The audit loop reads this to stamp comprehension_verdict on the skill's
    // Audit Status. comprehension_verdict is the Behavior-Gate quality signal.
...
function runEval(evalFile, options = {}) {
  ...
  // silent weak-model ('sonnet') fallback in runGraderPrompt (SH-6641); only affects the
...
function stampComprehensionVerdict(evalFilePath, comprehensionResult, dryRun) {
  ...
  // SKI-355: a real content-earning verdict (PASS/PROVISIONAL) earns the
  // skill_graph_protocol content label. Stamp it here too so the standalone
  // `evaluate --mode comprehension` write-back keeps the earned label in sync
  // with the verdict it just recorded (UNVERIFIED/SHALLOW/REDUNDANT do not earn it).
  const protocolLabel = stampEarnedProtocolLabel(skillMd, { comprehensionVerdict: verdict });
  if (protocolLabel) console.log(`[write-back] Stamped earned content label: ${protocolLabel}`);
}
...
function main() {
  ...
  // Comprehension: dual-run baseline vs with_skill grading.
    result = runComprehensionEval(evalFile, {
      ...
      // (a true no-call preview).
      dryRun: args['dry-run'] === true || args['dry-run'] === 'true',
      // Cross-family certifying attestation. PASS caps
  ...
  // single opt-out (preview only), consistent with the comprehension
  // stamp path which already honors it.
  ...
  // the Health Block instead of crawling eval-history.jsonl.
  //
