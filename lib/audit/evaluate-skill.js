...
function runPromptWithCli(prompt, {
  ...
  if (cli === 'gemini') {
    ...
    // — for a clean no-web baseline use claude (or codex). (The certifying pair is
    // Opus⇄GPT, i.e. claude⇄codex; gemini-as-generator is an edge case.)
    if (allowTools && !web) {
 ...
 *   - Never stamp without a real result (dryRun or errorCount > 0 abort).
...
function stampApplicationVerdict(evalFilePath, applicationResult, dryRun, options = {}) {
  ...
  // If any case errored, the verdict is not trustworthy. The runner may still
  // report a completed-subset verdict for debugging, but write-back requires the
  // full case set to have completed.
  const totalCases = applicationResult.total || 0;
  const errorCount = applicationResult.errors || 0;
  if (totalCases > 0 && errorCount > 0) {
    console.warn('\n[write-back] One or more cases errored — not stamping application_verdict (run is incomplete).');
    return;
  }
  ...
  if (verdict === 'APPLICABLE' && applicationResult.calibrated !== true) {
    console.log('\n[write-back] downgrading APPLICABLE → PROVISIONAL (application grader is uncalibrated; no durable calibration receipt proves ≥85% human agreement).');
    verdict = 'PROVISIONAL';
  }
  if (verdict === 'APPLICABLE' && Number(applicationResult.red_herring_cases_total || 0) < 1) {
    console.log('\n[write-back] downgrading APPLICABLE → PROVISIONAL (application eval has zero red_herring:true cases; boundary behavior is untested).');
    verdict = 'PROVISIONAL';
  }
...
function main() {
  ...
  if (statusFile) {
    const hbSkill = (typeof applicationFlagRaw === 'string' && applicationFlagRaw !== 'true')
      ? path.basename(applicationFlagRaw)
      : (evalFile ? path.basename(path.dirname(path.dirname(path.resolve(evalFile)))) : null);
  ...
  if (mode === 'application') {
    if (!evalFile) {
      console.error('Usage: node skill-graph/lib/audit/evaluate-skill.js --mode application [--application <skill-dir>] [--workspace DIR] [--eval-id 1,2] [--dry-run] [--trials N] [--certifying --generator-family F --grader-family G] [--calibration-receipt FILE] [--single-model] <application.json>');
      ...
      console.error('  --calibration-receipt FILE  durable human-agreement receipt proving the application grader is calibrated; REQUIRED for APPLICABLE certification');
    ...
    // Skipped for dry-run and incomplete runs — see stampApplicationVerdict().
    ...
    process.exit(Number(applicationResult.errors || 0) > 0 ? 1 : 0);
  }
...
function runEvalDirection({
  ...
  baselineWorkspace,
  calibrationReceipt,
  slice, // A10: held-out slicing for application mode (all | iteration | certification)
}) {
  ...
  if (mode === 'comprehension') {
    ...
    result = runApplicationEval(file, {
      ...
      executionProfile,
      calibrationReceipt,
      slice, // A10: held-out slicing (default 'all' when undefined)
