...
// — so a verdict could outlive its evidence (Break #4). They now default to
// audits/<skill>/eval-history/<ts>.json in the skill-graph repo, the same
// committed home as findings/verdict/scorecard, and the application receipt
// records the relative artifact path so the verdict always points at durable
// evidence.
const SKILL_GRAPH_REPO_ROOT = path.resolve(__dirname, '..', '..');
function durableEvalResultPath(skillSlug) {
  const slug = (skillSlug || 'unknown').toString().replace(/[^a-zA-Z0-9_-]+/g, '-');
  const ts = new Date().toISOString().replace(/[:.]/g, '-');
  return path.join(SKILL_GRAPH_REPO_ROOT, 'audits', slug, 'eval-history', `${ts}.json`);
}
...
function stampApplicationVerdict(evalFilePath, applicationResult, dryRun, options = {}) {
  ...
  if (options.artifactPath) {
    receipt.artifact = path.relative(SKILL_GRAPH_REPO_ROOT, options.artifactPath);
  }
