...
function stampApplicationVerdict(evalFilePath, applicationResult, dryRun, options = {}) {
  ...
  // ONLY from a certifying run (runApplicationEval stamps certification_tier from
  // the operator's declared attestation).
  ...
  // a deliberate, recorded certifying act. SH-6624.
  ...
  if (verdict === 'APPLICABLE' && (!isCertifyingRun || options.singleModel)) {
    const why = options.singleModel
      ? '--single-model set'
      : `not a certifying run (certification_tier=${applicationResult.certification_tier || 'provisional'})`;
    console.log(`\n[write-back] downgrading APPLICABLE → PROVISIONAL (${why}; single-perspective assessment, not the bidirectional frontier-judge guardrail).`);
    verdict = 'PROVISIONAL';
  }
...
function main() {
  ...
  if (mode === 'application') {
    if (!evalFile) {
      ...
      console.error('  --certifying         attest a certifying run; REQUIRED for APPLICABLE — else the run caps at PROVISIONAL');
      ...
      console.error('  --generator-family F declared family/model that produced responses (provenance + certification check; the runner never selects a model)');
    ...
    const applicationSkillDir = path.dirname(path.dirname(path.resolve(evalFile)));
    const applicationSkillName = skillNameFromDir(applicationSkillDir);
    try {
      applicationResult = runApplicationEval(evalFile, {
        ...
        deps: {
          runGraderPrompt: (prompt, options) => runGraderPrompt(prompt, {
            ...options,
            skillDir: applicationSkillDir,
            skillName: applicationSkillName,
          }),
          getEvalResponse: (prompt, options) => getEvalResponse(prompt, {
            ...options,
            skillDir: applicationSkillDir,
            skillName: applicationSkillName,
          }),
      ...
      applicationResult.agent_telemetry_source = TELEMETRY_FILENAME;
    } catch (appError) {
...
function runEvalDirection({
  ...
  if (mode === 'comprehension') {
    ...
    result = runApplicationEval(file, {
      ...
      deps: {
        runGraderPrompt: (prompt, options) => runGraderPrompt(prompt, {
          ...options,
          skillDir,
          skillName: skillNameFromDir(skillDir),
        }),
        getEvalResponse: (prompt, options) => getEvalResponse(prompt, {
          ...options,
          skillDir,
    ...
    result.agent_telemetry_source = TELEMETRY_FILENAME;
