...
// Two-frontier bidirectional design (2026-06-02): the comprehension gate reuses
// the SAME cross-family certification logic as the application gate, so PASS is
// reachable only from a cross-family dual-run (never a same-family / undeclared
// run). The tools-ON execution profile carries the lockstep-parity invariant.
...
//     `comprehension_verdict: SKIPPED_BASELINE_HIGH`. Default: 1.8.
//     The skip means "the agent already understands the concept WITHOUT the skill,
//     so the skill cannot demonstrate lift" — that is only true near the 2.0 ceiling.
//     At the old 1.0 midpoint the skip fired on half-understood concepts (baseline ≈ 1),
//     exactly the cells where a good skill lifts 1→2 and the measurement is most
//     informative (SH-6627 / SKI-6). 1.8 = 90% of the 2.0 max — near-saturation.
...
const DEFAULT_COMPREHENSION_BASELINE_SKIP_THRESHOLD = 1.8;
...
//
// Default FRONTIER (2026-06-02, two-frontier bidirectional design): the skills
// are deployed to and used by frontier agents, so a skill's value is whether it
// helps THAT agent. Measuring lift on a weaker stand-in measures the wrong thing
// — it keeps skills that only help the weak model and discards skills that
// genuinely help the frontier model (the weak model can't apply them well enough
// to show the gain), and the "ceiling effect" it tries to avoid is the CORRECT
// signal (a redundant skill, which enrichment must then push past). This is the
// canonical decision in docs/skill-audit-loop-philosophy.md § "WHY the generator
// (the measured agent) is a frontier model" — read it before changing this.
//
// This is NOT a no-lesser-models violation: that rule governs the GRADER (the
// quality judge), and 'opus' is a frontier model, not a lesser one. The bare
// `opus` alias resolves to the newest installed Opus via the claude CLI
// (deployment-matched, zero-touch). Override with COMPREHENSION_GENERATOR_MODEL;
// the bidirectional runner sets each direction's generator to a frontier from
// FRONTIER_PAIR explicitly, so this default is the single-direction fallback.
const DEFAULT_COMPREHENSION_GENERATOR_MODEL = 'opus';
...
//
// The `codex-current` role omits the `-m` flag so Codex serves its own current GPT
...
// which (with parity + cross-family) unblocks APPLICABLE/PASS. codex-current is
...
function runClaudeCliPrompt(prompt, {
  ...
  // model (e.g. resolveModelExecutor('codex-current').model === null reaching a
...
function runPromptWithCli(prompt, {
  ...
  // that supplied model='codex-current' but omitted `generator` silently ran the
  ...
  // No silent Sonnet (SH-6661): the generator REASONS to produce the eval answer;
  // a deliberately weak default measures model capability, not skill lift, and can
  // discard skills that genuinely help the frontier models this library deploys
  // on. The generator model must be supplied explicitly (a frontier, deployment-
  // matched model — see workspace AGENTS.md § Model Identity Discipline).
  if (!model) {
    throw new Error(
      'runPromptWithCli: no generator model supplied. The eval generator must run on an '
      + 'explicit frontier (deployment-matched) model — never a silent Sonnet default.',
    );
  }
  ...
  if (cli === 'codex') {
    ...
    // + network research); when tools-OFF it gets read-only. --skip-git-repo-check
    // keeps it working in sparse/sandboxed worktrees.
    ...
    return recordCodexModelFromOutput(runCommand('codex', args, { cwd: workspace }));
  }
  return runCommand(
    cli,
...
function runGraderPrompt(prompt, {
  ...
  if (grader === 'claude') {
    ...
    if (!model) {
      throw new Error(
        ...
        + 'explicit strong-tier model (e.g. the strongest-reasoning-grader role). '
  ...
  if (grader === 'gemini') {
    ...
    return runCommand('gemini', ['-p', prompt, '-m', resolvedGeminiModel, '--sandbox'], { cwd: workspace });
  }

  // Codex grader branch (two-frontier bidirectional "Claude" direction: Claude/opus
  // generates → Codex/gpt grades). Without this branch a codex grader silently fell
  // through to the OpenCode GPT route, so the receipt's grader identity disagreed
  // with the requested `codex-current` role. `codex-current` omits -m so Codex
  ...
  if (grader === 'codex') {
    const exec = resolveModelExecutor(model || 'codex-current');
    ...
    // comprehensionGraderModel(), or the 'strongest-reasoning-grader' role), route
    // to codex-current semantics (omit -m → Codex serves its own current GPT)
...
function runComprehensionEval(evalFile, options = {}) {
  ...
  // ── Cross-family certification (two-frontier bidirectional design) ──
  // PASS is reachable ONLY from a cross-family certifying run (e.g. Opus answers
  // → GPT-5.5 grades). A same-family or undeclared run is structurally only
  // PROVISIONAL (self-preference inflation guard, certification.js). This
  // generalizes the older `--single-model` cap: the DEFAULT is now provisional
  // unless the operator attests a certifying cross-family configuration with
  // --certifying + differing --generator-family/--grader-family. resolveCertificationTier
  // never selects a model — it records the operator's attestation.
  ...
  // ── Tools-ON execution profile (lockstep parity) ──
  // When an executionProfile is supplied (the bidirectional runner always supplies
  // one), the generator runs with FULL tools so it can research repo+web — research
  // IS the curation mechanism (skill-audit-loop-philosophy.md). Without a profile,
  // the single-direction default stays tools-OFF for back-compat. The profile is
  // recorded on the receipt so parity across the two directions can be asserted.
  const executionProfile = options.executionProfile
    || (options.toolsOn ? buildExecutionProfile({ cwd: workspace }) : null);
  const generatorAllowTools = executionProfile
    ? cliAccessForProfile(options.generator || 'claude', executionProfile).allowTools
    : false;
  ...
  if (options.dryRun) {
    ...
    console.log(
      ...
      `(${evalCount} eval(s) × 2 arms baseline/with_skill) + ` +
  ...
  for (const testCase of evals) {
    ...
    try {
      ...
      const baselineResponse = getEvalResponse(testCase.prompt, {
        ...
        disableSlashCommands: true,
        // Tools-ON when a bidirectional execution profile is supplied (research IS
        // the curation mechanism); tools-OFF for the legacy single-direction default.
      ...
      const withSkillResponse = getEvalResponse(withSkillPrompt, {
        ...
        model: generatorModel,
        disableSlashCommands: false,
        // Same tools level as the baseline run — lockstep parity within the
  ...
  // PASS is reserved for a cross-family dual-run grader confirmation. A
  // single-model self-assessment (--single-model) OR any run that is not a
  // cross-family certifying configuration (the DEFAULT, 2026-06-02) caps the
...
function validateApplicationReceiptArtifact(evalFilePath, applicationResult, artifactPath) {
  if (!artifactPath) {
    return { ok: false, reason: 'missing artifactPath; refusing to stamp a behavioral verdict without a durable application eval receipt' };
  }
  const resolvedArtifact = path.resolve(artifactPath);
  if (!fs.existsSync(resolvedArtifact)) {
    return { ok: false, reason: `receipt artifact does not exist: ${resolvedArtifact}` };
  }
  let parsed;
  try {
    parsed = JSON.parse(fs.readFileSync(resolvedArtifact, 'utf8'));
  } catch (error) {
    return { ok: false, reason: `receipt artifact is not parseable JSON: ${error.message}` };
  }
  if (parsed.mode !== 'application') {
    return { ok: false, reason: `receipt artifact mode is ${JSON.stringify(parsed.mode)}, expected "application"` };
  }
  if (parsed.aggregate_verdict !== applicationResult.aggregate_verdict) {
    return { ok: false, reason: `receipt aggregate_verdict=${JSON.stringify(parsed.aggregate_verdict)} does not match write-back result ${JSON.stringify(applicationResult.aggregate_verdict)}` };
  }
  if (Number(parsed.total || 0) !== Number(applicationResult.total || 0) || Number(parsed.errors || 0) !== Number(applicationResult.errors || 0)) {
    return { ok: false, reason: 'receipt total/errors do not match the write-back result' };
  }
  const evalSkillName = path.basename(path.dirname(path.dirname(path.resolve(evalFilePath))));
  if (parsed.skillName && parsed.skillName !== applicationResult.skillName && parsed.skillName !== evalSkillName) {
    return { ok: false, reason: `receipt skillName=${JSON.stringify(parsed.skillName)} does not match result/eval path` };
  }
  return {
    ok: true,
    path: resolvedArtifact,
    receipt: path.relative(WORKSPACE, resolvedArtifact),
    receipt_hash: fileSha256(resolvedArtifact),
  };
}

function unresolvedApplicationModelFields(applicationResult) {
  const fields = [];
  for (const key of ['resolved_generator_model', 'resolved_grader_model']) {
    const value = applicationResult && applicationResult[key];
    if (!value || value === 'latest-alias-unresolved') fields.push(key);
  }
  return fields;
}
 ...
 * @param {object} receipt   { at: ISO timestamp, status: 'pass'|'fail'|'mixed', runner: string, receipt?: string, receipt_hash?: string }
...
function updateEvalLastRunBlock(content, receipt) {
  ...
  // The durable eval-result receipt path: a verdict references the evidence behind it.
  // Accept legacy `artifact` input so old tests/callers still land on the current field name.
  const receiptPath = receipt.receipt || receipt.artifact;
  if (receiptPath) {
    blockLines.push(`  receipt: "${String(receiptPath).replace(/"/g, '\\"')}"`);
  }
  if (receipt.receipt_hash) {
    blockLines.push(`  receipt_hash: "${String(receipt.receipt_hash).replace(/"/g, '\\"')}"`);
  }
...
function stampApplicationVerdict(evalFilePath, applicationResult, dryRun, options = {}) {
  ...
  const unresolvedModels = unresolvedApplicationModelFields(applicationResult);
  if (verdict === 'APPLICABLE' && unresolvedModels.length > 0) {
    console.log(`\n[write-back] downgrading APPLICABLE → PROVISIONAL (unresolved concrete model ids: ${unresolvedModels.join(', ')}; alias-only receipts cannot certify).`);
    verdict = 'PROVISIONAL';
  }
  ...
  const statusMap = {
    APPLICABLE: 'pass',
    PROVISIONAL: 'pass',
    NOT_DISCRIMINATED_CEILING: 'mixed',
    EQUIVALENT_ON_FRONTIER: 'fail',
    REDUNDANT: 'fail',
    HARMFUL: 'fail',
    MIXED: 'mixed',
    FALSE_POSITIVE: 'fail',
  ...
  if (dryRun && !options.artifactPath) {
    console.log(`\n[write-back] DRY RUN — would stamp to ${skillMd}:`);
    console.log(`  application_verdict: ${verdict}`);
    console.log('  eval_last_run.receipt: (not required for dry-run preview)');
    return;
  }

  const receiptEvidence = validateApplicationReceiptArtifact(evalFilePath, applicationResult, options.artifactPath);
  if (!receiptEvidence.ok) {
    console.warn(`\n[write-back] Refusing to stamp application_verdict: ${receiptEvidence.reason}`);
    return;
  }
  ...
  const receipt = {
    ...
    receipt_hash: receiptEvidence.receipt_hash,
  };
  if (applicationResult.grader_model) receipt.model = applicationResult.grader_model;
  ...
  if (dryRun) {
    ...
    console.log(`  eval_last_run.receipt: ${receipt.receipt}`);
    console.log(`  eval_last_run.receipt_hash: ${receipt.receipt_hash}`);
    return;
  }
...
function main() {
  ...
  //   node .../evaluate-skill.js --application ../skills/skills/software-engineering-method/version-control
  ...
  if (mode === 'application') {
    ...
    try {
      applicationResult = runApplicationEval(evalFile, {
        ...
        // SH-6661: the measured agent runs on an explicit frontier model (default
        // Opus). Controlled via env APPLICATION_GENERATOR_MODEL (mirrors the
        // grader's env-only knob — per-run model CLI flags are REMOVED_FLAGS); the
        // in-process bidirectional runDirection passes options.generatorModel
        // directly per direction.
        // Tools-ON parity: --tools-on builds the lockstep execution profile so the
        // generator can research repo+web (research IS the curation mechanism).
  ...
  if (!evalFile) {
    console.error('Usage: node skill-graph/lib/audit/evaluate-skill.js [--mode ab|matrix|comprehension|application] [--application <skill-dir>] [--baseline-skill PATH] [--baseline-evals PATH] [--workspace DIR] [--eval-id 1,2] [--artifacts-dir DIR] [--output FILE] [--regression-gate] [--dry-run] <evals.json | comprehension.json | application.json>');
    ...
    console.error('  --dry-run             comprehension/application: preview planned model calls without invoking any CLI (true no-call preview, matching both modes — SKI-205).');
    process.exit(1);
  }
...
// It wires ONE direction: a per-direction frontier generator + the OTHER frontier as
// grader, under the SHARED tools-ON execution profile, then returns a normalized
...
// opus, codex for codex-current). Model identity stays role/alias-based — never a
// dated version. This is the ONLY place the two runners are wired with per-direction
// models; runBidirectionalEval owns the swap + reconciliation + parity gate.
function runEvalDirection({
  ...
  // model-inferred CLI and send a codex-current generator to the claude CLI with a
  ...
  // SH-6680: capture the concrete codex model from the actual run. codex-current
  ...
  if (mode === 'comprehension') {
    ...
    const file = evalFile || path.join(skillDir, 'evals', 'application.json');
    result = runApplicationEval(file, {
      workspace: cwd,
      // Skill-absent baseline working dir (keeps research tools ON) — see comprehension above.
      baselineWorkspace: baselineWorkspace || cwd,
      generator: generatorCli,
      grader: graderCli,
      generatorModel,
      ...
      graderFamily: graderFamily || graderModel,
      executionProfile,
      deps: { runGraderPrompt, getEvalResponse, logGraderDiscrepancy },
    });
    verdict = String(result.aggregate_verdict || '').toUpperCase();
  } else {
    throw new Error(`runEvalDirection: mode must be 'comprehension' | 'application' (got '${mode}')`);
  }
