...
//
// See docs/adr/0011-split-audit-verdict-into-four-verdicts.md (the v6→v7
// migration narrative is in git history + that ADR; the standalone
// docs/migrations/v6-to-v7.md was retired by ADR 0014).
...
function writeHealthFieldsFromEvalResult(evalFile, mode, result) {
  ...
  const updated = updateFrontmatterFields(original, {
    eval_score: String(score),
  ...
  return { written: true, skillFile, score, failedIds };
}
...
function stampComprehensionVerdict(evalFilePath, comprehensionResult, dryRun) {
  ...
  let content = fs.readFileSync(skillMd, 'utf8');
  content = updateFrontmatterField(content, 'comprehension_verdict', verdict);
  fs.writeFileSync(skillMd, content);
  console.log(`\n[write-back] Stamped comprehension_verdict: ${verdict} → ${skillMd}`);
}
...
function main() {
  ...
  if (mode === 'application') {
    if (!evalFile) {
      console.error('Usage: node skill-graph/lib/audit/evaluate-skill.js --mode application [--application <skill-dir>] [--workspace DIR] [--eval-id 1,2] [--dry-run] [--single-model] <application.json>');
      console.error('  --single-model   stamp PROVISIONAL instead of APPLICABLE (single-model self-assessment, not dual-run grader)');
      process.exit(1);
    }
    ...
    stampApplicationVerdict(evalFile, applicationResult, writeBackDryRun, { singleModel });
  ...
  // v6 Health Block — opt-in via --write-verdict. Writes eval_score /
  // eval_failed_ids / freshness to the skill's frontmatter so /evolve and
  // /audit can read the latest eval state from the Health Block instead of
  // crawling eval-history.jsonl. (Application mode stamps its verdict in the
  // dedicated branch above and never reaches here.)
  //
  // 2026-05-19: this gate must run BEFORE the comprehension errors-exit below.
  // An incomplete run (errors > 0) must NOT overwrite the prior baseline with a
  // misleading eval_score=0 — downstream readers (skill-evolution loop, /audit)
  // would then see "skill scored 0" instead of "skill never got a clean
  // baseline". Gating the write on errors=0 preserves the prior NO_BASELINE
  // state so the next clean run can fill it in. (formal-methods-tla,
  // rag-evaluation 2026-05-19.)
  const isIncompleteRun = mode === 'comprehension' && result && Number(result.errors) > 0;
  const writeVerdict = args['write-verdict'] === true || args['write-verdict'] === 'true';
  if (writeVerdict) {
    if (isIncompleteRun) {
      console.log(
      ...
      if (writeRes.written) {
        console.log(
          `Health Block updated at ${writeRes.skillFile}: ` +
