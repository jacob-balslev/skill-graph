...
const PROGRESS_DIR = path.join(REPO_ROOT, '.opencode', 'progress', 'skill-evolution');
...
function findSkillDir(skillName) {
  const candidates = [
    path.join(REPO_ROOT, 'skills', skillName),
    path.join(REPO_ROOT, 'skills', 'sales-hub', skillName),
  ];
...
function checkpoint(cmd, extraArgs = []) {
  const result = spawnSync('node', [
    path.join(REPO_ROOT, 'scripts', 'loop-checkpoint.js'),
    cmd,
    '--loop', LOOP_NAME,
    ...extraArgs,
...
function emitEvent(type, data = {}) {
  try {
    ...
    spawnSync('node', [
      path.join(REPO_ROOT, 'scripts', 'agent-events.js'),
      ...args,
    ], { encoding: 'utf8', timeout: 5000, cwd: REPO_ROOT });
  } catch {}
}
...
function phaseAnalyze() {
  ...
  const result = spawnSync('node', [
    path.join(REPO_ROOT, 'scripts', 'skill-evolution-analyzer.js'),
    '--json',
    '--out', analysisPath,
...
function buildImprovementLoopArgs(skillName, childArtifactsDir, options = {}) {
  ...
  const args = [
    path.join(REPO_ROOT, 'scripts', 'run-skill-improvement-loop.js'),
...
function executeItem(item, options = {}) {
  ...
  switch (item.action) {
    case 'improve_skill': {
      ...
      const result = spawnSync('node', args, { stdio: 'inherit', cwd: REPO_ROOT });
    ...
    case 'scaffold_skill': {
      ...
      if (redirectTarget) {
        const result = spawnSync('node', [
          path.join(REPO_ROOT, 'scripts', 'run-skill-improvement-loop.js'),
          '--skills-root', path.join(REPO_ROOT, 'skills'),
      ...
      const result = spawnSync('node', [
        path.join(REPO_ROOT, 'scripts', 'skill-auto-create.js'),
    ...
    case 'generate_evals': {
      ...
      const result = spawnSync('node', [
        path.join(REPO_ROOT, 'scripts', 'dispatch-solver.js'),
...
function runAutoImproveCycle(args) {
  ...
  for (const item of queue) {
    ...
    const score = item._asset.last_score !== null ? item._asset.last_score.toFixed(3) : 'n/a';
