...
function validateGeneratedEvalsBundle(filePath) {
  ...
  return validateGeneratedComprehensionEvals(parsed);
}

function resolveEvalType() {
...
Eval generation writes evals/comprehension.json.
`);
}
...
function buildFallbackAnalysis(args, analysisPath) {
  ...
  for (const filePath of skillFiles) {
    ...
    const comprehensionVerdict = fm.comprehension_verdict || 'UNVERIFIED';
...
function executeItem(item, options = {}) {
  ...
  switch (item.action) {
    ...
    case 'generate_evals': {
      ...
      const evalFilePath = path.join(evalsDir, 'comprehension.json');
      ...
      if (fs.existsSync(evalFilePath)) {
        try {
          const validation = validateGeneratedEvalsBundle(evalFilePath);
          success = validation.valid;
          details = success ? `generated ${validation.count} comprehension evals` : validation.reason;
          if (!success) {
            try { fs.unlinkSync(evalFilePath); } catch {}
          }
        } catch {
          success = false;
          details = 'comprehension.json parse error after generation';
          try { fs.unlinkSync(evalFilePath); } catch {}
        }
      } else {
        success = result.status === 0;
        details = success ? 'dispatch completed but no comprehension.json file found' : 'dispatch failed';
      }
      break;
    }
