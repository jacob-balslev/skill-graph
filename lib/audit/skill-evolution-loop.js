...
function validateGeneratedComprehensionEvals(parsed) {
  ...
  if (evals.length < 5) {
    return { valid: false, reason: `expected at least 5 comprehension evals, got ${evals.length}` };
  }

  const seenIds = new Set();
  for (const entry of evals) {
    if (!Number.isInteger(Number(entry.id)) || Number(entry.id) < 1) {
      return { valid: false, reason: 'each comprehension eval requires a positive integer id' };
    }
  ...
  if (!evals.some((entry) => typeof entry.negative_expectation === 'string' && entry.negative_expectation.trim())) {
    return { valid: false, reason: 'comprehension eval set needs at least one negative_expectation' };
  }
    ...
    case 'generate_evals': {
      ...
      const evalPrompt = evalType === 'application'
        ...
        : [
          ...
          '',
          'Read the skill content below and create 5-7 comprehension evals in the current schema:',
          '- Top-level fields: skill_name, subject, adjacent_concepts, evals[]',
          '- Each eval needs: id, dimension, prompt, substance, calibration, truth_mode, skill_type, criticality',
          '- Cover the major dimensions: definition, mental_model, purpose, boundary, taxonomy, analogy, application',
