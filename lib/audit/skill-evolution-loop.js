 ...
 *
 * Loop phases:
 *   ANALYZE  → Run analyzer when present, otherwise fallback scan
 ...
 *   skill-graph evolve --pilot template-owned-meta --top 2
 *   skill-graph evolve --auto-improve --max-cycles 3 --failure-budget 5
 *   skill-graph evolve --analyze-only
 *   skill-graph evolve --resume
 *   skill-graph evolve --workspace-root /path/to/workspace --skills-dir /path/to/workspace/skills
 *   skill-graph evolve --output-dir /path/to/workspace/audits
 *   skill-graph evolve --actions improve,scaffold
 *   skill-graph evolve --continue-after-failures
 */
...
// resolveModelExecutor maps a model token → { cli, model } so generate_evals can
// invoke the model IN-LOOP (SH-6643 option B) instead of spawning a Development-tree
// dispatch-solver.js — keeping @skill-graph/cli self-contained per ADR-0009.
const {
...
const DEFAULT_PROGRESS_DIR = path.join(_PROGRESS_BASE, 'skill-evolution');
let PROGRESS_DIR = DEFAULT_PROGRESS_DIR;
...
function resolveProgressDir(args = {}) {
  if (args['output-dir']) return path.resolve(args['output-dir']);
  return DEFAULT_PROGRESS_DIR;
}

function setProgressDir(args = {}) {
  PROGRESS_DIR = resolveProgressDir(args);
  return PROGRESS_DIR;
}
...
function findSkillDir(skillName) {
  ...
  // Nested sibling layout: the canonical library lives at the configured
  // skill_roots (e.g. ../skills/skills) as <root>/[<subject>/]<name>/SKILL.md.
  // A flat <root>/<name> lookup misses the v8 subject-nested layout, so walk
  // one directory level under each configured root. (SH-6489 / SH-6317 class.)
  for (const root of resolveConfiguredSkillRoots(_WORKSPACE)) {
    const direct = path.join(root, skillName);
    ...
    try {
      subdirs = fs.readdirSync(root, { withFileTypes: true })
        ...
        .map(d => d.name);
    } catch { /* root may not exist in some layouts */ }
    for (const sub of subdirs) {
      const nested = path.join(root, sub, skillName);
...
const COMPREHENSION_DIMENSIONS = new Set(['definition', 'mental_model', 'purpose', 'boundary', 'taxonomy', 'analogy', 'application', 'misconception']);
const COMPREHENSION_SUBSTANCE = new Set(['concept', 'procedure', 'boundary']);
const COMPREHENSION_CALIBRATION = new Set(['semantic', 'exact-match', 'structural']);
const COMPREHENSION_TRUTH_MODE = new Set(['conceptual_correctness', 'conceptual_correctness_plus_repo_application', 'spec_grounded', 'principle_grounded']);
const COMPREHENSION_SKILL_TYPE = new Set(['concept', 'procedure', 'router', 'overlay']);
const COMPREHENSION_CRITICALITY = new Set(['critical', 'high', 'medium', 'low']);
const APPLICATION_CRITICALITY = new Set(['critical', 'high', 'normal', 'low']);

function validateGeneratedComprehensionEvals(parsed) {
  const evals = parsed && parsed.evals;
  if (!Array.isArray(evals)) {
    return { valid: false, reason: 'comprehension.json must contain { "evals": [] }' };
  }
  if (evals.length < 5) {
    return { valid: false, reason: `expected at least 5 Comprehension cases, got ${evals.length}` };
  }

  const seenIds = new Set();
  for (const entry of evals) {
    if (!Number.isInteger(Number(entry.id)) || Number(entry.id) < 1) {
      return { valid: false, reason: 'each Comprehension case requires a positive integer id' };
    }
    if (seenIds.has(Number(entry.id))) {
      return { valid: false, reason: `duplicate eval id ${entry.id}` };
    }
    seenIds.add(Number(entry.id));
    if (!COMPREHENSION_DIMENSIONS.has(entry.dimension)) return { valid: false, reason: `eval ${entry.id} has invalid dimension` };
    if (typeof entry.prompt !== 'string' || !entry.prompt.trim()) return { valid: false, reason: `eval ${entry.id} is missing a prompt` };
    if (!COMPREHENSION_SUBSTANCE.has(entry.substance)) return { valid: false, reason: `eval ${entry.id} has invalid substance` };
    if (!COMPREHENSION_CALIBRATION.has(entry.calibration)) return { valid: false, reason: `eval ${entry.id} has invalid calibration` };
    if (!COMPREHENSION_TRUTH_MODE.has(entry.truth_mode)) return { valid: false, reason: `eval ${entry.id} has invalid truth_mode` };
    if (!COMPREHENSION_SKILL_TYPE.has(entry.skill_type)) return { valid: false, reason: `eval ${entry.id} has invalid skill_type` };
    if (!COMPREHENSION_CRITICALITY.has(entry.criticality)) return { valid: false, reason: `eval ${entry.id} has invalid criticality` };
  }
  if (!evals.some((entry) => typeof entry.negative_expectation === 'string' && entry.negative_expectation.trim())) {
    return { valid: false, reason: 'Comprehension needs at least one negative_expectation' };
  }

  return { valid: true, count: evals.length };
}

function validateGeneratedApplicationEvals(parsed) {
  const cases = parsed && parsed.cases;
  if (!Array.isArray(cases)) {
    return { valid: false, reason: 'application.json must contain { "mode": "application", "cases": [] }' };
  }
  if (parsed.mode !== 'application') {
    return { valid: false, reason: 'application.json mode must be "application"' };
  }
  if (cases.length < 5) {
    return { valid: false, reason: `expected at least 5 application cases, got ${cases.length}` };
  }

  const seenIds = new Set();
  for (const entry of cases) {
    if (!Number.isInteger(Number(entry.id)) || Number(entry.id) < 1) return { valid: false, reason: 'each application case requires a positive integer id' };
    if (seenIds.has(Number(entry.id))) return { valid: false, reason: `duplicate case id ${entry.id}` };
    seenIds.add(Number(entry.id));
    if (typeof entry.scenario_type !== 'string' || !entry.scenario_type.trim()) return { valid: false, reason: `case ${entry.id} is missing scenario_type` };
    if (!APPLICATION_CRITICALITY.has(entry.criticality)) return { valid: false, reason: `case ${entry.id} has invalid criticality` };
    if (typeof entry.red_herring !== 'boolean') return { valid: false, reason: `case ${entry.id} is missing boolean red_herring` };
    for (const key of ['scenario', 'context', 'question']) {
      if (typeof entry[key] !== 'string' || !entry[key].trim()) return { valid: false, reason: `case ${entry.id} is missing ${key}` };
    }
    for (const key of ['expected_flags', 'expected_fix_hints', 'absent_signals']) {
      if (!Array.isArray(entry[key])) return { valid: false, reason: `case ${entry.id} is missing ${key}[]` };
    }
  }
  if (!cases.some((entry) => entry.red_herring === true)) {
    return { valid: false, reason: 'application eval set needs at least one red_herring case' };
  }

  return { valid: true, count: cases.length };
}

function validateGeneratedEvalsBundle(filePath, options = {}) {
  const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8'));
  const evalType = options.evalType || (path.basename(filePath) === 'application.json' ? 'application' : 'comprehension');
  return evalType === 'application'
    ? validateGeneratedApplicationEvals(parsed)
    : validateGeneratedComprehensionEvals(parsed);
}

function resolveEvalType(args = {}) {
  const raw = String(args['eval-type'] || args['eval-mode'] || args.type || 'comprehension').toLowerCase();
  return raw === 'application' ? 'application' : 'comprehension';
}

function printHelp() {
  process.stdout.write(`Usage: node lib/audit/skill-evolution-loop.js [options]

Run the Skill Graph evolution loop.

Options:
  --top <n>                    Number of triaged items to show (default: 5)
  --max-iterations <n>          Maximum items to execute (default: 20)
  --actions <list>              Comma-separated action filter
  --pilot <skill>               Restrict execution to one skill
  --auto-improve                Run continuous improve/evaluate cycles
  --continuous                  Poll and execute repeatedly
  --continue-after-failures     Keep processing after failures
  --failure-budget <n>          Consecutive failure budget
  --eval-type <type>            Eval artifact to generate: comprehension or application
  --help, -h                    Show this help text

Current eval generation writes evals/comprehension.json by default. Use
--eval-type application to write evals/application.json.
`);
}
...
// ─── Workspace-script resolution (SH-6643) ───────────────────────
//
// loop-checkpoint.js and agent-events.js are WORKSPACE orchestration scripts; in
// the ~/Development context they live under scripts/loop/ and scripts/agent/ (NOT
// flat scripts/). A standalone `npm install -g @skill-graph/cli` has neither. So
// these helpers resolve the real path defensively (known subdirs first, then flat)
// and GRACEFULLY NO-OP when absent — checkpointing/telemetry is optional coordination,
// never core to loop correctness. This is the self-contained half of SH-6643 option B:
// the loop never hard-fails on a missing Development-tree orchestration script.
const _WORKSPACE_SCRIPT_SUBDIRS = ['loop', 'agent', 'monitoring', '.'];
function resolveWorkspaceScript(name) {
  for (const sub of _WORKSPACE_SCRIPT_SUBDIRS) {
    const candidate = sub === '.'
      ? path.join(_WORKSPACE, 'scripts', name)
      : path.join(_WORKSPACE, 'scripts', sub, name);
...
function checkpoint(cmd, extraArgs = []) {
  const script = resolveWorkspaceScript('loop-checkpoint.js');
  if (!script) {
    // Standalone / no workspace checkpoint integration — degrade gracefully.
    return cmd === 'read' ? {} : false;
  }
...
function emitEvent(type, data = {}) {
  try {
    const script = resolveWorkspaceScript('agent-events.js');
    if (!script) return; // no workspace event bus — silently skip.
...
// Pure arg-builder for the in-loop model invocation (SH-6643 option B). Returns
// { cli, args } for the resolved model executor. Extracted as a pure function so the
// invocation contract (real CLI, never a `node <missing-script>` spawn) is unit-testable
// without a live model call. Mirrors run-skill-improvement-loop.js runImprover's dispatch.
function buildModelInvocation(prompt, { model = 'opus', apply = true } = {}) {
  const { cli, model: resolvedModel } = resolveModelExecutor(model);
  let args;
  if (cli === 'claude') {
    args = ['-p', prompt, '--model', resolvedModel, '--output-format', 'text',
      '--permission-mode', apply ? 'acceptEdits' : 'plan'];
    if (apply) args.push('--dangerously-skip-permissions');
  } else if (cli === 'gemini') {
    args = apply ? ['--yolo', '-m', resolvedModel, '-p', prompt] : ['-m', resolvedModel, '-p', prompt];
  } else {
    // OpenCode-family CLI.
    args = ['run', '-m', resolvedModel, '--format', 'default', prompt];
  }
  return { cli, args };
}

// In-loop model invocation. Kept self-contained here so generate_evals does not spawn
// a Development-tree dispatch-solver.js. `apply: true` lets the model write the target
// file (acceptEdits / --yolo), matching the eval-generation contract.
function runInLoopModel(prompt, opts = {}) {
  const { cli, args } = buildModelInvocation(prompt, opts);
  return spawnSync(cli, args, { encoding: 'utf8', timeout: 180000, cwd: _WORKSPACE });
}
...
function phaseTriage(analysis, topN, actionFilter) {
  ...
  for (const item of triaged.slice(0, 10)) {
    const tag = { improve_skill: 'IMPROVE', scaffold_skill: 'CREATE', fix_semantics: 'SEMANTIC', archive_skill: 'REMOVE' }[item.action] || item.action;
...
function buildImprovementLoopArgs(skillName, childArtifactsDir, options = {}) {
  ...
  const args = [
    // SH-6640 Break #5: the canonical improve loop is this file's sibling in
    // lib/audit/, NOT <workspace>/scripts/ (which does not exist after the
    // ADR-0009 consolidation — same hardcoded-wrong-path bug class as Breaks
    // 1-4). Resolve relative to __dirname so it works regardless of cwd.
    path.join(__dirname, 'run-skill-improvement-loop.js'),
  ...
  // SH-6641 follow-on: thread the grader through so the keep/revert gate uses a
  // reliable, top-tier judge. The improve loop's own default is `opencode`,
  // which hangs when driven from Node on eval prompts (the known
  // opencode-run-from-node gotcha) and would stall every evolve cycle on the
  // first eval. `evolve --grader claude` runs the bundled, reliable Opus grader.
  if (options.grader) {
    args.push('--grader', options.grader);
  }
...
function executeItem(item, options = {}) {
  ...
  switch (item.action) {
    ...
    case 'scaffold_skill': {
      ...
      if (redirectTarget) {
        // SH-6640 Break #5: canonical improve loop is the sibling in lib/audit/,
        // and the skills root is the real (sibling-repo) location of the redirect
        // target — NOT <workspace>/scripts/ and <workspace>/skills (both wrong
        // post-consolidation). Resolve the target's dir so --skills-root/--include
        // point at the actual skill; fall back to a bare --include (the improve
        // loop resolves the configured skill root itself) when not found.
        const redirSkillDir = findSkillDir(redirectTarget);
        const result = spawnSync('node', [
          path.join(__dirname, 'run-skill-improvement-loop.js'),
          ...(redirSkillDir
            ? ['--skills-root', path.dirname(redirSkillDir), '--include', path.basename(redirSkillDir)]
      ...
      //
      // --skip-eval is DELIBERATE and stays (Step 4 / finding E4 decision, 2026-05-31).
      // It skips skill-auto-create's Phase 4 (the A/B GRADING run), NOT Phase 2 (eval
      // authoring): the new skill still ships with its SKILL.md AND eval artifacts, it
      // is just not graded inline. Grading a brand-new, unproven skill during a
      // throughput-oriented batch walk would add ~10min per skill, and the correct
      // place to grade is a dedicated evaluate pass, not creation time. The skill
      // therefore enters the corpus eval-authored but UNVERIFIED — its correct
      // intermediate state. The create→evaluate lifecycle COMPLETES on a later walker
      // pass: an UNVERIFIED skill is top-priority in triage and routes to improve_skill,
      // whose inner loop runs generate→evaluate→keep/revert.
      //   Lifecycle caveat (do not mistake for abandonment): improve/evaluate passes
      //   operate on a `git worktree add HEAD`, which only checks out COMMITTED files,
      //   so a scaffolded skill must be committed before that later pass can see and
      //   grade it. The completion is deferred, not skipped. For eval-GATED creation at
      //   creation time (grade before the skill is accepted), use /skill-discovery
      //   instead of the evolve scaffold.
      //
      // skill-auto-create.js lives under scripts/skill/, not scripts/. A wrong
      // path here previously made EVERY scaffold_skill action fail silently into
      // the result log (the spawn just returned non-zero). Resolve explicitly and
      // fail loud if the script ever moves again, so the break is greppable.
      const autoCreateScript = path.join(_WORKSPACE, 'scripts', 'skill', 'skill-auto-create.js');
      if (!fs.existsSync(autoCreateScript)) {
        success = false;
        details = `scaffold aborted: skill-auto-create.js not found at ${autoCreateScript}`;
        break;
      }
      const result = spawnSync('node', [
        autoCreateScript,
      ...
      success = result.status === 0;
      // Make the deferral explicit, not silent: report the new skill's true state so
      // a reader does not assume it was graded (the E4 "silently orphaned" worry).
      details = success
        ? 'created (UNVERIFIED — eval artifacts authored, grading deferred to a later evaluate/improve pass; commit the skill so that pass can see it)'
        : buildCommandFailureDetails(result);
      break;
    }
    ...
    case 'generate_evals': {
      ...
      const evalType = options.evalType || 'comprehension';
      const evalFileName = evalType === 'application' ? 'application.json' : 'comprehension.json';
      const evalFilePath = path.join(evalsDir, evalFileName);

      // Dispatch a model to generate current-contract evals grounded in the skill's content.
      ...
      const evalPrompt = evalType === 'application'
        ? [
          `Generate an evals/application.json file for the "${item.skill}" skill.`,
          '',
          'Read the skill content below and create 5-7 application cases in the current schema:',
          '- Top-level fields: skill_name, subject, mode: "application", cases[]',
          '- Each case needs: id, scenario_type, criticality, red_herring, scenario, context, question, expected_flags[], expected_fix_hints[], absent_signals[]',
          '- Include at least one red_herring: true case that should NOT trigger this skill',
          '- Use concrete scenarios that measure behavior change, not definition recall',
          '',
          `Write the output to: ${evalFilePath}`,
          '',
          '## Skill Content',
          skillContent.slice(0, 6000),
        ].join('\n')
        : [
          `Generate an evals/comprehension.json file for the "${item.skill}" skill.`,
          '',
          'Read the skill content below and create 5-7 Comprehension cases in the current schema:',
          '- Top-level fields: skill_name, subject, adjacent_concepts, evals[]',
          '- Each case needs: id, dimension, prompt, substance, calibration, truth_mode, skill_type, criticality',
          '- Cover the major Comprehension criteria: definition, mental_model, purpose, boundary, taxonomy, analogy, application, and misconception when relevant',
          '- The JSON field is named dimension for schema compatibility; it names the primary criterion for the case',
          '- Include at least one negative_expectation, especially for boundary or critical cases',
          '- Use scenarios that test the skill-specific judgment, not generic advice',
          '',
          `Write the output to: ${evalFilePath}`,
          '',
          '## Skill Content',
      ...
      fs.writeFileSync(promptFile, evalPrompt); // retained as a debugging artifact

      // SH-6643 option B: invoke the generator IN-LOOP (self-contained) instead of
      // spawning the Development-tree dispatch-solver.js (which (a) does not exist in
      // a standalone @skill-graph/cli install and (b) lives at scripts/loop/, not the
      // flat scripts/ this used to assume). Sonnet is the GENERATOR tier here, not a
      // grader — eval AUTHORING, not quality JUDGMENT — so it is the right model per
      // no-lesser-models-for-quality (graders stay Opus/GPT-5.4 elsewhere).
      const result = runInLoopModel(evalPrompt, { model: 'opus', apply: true });
      ...
      if (fs.existsSync(evalFilePath)) {
        try {
          const validation = validateGeneratedEvalsBundle(evalFilePath, { evalType });
          success = validation.valid;
          details = success ? `generated ${validation.count} ${evalType} evals` : validation.reason;
          if (!success) {
            try { fs.unlinkSync(evalFilePath); } catch {}
          }
        } catch {
          success = false;
          details = `${evalFileName} parse error after generation`;
          try { fs.unlinkSync(evalFilePath); } catch {}
        }
      } else {
        success = result.status === 0;
        details = success ? `dispatch completed but no ${evalFileName} file found` : 'dispatch failed';
      }
      break;
    }

    case 'archive_skill': {
      // Legacy action key: suggest removal to git history, never an in-tree archive.
      console.log(`    SUGGESTION: review ${item.skill} for removal to git history (zero injections, low eval scores)`);
      success = true;
      details = 'suggested for removal (not auto-executed)';
      break;
    }
...
function phaseExecute(triaged, maxIterations, options = {}) {
  ...
  // evolve corpus walk too. options.statusFile opts in; one agents[] row per planned skill.
  const statusFile = options.statusFile || null;
  const hbRows = triaged.slice(0, limit).map((it) => ({ model: it.skill, tier: 'evolve', phase: it.action || 'evolve', state: 'pending' }));
  let hbDone = 0;
  let hbFailed = 0;
  const heartbeat = (complete = false) => writeRunnerHeartbeat(statusFile, {
    skill: null, phase: 'evolve', total: hbRows.length, done: hbDone, failed: hbFailed, complete, agents: hbRows,
  });
  heartbeat();
  ...
  for (let i = 0; i < limit; i++) {
    ...
    if (skillFails >= perSkillFailureLimit) {
      ...
      if (hbRows[i]) { hbRows[i].state = 'failed'; hbFailed += 1; heartbeat(); }
      continue;
    }

    if (hbRows[i]) { hbRows[i].state = 'running'; heartbeat(); }
    const result = execute(item, options);
    results.push(result);
    if (hbRows[i]) { hbRows[i].state = result.success ? 'done' : 'failed'; result.success ? (hbDone += 1) : (hbFailed += 1); heartbeat(); }
...
function runAutoImproveCycle(args) {
  ...
  const execution = phaseExecute(triaged, maxIterations, {
    ...
    grader: args.grader,
    evalType: resolveEvalType(args),
    statusFile: args['status-file'], // D5: heartbeat status.json (watch-panel.js / watch-audit-batch.sh)
  });
