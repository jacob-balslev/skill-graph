...
//   { model, tier, proposalPath, noveltyMemoPath, iterationSuggestionsPath, diagnosticsPath, ok, bytes }.
...
function main(argv) {
  ...
  if (!p || p.ok === false || !p.proposalPath) {
    finish({ ok: false, error: (p && p.error) || 'no proposal produced', diagnosticsPath: p && p.diagnosticsPath }, 1);
  }
  let bytes = 0;
  ...
  finish({
    ...
    diagnosticsPath: p.diagnosticsPath || null,
  }, bytes > 0 ? 0 : 1);
}
