...
// the dispatch mechanism the standalone `run-panel-enrich.js` could never reach (a Node CLI
// cannot call the Agent tool; only the session can). It REUSES the tested live deps
// (`panel-enrich-live-deps.js`) — no re-implementation of research/propose/write/verify.
//
...
function main(argv) {
  ...
  const deps = createPanelEnrichDeps({ skillGraphRoot: cwd, dryRun: Boolean(args['dry-run']), advisoryTimeoutMs: timeoutMs });
  ...
  finish({ ok: bytes > 0, bytes, proposalPath: p.proposalPath, noveltyMemoPath: p.noveltyMemoPath || null }, bytes > 0 ? 0 : 1);
}
