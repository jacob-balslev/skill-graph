...
const { buildExecutionProfile, RESEARCH_REPO_ONLY, FENCE_OS_ISOLATED } = require('./eval-execution-profile');
...
function prepareIsolatedEvalWorkspace({ skillGraphRoot, research, fsImpl = fs } = {}) {
  ...
  return {
    ...
    // A6: this cwd is a public-only temp checkout, so the repoScope/answer-key fence is
    // OS-enforced (the private Development tree is physically off the agent's path) —
    // fence: 'os-isolated'. A real-repo-root cwd would only be prompt-level fenced.
    executionProfile: buildExecutionProfile({ cwd: publicSkillGraphRoot, research: research || RESEARCH_REPO_ONLY, fence: FENCE_OS_ISOLATED }),
