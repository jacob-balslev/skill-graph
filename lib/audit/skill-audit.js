 ...
 *                         --fix, reports the (currently empty) repair catalog.
 *   --fix                 Deterministic Integrity-gate remediation framework. The
 *                         deterministic repair catalog is currently EMPTY: the v7 ->
 *                         v8 shape codemod was retired to git history per AGENTS.md
 *                         § "Major Version Is a Clean Cut" (it produced the interim
 *                         deployment_target shape that the ADR-0017 amendment
 *                         replaced with public + free-text scope). The framework
 *                         stays for the next version's catalog; today remaining lint
 *                         errors route to improve/manual. See skill-audit-loop/SKILL_AUDIT_LOOP.md and
 ...
 *   scorecard.md  — seven qualitative scorecard rows with scores from lint (metadata) and grader (others)
 *
...
// Deterministic repair catalog applied under --fix. The v7 -> v8 shape codemod
// (migrate-frontmatter.js) was retired to git history per AGENTS.md § "Major
// Version Is a Clean Cut" once v8 became canonical (it produced the interim
// deployment_target shape that the ADR-0017 amendment replaced with public +
// free-text scope, so it could no longer emit a valid current-v8 skill). The
// --fix FRAMEWORK stays for the next version's catalog; the catalog is empty today.
// Recover the codemod via `git show <sha>^:lib/audit/migrate-frontmatter.js` / `git tag schema-v7`.
...
function main() {
  ...
  if (opts.dryRun) {
    ...
    if (opts.fix) {
      console.log('dry-run: --fix deterministic repair catalog is empty (v7->v8 codemod retired per clean-cut) — no deterministic shape changes available; remaining lint errors route to improve/manual.');
    }
  ...
  // --fix: deterministic Integrity-gate remediation framework. The deterministic
  // repair catalog is currently EMPTY (see below). The framework (runLint /
  // runDrift) stays for the next version's catalog. See skill-audit-loop/SKILL_AUDIT_LOOP.md
  // § The Inner Pipeline of `audit` and docs/proposals/skill-audit-loop-fix-paths.md.
  if (opts.fix && lintErrors.length > 0) {
    // The deterministic repair catalog is currently EMPTY. The v7 -> v8 shape
    // codemod was retired to git history per the clean-cut doctrine (see the
    // require-block comment above); it produced the interim deployment_target
    // shape that the ADR-0017 amendment replaced (public + free-text scope), so
    // it could no longer emit a valid current-v8 skill. The --fix framework
    // (runLint / runDrift) stays for the next version's catalog.
    console.log('  --fix: deterministic repair catalog is empty (v7->v8 codemod retired per clean-cut) — remaining lint errors are not deterministic-shape issues; route to improve/manual.');
  }
