...
function main() {
  ...
  try {
    ...
    if (writeRes.written) {
      console.log(`Stamped Health Block (audit-state sidecar) on ${path.relative(_WORKSPACE, writeRes.path)}:`);
      ...
      console.log(`Health Block already current on ${path.relative(_WORKSPACE, writeRes.path)} — no fields changed.`);
    }
  } catch (err) {
