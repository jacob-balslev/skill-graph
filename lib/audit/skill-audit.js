 ...
 *    gate, seeds findings.md / verdict.md / scorecard.md, and **stamps the
 *    structural layer of the Health Block back onto the skill's SKILL.md
 *    frontmatter** (last_audited, lint_verdict, structural_verdict). Fast,
 ...
 *    auditor or by `--graded`. truth_verdict is **not** written in this mode —
 *    run `skill-graph drift` separately to populate it.
 *
...
function parseArgs(argv) {
  ...
  const result = {
    skillName:      null,
    auditRoot:      path.join(_WORKSPACE, 'examples', 'audits'),
    force:          false,
    dryRun:         false,
    graded:         false,
...
function main() {
  ...
  if (opts.dryRun) {
    ...
    const { exitCode } = runLint(skillDir);
    console.log(`dry-run: lint ${exitCode === 0 ? 'PASS' : 'FAIL'} (exit ${exitCode})`);
  ...
  // F14 / SAL-1 caught. We can compute structural_verdict from lint here; the
  // truth_verdict still requires a separate `skill-graph drift` pass because
  // this command does not invoke the drift sentinel inline (tracked separately).
  ...
  try {
    ...
    if (updated !== skillMdContent) {
      ...
      console.log('  truth_verdict: (unchanged — run `skill-graph drift` to populate)');
  ...
  if (lintErrors.length > 0) {
    ...
    console.log('\nNext step: open findings.md and complete the human-judgment TODO sections, or re-run with --graded to invoke the grader. Run `skill-graph drift` separately to populate truth_verdict.');
  } else {
