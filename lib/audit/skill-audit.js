 ...
 *    auditor or by `--graded`. NOTE: the Behavior-Gate verdict
 *    (comprehension_verdict) is NOT written by this
 *    script in either mode — `--graded` grades the seven qualitative scorecard
 *    dimensions into the artifacts, but the behavior verdict is stamped only
 *    by `evaluate-skill.js --mode comprehension`.
 *
...
// Map the drift sentinel's per-skill status to the Audit Status
...
function main() {
  ...
  if (opts.graded) {
    console.log(`Mode: GRADED audit on skills/${opts.skillName} — Integrity Gate (lint + drift) + qualitative grading of the seven scorecard dimensions. Stamps structural_verdict + truth_verdict + last_audited. The Behavior-Gate verdict (comprehension_verdict) is NOT stamped here — run \`evaluate --mode comprehension\` for it.`);
  } else {
    console.log(`Mode: INTEGRITY-only audit on skills/${opts.skillName} (no --graded) — runs lint + drift, stamps structural_verdict + truth_verdict + last_audited. Qualitative scorecard dimensions are written as TODO placeholders for human review. Re-run with --graded to invoke the grader on those qualitative dimensions. (comprehension_verdict is stamped by \`evaluate --mode comprehension\`, not by audit.)`);
  }
  ...
  // from drift) are now written here; the Behavior Gate verdict
  // (comprehension_verdict) is NOT written by audit at
  // all — it is stamped only by `evaluate --mode comprehension`.
  ...
  try {
    ...
    if (writeRes.written) {
      ...
      console.log('  comprehension_verdict: (unchanged — stamped by `evaluate --mode comprehension`, not by audit)');
    } else {
