 ...
 *    layer of Audit Status into the skill's audit-state.json sidecar**
 ...
 *    auditor or by `--graded`. NOTE: the Behavior-Gate verdicts
 *    (comprehension_verdict / application_verdict) are NOT written by this
 *    script in either mode — `--graded` grades the seven qualitative scorecard
 *    dimensions into the artifacts, but the behavior verdicts are stamped only
 *    by `evaluate-skill.js --mode comprehension|application`.
 *
 ...
 *   node scripts/skill-audit.js <skill-name> --fix
 ...
 *                         Useful for smoke-testing standalone installs. With
 *                         --fix, previews the migration without writing.
 *   --fix                 Deterministic Integrity-gate remediation. When lint
 *                         finds shape violations, apply the v7 -> v8 frontmatter
 *                         migration (remove retired type/category/operation/etc.,
 *                         rename domain -> taxonomy_domain & domain_object ->
 *                         subject_matter, drop enum scope, add deployment_target),
 *                         regenerate field comments, and re-lint. No LLM, no
 *                         evals, no keep-or-revert — binary lint pass/fail. The
 *                         caller commits per-skill. See skill-audit-loop/SKILL_AUDIT_LOOP.md and
 *                         docs/proposals/skill-audit-loop-fix-paths.md.
...
// ADR-0019: Integrity-gate verdicts (structural/truth/lint/last_audited) are
// stamped into the audit-state.json sidecar, NOT SKILL.md frontmatter.
const { writeSidecarFields } = require('./audit-state-sidecar');
// Deterministic v7 -> v8 shape migration applied under --fix (repair catalog).
const { migrateFrontmatterToV8 } = require('./migrate-frontmatter');
...
// ---------------------------------------------------------------------------
// Field-purpose comment backfill runner (used by --fix)
// ---------------------------------------------------------------------------

// Invoke scripts/backfill-field-purpose-comments.js on a single SKILL.md to
// regenerate the v8-aware field-purpose comments after migrate-frontmatter.js
// has stripped the stale v7 comments and rewritten the field shape. The script
// applies by default (no flag) and writes in place; it does NOT commit. Same
// subprocess pattern as runLint/runDrift to avoid the standalone-install path
// escape (the script lives in scripts/, not lib/audit/). Returns { ran } so
// the --fix block can warn when the script is absent without aborting the fix.
function runBackfill(skillFile) {
  const backfillScriptCandidates = [
    path.join(__dirname, '..', '..', 'scripts', 'backfill-field-purpose-comments.js'),
    path.join(__dirname, 'backfill-field-purpose-comments.js'),
  ];
  const backfillScript = backfillScriptCandidates.find(candidate => fs.existsSync(candidate));
  if (!backfillScript) {
    return { ran: false, exitCode: null, stdout: '', stderr: 'backfill-field-purpose-comments.js not found on either candidate path' };
  }
  const result = spawnSync(
    process.execPath,
    [backfillScript, skillFile],
    {
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'pipe'],
...
// Map the drift sentinel's per-skill status to the four-verdict Audit Status
// truth_verdict enum (PASS | DRIFT | BROKEN | UNVERIFIED). Rollup defined in
// skill-audit-loop/SKILL_AUDIT_LOOP.md § The Inner Pipeline of `audit` — "OK → PASS, DRIFT →
// DRIFT, BROKEN → BROKEN, else UNVERIFIED". Accept legacy CLEAN for older
// drift outputs so historical artifacts remain readable.
// UNGROUNDED is not truth certification: it only says there was no local
// truth-source baseline to compare, so it rolls up as UNVERIFIED.
function mapDriftStatusToTruthVerdict(driftStatus) {
  switch (driftStatus) {
    ...
    case 'BROKEN':
      return 'BROKEN';
    default:
      // STALE, NO_BASELINE, EXTERNAL_UNHASHED, UNGROUNDED, NO_FRONTMATTER, and
      // any future states all roll up as UNVERIFIED — the audit-loop doc convention.
      return 'UNVERIFIED';
  }
}
...
function inferCategory(message) {
  ...
  if (/grounding|subject_matter|domain_object|truth_sources/.test(message)) return 'Grounding quality';
...
function inferFix(d) {
  ...
  if (/missing required field: (.+)/.test(d.message)) {
    ...
    return `Add the required \`${field}:\` field. See skill-metadata-protocol/field-reference.md for allowed values.`;
  }
  if (/unknown field: (.+)/.test(d.message)) {
    const field = d.message.match(/unknown field: (.+)/)[1];
    return `Remove or rename \`${field}\`. Check skill-metadata-protocol/field-reference.md for the canonical field list.`;
  }
  if (/not in enum/.test(d.message)) return 'Replace the value with one of the canonical enum values listed in skill-metadata-protocol/field-reference.md.';
...
function buildVerdictStub(skillName, diagnostics, isoDate) {
  ...
  if (errors.length === 0 && warnings.length === 0) {
    rationale = [
      'The skill passes the canonical-source schema lint gate: valid frontmatter, `schemas/SKILL_METADATA_PROTOCOL_schema.json` validation, valid identifier shape, non-empty description, and parent-directory/name alignment.',
      'This does not certify relation semantics, truth fidelity, routing behavior, or teaching usefulness.',
      'Run the remaining Integrity Gate checks and Behavior Gate evals before promoting Audit Status verdicts.',
    ].join(' ');
...
function main() {
  ...
  if (opts.errors.length > 0) {
    ...
    console.error('\nUsage: node scripts/skill-audit.js <skill-name> [--audit-root <path>] [--force] [--dry-run] [--fix] [--graded [--grader-cli <cmd>] [--grader-timeout <ms>]]');
    process.exit(1);
  }
  ...
  if (opts.dryRun) {
    ...
    if (opts.fix) {
      const mig = migrateFrontmatterToV8(fs.readFileSync(skillFile, 'utf8'));
      if (mig.changed) {
        console.log(`dry-run: --fix WOULD migrate (${mig.encoding}; scope=${mig.scopeVal} grounding_mode=${mig.groundingMode || '-'} -> deployment_target=${mig.deploymentTarget}) then regenerate field comments and re-lint.`);
      } else {
        console.log('dry-run: --fix no deterministic shape changes applicable (already v8-clean, or remaining lint errors are not shape issues).');
      }
    }
  ...
  if (opts.graded) {
    console.log(`Mode: GRADED audit on skills/${opts.skillName} — Integrity Gate (lint + drift) + qualitative grading of the seven scorecard dimensions. Stamps structural_verdict + truth_verdict + last_audited. The Behavior-Gate verdicts (comprehension_verdict / application_verdict) are NOT stamped here — run \`evaluate --mode comprehension|application\` for those.`);
  } else {
    console.log(`Mode: INTEGRITY-only audit on skills/${opts.skillName} (no --graded) — runs lint + drift, stamps structural_verdict + truth_verdict + last_audited. Qualitative scorecard dimensions are written as TODO placeholders for human review. Re-run with --graded to invoke the grader on those qualitative dimensions. (comprehension_verdict / application_verdict are stamped by \`evaluate --mode\`, not by audit.)`);
  }

  console.log(`Running skill-lint.js on skills/${opts.skillName}…`);
  let lintRun = runLint(skillDir);
  let diagnostics = normalizeLintDiagnostics(lintRun, parseLintOutput(lintRun.stderr), skillFile);
  let lintErrors   = diagnostics.filter(d => d.severity === 'error');
  let lintWarnings = diagnostics.filter(d => d.severity === 'warn');
  ...
  // --fix: deterministic Integrity-gate remediation (the repair catalog). When
  // lint found shape errors, apply the v7 -> v8 transform, regenerate field
  // comments, then re-lint so the verdicts stamped below reflect the fixed
  // state. Deterministic only — no LLM, no evals, no keep-or-revert. Errors
  // that are NOT deterministic-shape issues survive and keep structural_verdict
  // at FAIL for an `improve`/manual follow-up. See skill-audit-loop/SKILL_AUDIT_LOOP.md
  // § The Inner Pipeline of `audit` and docs/proposals/skill-audit-loop-fix-paths.md.
  if (opts.fix && lintErrors.length > 0) {
    const before = fs.readFileSync(skillFile, 'utf8');
    const mig = migrateFrontmatterToV8(before);
    if (!mig.changed) {
      console.log(`  --fix: no deterministic shape changes applicable (${mig.reason || 'remaining lint errors are not shape issues'}).`);
    } else {
      fs.writeFileSync(skillFile, mig.output, 'utf8');
      const backfill = runBackfill(skillFile);
      if (!backfill.ran) {
        console.warn('  --fix: backfill-field-purpose-comments.js not found — field comments not regenerated.');
      }
      console.log(`  --fix: migrated ${opts.skillName} (${mig.encoding}; scope=${mig.scopeVal} grounding_mode=${mig.groundingMode || '-'} -> deployment_target=${mig.deploymentTarget})${backfill.ran ? ' + regenerated field comments' : ''}. Re-linting…`);
      lintRun = runLint(skillDir);
      diagnostics = normalizeLintDiagnostics(lintRun, parseLintOutput(lintRun.stderr), skillFile);
      lintErrors = diagnostics.filter(d => d.severity === 'error');
      lintWarnings = diagnostics.filter(d => d.severity === 'warn');
      if (lintErrors.length === 0) {
        console.log(`  --fix: lint PASS${lintWarnings.length > 0 ? ` (${lintWarnings.length} warning(s))` : ''}.`);
      } else {
        console.log(`  --fix: lint still FAIL (${lintErrors.length} error(s)) — remaining errors are not deterministic-shape issues; route to improve/manual.`);
      }
    }
  }

  // Integrity Gate Phase 2 — truth via inline drift sentinel. Per
  // skill-audit-loop/SKILL_AUDIT_LOOP.md § "The Inner Pipeline of `audit`", the drift check
  // runs in the same pass as lint and feeds truth_verdict into Audit Status.
  // Without this call, truth_verdict would stay UNVERIFIED on every
  ...
  if (!opts.graded) {
    ...
    // Writes structural_verdict + lint_verdict + last_audited to Audit Status
    // (see below). The qualitative dimensions in findings.md remain
  ...
  // Stamp Integrity Gate verdicts back into the skill's own Audit Status. Until
  ...
  // (comprehension_verdict / application_verdict) are NOT written by audit at
  // all — they are stamped only by `evaluate --mode comprehension|application`.
  ...
  try {
    // ADR-0019: stamp the Integrity-gate verdicts into the audit-state.json
    // sidecar (sibling of SKILL.md), not the frontmatter.
    const writeRes = writeSidecarFields(skillFile, healthFields);
    if (writeRes.written) {
      console.log(`Stamped Audit Status sidecar on ${path.relative(_WORKSPACE, writeRes.path)}:`);
      ...
      console.log('  comprehension_verdict / application_verdict: (unchanged — stamped by `evaluate --mode comprehension|application`, not by audit)');
    } else {
      console.log(`Audit Status sidecar already current on ${path.relative(_WORKSPACE, writeRes.path)} — no fields changed.`);
    }
  } catch (err) {
    // Surface the failure loudly. Silent swallow would re-create the SAL-7 silent-fail pattern.
    console.error(`error: Audit Status sidecar write-back FAILED for ${skillFile}: ${err.message}`);
