 ...
 *                         caller commits per-skill. See SKILL_AUDIT_LOOP.md and
...
function inferFix(d) {
  ...
  if (/missing required field: (.+)/.test(d.message)) {
    ...
    return `Add the required \`${field}:\` field. See docs/SKILL_METADATA_PROTOCOL_field-reference.md for allowed values.`;
  }
  if (/unknown field: (.+)/.test(d.message)) {
    const field = d.message.match(/unknown field: (.+)/)[1];
    return `Remove or rename \`${field}\`. Check docs/SKILL_METADATA_PROTOCOL_field-reference.md for the canonical field list.`;
  }
  if (/not in enum/.test(d.message)) return 'Replace the value with one of the canonical enum values listed in docs/SKILL_METADATA_PROTOCOL_field-reference.md.';
...
function main() {
  ...
  // at FAIL for an `improve`/manual follow-up. See SKILL_AUDIT_LOOP.md
