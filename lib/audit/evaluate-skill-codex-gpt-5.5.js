 ...
 * the comprehension gate, forces the Codex backend for generator and grader
 * calls, enables tools-on execution for the generator, and marks the run as
 * single-model so PASS cannot be certified from same-family evidence.
 */
...
Confidence:
  ...
  provisional: it cannot earn comprehension PASS.
...
function buildCodexGpt55Invocation(argv, baseEnv = process.env, options = {}) {
  ...
  if (hasFlag(args, '--certifying')) {
    throw new Error('The Codex GPT-5.5 profile is single-family evidence. Do not pass --certifying; use a cross-family run for PASS.');
  }
