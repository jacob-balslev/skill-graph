...
function createPanelEnrichDeps(options = {}) {
  ...
  // "❌ Disabling the agents' tools or forbidding research"). The fence makes ONLY the public
  // roots (skill-graph + skills tree + audit-artifacts) visible to the model process; every
  // private sibling is kernel-DENIED — so advisory models research the repo + web exactly like
  // the frontier pair. This is the POLICY-equivalent isolated checkout (isolated-checkout.js
  // § "Why a policy and not a physical checkout"), reused — not a new physical checkout.
