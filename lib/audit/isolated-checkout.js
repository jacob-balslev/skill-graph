...
// ─── The FULL OS fence for audit/eval CLI dispatch (SH-6681) ─────────────────
//
...
const OS_FENCE_DENY_REASON =
  'The audit/eval private-content boundary is HARD — only the public skill-graph repo '
...
/**
 * Build the Seatbelt (SBPL) profile string for an audit/eval run.
 *
...
function buildSeatbeltProfile({ workspaceRoot, publicRoots, readOnlyRoots } = {}) {
  ...
  const lines = [
    '(version 1)',
    ';; SH-6681 OS fence — audit/eval private-content boundary (kernel-enforced).',
...
/**
 * Prepare an OS fence for an audit/eval run. Returns a handle whose `wrap(cli, args)`
...
function prepareOsFence({ workspaceRoot, publicRoots, readOnlyRoots, enabled = true, tmpDir, warn } = {}) {
  ...
  if (enabled && !supported) {
    log(
      '[audit] OS fence requested but unavailable on this platform '
  ...
  function ensureProfile() {
    ...
    writtenDir = fs.mkdtempSync(path.join(tmpDir || os.tmpdir(), 'audit-osfence-'));
 ...
 * SKILL_AUDIT_OS_FENCE env var. Default is ENABLED (the fence is the security
 * boundary; an opt-in-off fence does not close the gap). `SKILL_AUDIT_OS_FENCE=0`
...
function resolveOsFenceEnabled(explicit, env = process.env) {
  ...
  return env.SKILL_AUDIT_OS_FENCE !== '0';
}
