...
function buildOpencodeEnrichArgs(prompt, { modelId, agent = 'build', exploreDir } = {}) {
  ...
  // --pure = run WITHOUT external plugins. Advisory delivery is TEXT-OUTPUT now (the model returns
  // the enriched document as its reply, captured from stdout) — it must NOT drive a tool loop.
  // Nemotron (2026-06-05) otherwise burned the whole timeout on Exa web-search PLUGIN calls and
  // never emitted the document; --pure strips those external tool plugins so the model just answers.
  const args = ['run', '--pure', '--agent', agent];
