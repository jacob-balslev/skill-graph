...
// ─── Live production deps for the bidirectional ENRICH orchestrator ───────────
//
// run-bidirectional-enrich.js is pure orchestration (sequencing + anti-loss +
...
// claude/codex CLIs to actually claim slots, dispatch each frontier model's enrich
...
//   - Private-content boundary (HARD): the enrich pass prompt + claim filter fence
//     research to the public skill-graph + skills tree + web. See
//     docs/audit-loop-enrich-philosophy.md and memory skill-graph-private-content-boundary.
...
//
// SH-6687: per-model enrich PROPOSE slots claim an AUDIT op (each model independently
...
//
// Per the enrich-pass prompt: each model writes to
...
// ── Enrich-pass prompt assembly ───────────────────────────────────────────────
//
// Load the per-model enrich-pass runner prompt (the fenced block) and append the
...
function loadEnrichPromptTemplate(skillGraphRoot) {
  const file = path.join(skillGraphRoot, 'prompts', 'skill-audit-loop-enrich-pass.md');
...
function buildEnrichPrompt({ template, skill, skillDir, model, brief, skillBody, proposalPath, noveltyMemoPath }) {
  ...
  // enrich WITHOUT filesystem access to the sibling skills tree — and so the codex
  // sandbox never needs to widen to reach (and risk) the private workspace.
  if (skillBody && skillBody.trim()) {
    parts.push(
      '',
      'CURRENT CANONICAL SKILL.md (embedded — you do NOT need filesystem access to it; this IS the skill to enrich):',
      '```markdown',
      skillBody,
      '```',
    );
  ...
  parts.push(
    ...
    `  - Proposed enriched skill / changeset: ${proposalPath}`,
    `  - Novelty memo + dissent + completeness: ${noveltyMemoPath}`,
  );
  return parts.join('\n');
}

function buildCuratePrompt({ skill, proposals, currentSkillPath, mergedSkillPath, mergeLedgerPath, mergeProtocolRef }) {
  const proposalLines = proposals.map((p) => `  - ${p.model}: ${p.proposalPath}  (novelty: ${p.noveltyMemoPath})`).join('\n');
  return [
    'You are the CURATOR for a two-frontier ENRICH merge. Read the union-curate merge',
    `protocol: ${mergeProtocolRef}. The objective is ENRICH — the UNION of valuable`,
    'knowledge from BOTH proposals + the current skill. Anti-loss: every contribution is',
    ...
    'Proposals to union (read each, plus its novelty memo):',
    proposalLines,
    '',
    'WRITE EXACTLY:',
    `  - The enriched, unioned SKILL.md: ${mergedSkillPath}`,
...
//
// Enrich/curate dispatch needs WRITE-capable tools (the model writes the proposal /
...
// ── Advisory-tier arg builders (panel enrich) ─────────────────────────────────
//
...
// opencode CLI: `run --model <modelId> --agent build <prompt>`. The `build` agent is
// write-capable. NOTE: opencode run blocks on stdin with no TTY and explores a real
// --dir, so the live dispatch MUST use spawnSync with stdin ignored and point --dir at
// an empty dir OUTSIDE the repo (cwd stays the skill-graph root). See memory
// `opencode-run-from-node-stdin-and-empty-dir`. The --dir is appended by the caller
// (it owns the temp empty dir lifecycle); this builder returns the stable prefix.
function buildOpencodeEnrichArgs(prompt, { modelId, agent = 'build', exploreDir } = {}) {
  // CORRECT sandbox-respecting delivery (no --dangerously-skip-permissions — that disables
  // the agent's approval gate and is not authorized): run with --dir set to the artifact
  // run dir so WRITES into it are INTERNAL (no external_directory permission needed), and
  // give the model a prompt that needs NO external reads (skill body embedded inline). The
  // model writes the proposal to a RELATIVE filename in its working dir (--dir). Probe
  // 2026-06-05T11:01 showed the failure mode: the model tried to READ the external skills
  // tree (auto-rejected) and never wrote — so the advisory prompt must inline everything
  // and target a relative write path inside --dir.
  // Advisory models are full RESEARCHERS (tools ON) under the kernel Seatbelt fence — they research
  // repo + web, then EMIT the enriched SKILL.md as text (captured from stdout). Do NOT add --pure
  // here: it strips external plugins (incl. web search) and would demote advisory back to a
  // non-researcher, violating the research mandate (docs/audit-loop-enrich-philosophy.md line 35/97).
  // A model that web-search-loops past the timeout (Nemotron, free-tier throughput) is handled as a
  // best-effort NON-BLOCKING advisory failure (result.json), never by disabling research corpus-wide.
...
/**
 * Build the production `deps` object for runBidirectionalEnrich.
 *
 ...
 * @returns {object} deps for runBidirectionalEnrich.
 */
function createLiveEnrichDeps(options = {}) {
  const {
    skillGraphRoot,
    curatorModel = 'opus',
  ...
  if (!skillGraphRoot) throw new Error('createLiveEnrichDeps: skillGraphRoot is required.');
  ...
  const dispatchModel = ({ cli, args, cwd, env }) => {
    ...
    return dispatch({ cli: w.cli, args: w.args, cwd, env });
  };
  // Lazy: the enrich-pass template is only needed for LIVE dispatch. A dry-run (CI
  ...
  function runClaim(subcommandArgs, { model, agentId } = {}) {
    const id = agentId || `enrich-${sanitizeModelForFilename(model || 'slot')}`;
  ...
  function claimSlot({ skill, model }) {
    if (dryRun) {
      ...
      const runDirAbs = path.join(skillGraphRoot, '.opencode', 'progress', 'skill-audits', skill, 'runs', `dryrun--${sanitizeModelForFilename(model)}`);
  ...
  function researchAndPropose({ skill, skillDir, model, brief, artifactsDir }) {
    const runDirAbs = artifactsDir || path.join(skillGraphRoot, '.opencode', 'progress', 'skill-audits', skill, 'runs', sanitizeModelForFilename(model));
    // SH-6681: refuse any artifact/skill path outside the public scope before writing.
    fence(skillDir, `enrich skillDir for ${skill}`);
    fence(runDirAbs, `enrich run dir for ${skill}/${model}`);
    fs.mkdirSync(runDirAbs, { recursive: true });
    const { proposalPath, noveltyMemoPath } = proposalPaths(runDirAbs, skill, model);
    if (dryRun) {
      // Synthetic proposal so curate has inputs to union; never a real enrichment.
    ...
    // Embed the current SKILL.md so the model can enrich without cross-tree filesystem
    ...
    const canonicalSkillPath = fence(path.join(skillDir, 'SKILL.md'), `enrich read SKILL.md for ${skill}`);
    ...
    if (!fs.existsSync(proposalPath)) {
      throw new Error(`researchAndPropose(${model}): proposal not written at ${proposalPath} — the model did not produce the artifact. Dispatch log: ${dispatchLogPath}`);
    }
    return { proposalPath, noveltyMemoPath };
  }

  function curate({ skill, skillDir, proposals, currentSkillPath }) {
    ...
    const curatorAgentId = 'enrich-curator';
    let curatorRunDir;
    try {
    ...
    try {
      const prompt = buildCuratePrompt({ skill, skillDir, proposals, currentSkillPath, mergedSkillPath, mergeLedgerPath, mergeProtocolRef });
  ...
  // SH-6686: the guardrail must grade the ENRICHED skill, not the canonical
  // pre-enrich version. Build a temp skill dir whose SKILL.md is the curator's merged
  ...
  function prepareEnrichedEval({ skill, skillDir, mergedSkillPath }) {
    fence(skillDir, `enriched-eval source skillDir for ${skill}`);
    fence(mergedSkillPath, `enriched-eval merged source for ${skill}`);
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), `enriched-${sanitizeModelForFilename(skill)}-`));
    ...
    return { evalSkillDir, cleanup: () => { try { fs.rmSync(tmpRoot, { recursive: true, force: true }); } catch (_) { /* best-effort */ } } };
  }

  // SH-6686: KEEP decided — apply the enriched skill to the canonical working tree.
  ...
  // ENOENT and discarded a successful enrichment.) Absence of an eval is NEVER a
  ...
  return { buildResearchBrief, claimSlot, releaseSlot, researchAndPropose, curate, prepareEnrichedEval, applyMerge, evalArtifactExists };
}
...
// NOTE: `revert` (git-revert-HEAD) was removed in SH-6686. The enriched skill is now
