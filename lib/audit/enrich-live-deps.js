...
function createLiveEnrichDeps(options = {}) {
  ...
  function revert({ skill, reason }) {
    if (dryRun) return;
    // The enrich commit is HEAD (the curator's merged SKILL.md was committed before
    // the eval). Revert it; surface a clear message for the operator log.
    try {
      dispatch({ cli: 'git', args: ['revert', '--no-edit', 'HEAD'], cwd: skillGraphRoot });
    } catch (e) {
      throw new Error(`revert(${skill}): git revert failed (${e.message}). Reason for revert: ${reason}`);
    }
  }

  return { buildResearchBrief, claimSlot, releaseSlot, researchAndPropose, curate, revert };
}
