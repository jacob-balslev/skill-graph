...
function buildCodexEnrichArgs(prompt, { model, writableRoots } = {}) {
...
function createLiveEnrichDeps(options = {}) {
  ...
  function researchAndPropose({ skill, skillDir, model, brief, artifactsDir }) {
    ...
    const args = cli === 'codex'
      ? buildCodexEnrichArgs(prompt, { model: undefined, writableRoots: [runDirAbs] })
      : buildClaudeEnrichArgs(prompt, { model });
    dispatchModel({ cli, args, cwd: skillGraphRoot });
    if (!fs.existsSync(proposalPath)) {
      throw new Error(`researchAndPropose(${model}): proposal not written at ${proposalPath} — the model did not produce the artifact.`);
    }
    return { proposalPath, noveltyMemoPath };
  }
  ...
  function curate({ skill, skillDir, proposals, currentSkillPath }) {
    ...
    try {
      ...
      const args = cli === 'codex'
        ? buildCodexEnrichArgs(prompt, { model: undefined, writableRoots: [curatorRunDir] })
  ...
  return { buildResearchBrief, claimSlot, releaseSlot, researchAndPropose, curate, prepareEnrichedEval, applyMerge };
}
