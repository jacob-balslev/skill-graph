...
function buildClaimArgs({ skill, model, op = 'merge', lane }) {
...
function buildEnrichPrompt({ template, skill, skillDir, model, brief, proposalPath, noveltyMemoPath }) {
  return [
    template,
    '',
    '─── THIS RUN ───',
...
function buildCodexEnrichArgs(prompt, { model } = {}) {
...
function createLiveEnrichDeps(options = {}) {
  ...
  // per model (a per-model audit slot, per the two-frontier design).
  function runClaim(subcommandArgs, model) {
    const env = { ...process.env, MODEL: model || '', AGENT_ID: process.env.AGENT_ID || `enrich-${sanitizeModelForFilename(model)}` };
  ...
  function claimSlot({ skill, model }) {
    ...
    const out = runClaim(buildClaimArgs({ skill, model, op: 'merge' }), model);
  ...
  function researchAndPropose({ skill, skillDir, model, brief, artifactsDir }) {
    ...
    const prompt = buildEnrichPrompt({ template: getEnrichTemplate(), skill, skillDir, model, brief, proposalPath, noveltyMemoPath });
    const cli = cliForModel(model);
    const args = cli === 'codex'
      ? buildCodexEnrichArgs(prompt, { model: undefined })
  ...
  function curate({ skill, skillDir, proposals, currentSkillPath }) {
    ...
    // SH-6681: the curator writes the enriched SKILL.md + ledger — fence both targets
    // and the canonical skill path it reads.
