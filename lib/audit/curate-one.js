...
//   [{ model, tier, proposalPath, noveltyMemoPath, iterationSuggestionsPath? }].
//   STRICT anti-loss applies to --proposals; --advisory is discretionary (the frontier
//   curator decides; advisory never auto-merges).
//
...
function main(argv) {
  ...
  const finish = (obj, code) => {
    // `_ledgerContributions` is an internal hand-off to the heartbeat only — strip it from the
    // authoritative result.curate.json so the result file shape is unchanged.
    const { _ledgerContributions, ...rest } = obj;
    const out = { skill, curator: curatorModel, ts: new Date().toISOString(), ...rest };
    try { fs.writeFileSync(resultPath, `${JSON.stringify(out, null, 2)}\n`); } catch (_) { /* best-effort */ }
    if (statusFile) {
      // On success, surface the curated merge-ledger contributions into the shared heartbeat so the
      // findings-review viewer sees in-session findings live (mirrors the monolithic runner's emit).
      const ledgerFindings = obj.ok && Array.isArray(_ledgerContributions) ? _ledgerContributions : undefined;
      updateCellStatus(statusFile, { skill, model: curatorModel, tier: 'mandatory', phase: 'curate', state: obj.ok ? 'done' : 'failed', findings: ledgerFindings });
    }
  ...
  finish({
    ...
    _ledgerContributions: Array.isArray(merge.mergeLedger.contributions) ? merge.mergeLedger.contributions : undefined,
  }, ok ? 0 : 1);
}
