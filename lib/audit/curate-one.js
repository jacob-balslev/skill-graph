...
// own panel row. REUSES the tested live deps (`panel-enrich-live-deps.js` → `curate`, which
...
function main(argv) {
  ...
  const mandatoryModels = String(args['mandatory-models'] || 'opus,codex-current')
  ...
  const { validateAntiLoss } = require('./run-bidirectional-enrich');
  const { validateMandatoryCoverage } = require('./run-panel-enrich');
  const deps = createPanelEnrichDeps({ skillGraphRoot: cwd, dryRun: Boolean(args['dry-run']), curatorModel });
  ...
  // SAME guards as run-panel-enrich.js Phase 3 — a violation is a hard failure (the curator
