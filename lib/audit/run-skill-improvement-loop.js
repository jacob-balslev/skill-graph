...
function runImprover(prompt, { workspace, generatorModel, apply }) {
  ...
  // The improver WRITES files; the `codex-current` grader role (cli 'codex',
  // model omitted) is read-only by design and is not a valid generator. Refuse
  // clearly rather than build a broken `-m null` invocation.
  if (cli === 'codex' || model == null) {
    throw new Error(
      `runImprover: '${generatorModel}' is not a valid generator model. ` +
      `Use a generator role/alias such as 'representative-generator', 'sonnet', or 'opus'.`,
    );
  }
  if (cli === 'claude') {
