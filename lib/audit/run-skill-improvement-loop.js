...
// CLI loop that iterates skill improvement candidates through eval gates and
// auto-improvement registry bookkeeping.
...
// Infra-error classification is owned by panel-budget.js (single owner shared with the
// multi-model panel runner) so the TRANSIENT/STRUCTURAL/UNAVAILABLE buckets never drift.
const { INFRA_ERROR_CLASS, classifyInfraError } = require('./panel-budget');
...
function collectSkillDirs(skillsRoot) {
  ...
  // SKILL.md) and skips metadata/control dirs.
  try {
...
// Understanding fields — the flat frontmatter fields that describe what
...
function resolveImprovementEvalFile(skillDir) {
  const evalsDir = path.join(skillDir, 'evals');
  const candidates = [
    { path: path.join(evalsDir, 'evals.json'), kind: 'legacy' },
    { path: path.join(evalsDir, 'comprehension.json'), kind: 'comprehension' },
  ];
  ...
  const applicationPath = path.join(evalsDir, 'application.json');
  if (fs.existsSync(applicationPath)) {
    return {
      path: applicationPath,
      kind: 'application',
      unsupportedByLegacyGate: true,
    };
  }
...
function renderPrompt({
  ...
  const fieldScopeBlock = fieldScope && UNDERSTANDING_FIELDS.includes(fieldScope)
    ? [
        ...
        '- Do NOT modify the selected eval artifact or evals/eval-set.json.',
  ...
  const researchChecklist = researchFirst
    ? [
        ...
        '- Read skill-metadata-protocol/SKILL_METADATA_PROTOCOL.md and examples/skill-metadata-template.md (the authoritative skill contract and current template). The output MUST conform to those surfaces.',
  ...
  return [
    ...
    '- Apply the current skill contract (skill-metadata-protocol/SKILL_METADATA_PROTOCOL.md + examples/skill-metadata-template.md):',
    '  - Core frontmatter MUST include: name, description, subject (closed 12-enum browse shelf), public (boolean publishability/private-data gate), scope (free-text PRD-style statement), and the other current template fields that match the skill. The audit-state sidecar owns schema_version and audit/eval verdict state.',
    ...
    '  - For project-anchored skills, non-empty project[] belonging-entity references require grounding.subject_matter and a populated grounding block.',
    '  - Do NOT author retired fields: type, category, categories, secondary_categories, primaryCategory, layerPrimary, routingRole, family, layer, archetype, operation, deployment_target, eval_status, workspace_tags. These were removed or replaced in the v7 -> v8 clean cut and fail current authoring validation; deployment_target is accepted only as a normalizer back-compat alias for unmigrated skills.',
    ...
    '  - Follow the uniform body contract. Do NOT force retired capability/workflow/router/overlay archetype headings.',
    '  - Description: routing contract in the form "Use when ... Covers ... Do NOT use ..." with clear positive triggers and negative bounds, <=300 words.',
    '',
    'Editing rules:',
    ...
    '- Improve the selected eval artifact so it targets real failure modes, anti-goals, and grounded expectations.',
...
function runImprover(prompt, { workspace, generatorModel, apply }) {
  ...
  // The improver WRITES files; the `gpt-5.5` model (cli 'codex', model omitted)
  // is dispatched read-only here and is not a valid generator on this route. Refuse
  // clearly rather than build a broken `-m null` invocation.
  if (cli === 'codex' || model == null) {
    throw new Error(
      `runImprover: '${generatorModel}' is not a valid generator model on this route. ` +
      `Use a frontier authoring alias such as 'opus'.`,
    );
  }
  if (cli === 'claude') {
  ...
  try {
    ...
    if (errorClass === INFRA_ERROR_CLASS.STRUCTURAL) {
      console.warn(`  ⚠ OpenCode structural failure (${error.message.slice(0, 80)}). Falling back to Claude Opus.`);
      const fallbackArgs = ['-p', prompt, '--model', 'opus', '--output-format', 'text'];
      if (apply) {
...
`);
  ...
  const generatorModel = args.generator || 'opus'; // frontier authoring model (no-lesser-models); Sonnet removed from the loop
  ...
  const filteredSkills = discoveredSkills
    .filter((skillDir) => {
      if (includeFilters.length === 0) return true;
      return includeFilters.some((filter) => skillDir.includes(filter));
    });
  // SKI-263: announce the cap when it actually truncates, so a partial run is
  // never silent (mirrors the batch-eval.js guard). Number.POSITIVE_INFINITY
  // (no --max-skills) never trips this.
  if (filteredSkills.length > maxSkills) {
    console.log(`[cap] Limiting to ${maxSkills} of ${filteredSkills.length} matched skill(s) (--max-skills)`);
  }
  const selectedSkills = filteredSkills.slice(0, maxSkills);
  ...
  for (const skillDir of selectedSkills) {
    ...
    const evalArtifact = resolveImprovementEvalFile(skillDir);
    const evalFile = evalArtifact && evalArtifact.path;
    ...
    if (!evalArtifact) {
      ...
      console.log(`Skipping ${skillName}: missing evals/evals.json or evals/comprehension.json`);
      continue;
    }

    if (evalArtifact.unsupportedByLegacyGate) {
      summary.skills.push({ skillName, status: STATUS.SKIPPED_NO_EVALS, reason: 'application-only eval artifact is not supported by the legacy keep/revert gate' });
      console.log(`Skipping ${skillName}: found evals/application.json, but improve currently needs evals/evals.json or evals/comprehension.json for its keep/revert gate`);
      continue;
    }
    ...
    try {
      ...
      if (auditTier === 'light') {
        const lightPrompt = [
          ...
          '2. Check the skill\'s relations (related/adjacent, suppresses/boundary) are still accurate.',
      ...
      if (apply) {
        ...
        if (assetMetadata.pilotTrack === 'template-owned-meta' && beforeEval === afterEval && beforeEvalSet === afterEvalSet) {
          gate.accepted = false;
          gate.reasons.push('candidate did not improve eval artifacts (selected eval artifact or `eval-set.json`) for the template-owned meta pilot');
        }
