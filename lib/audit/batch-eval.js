...
const EVAL_HISTORY = path.join(REPO_ROOT, 'agent-orchestration', 'logs', 'eval-history.jsonl');
const REPORT_DIR = path.join(REPO_ROOT, '.opencode', 'progress', 'skills', 'batch-eval');
const EVALUATE_SCRIPT = path.join(__dirname, 'evaluate-skill.js');

const SKILL_ROOTS = [
  { root: path.join(REPO_ROOT, 'skills'), scope: 'shared' },
  { root: path.join(REPO_ROOT, 'skills', 'sales-hub'), scope: 'sales-hub' },
];

// Legacy path kept as fallback — will be removed in a future version
const LEGACY_SH_SKILL_ROOT = path.join(REPO_ROOT, 'sales-hub', '.claude', 'skills');
if (fs.existsSync(LEGACY_SH_SKILL_ROOT) && !fs.existsSync(path.join(REPO_ROOT, 'skills', 'sales-hub'))) {
  console.warn('WARNING: Using legacy Sales Hub skill path (sales-hub/.claude/skills/). Migrate to skills/sales-hub/.');
  SKILL_ROOTS.push({ root: LEGACY_SH_SKILL_ROOT, scope: 'sales-hub' });
}
...
async function main() {
  ...
  const results = await runWithConcurrency(skills, concurrency, async (skill, index) => {
    ...
    return runSkillEval(skill, options);
  });
