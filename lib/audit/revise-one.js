...
function main(argv) {
  ...
  const finish = (obj, code) => {
    ...
    if (statusFile) updateCellStatus(statusFile, { skill, model, tier, phase: `revise-r${round}`, state: obj.ok ? 'done' : 'failed' });
    console.log(JSON.stringify(out));
    process.exit(code);
  };
  if (statusFile) updateCellStatus(statusFile, { skill, model, tier, phase: `revise-r${round}`, state: 'running' });
