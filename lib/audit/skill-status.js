...
//
// The single `audit_verdict` field is replaced by discrete verdicts (one per
// audit-pipeline layer): structural (Integrity), truth (Integrity), and
// comprehension (Behavior Gate). `lint_verdict` and `drift_status` remain
// unchanged — they are per-script signals the audit loop rolls up into
// `structural_verdict` and `truth_verdict` respectively. `audit_verdict` is
// retained in the display fallback ONLY for back-compat reads of older skills
// that have not yet been migrated; it is sorted last so current skills lead
// with the live verdicts.
...
const HEALTH_BLOCK_FIELDS = [
  ...
  'drift_status',
  'freshness',
  // Back-compat: present only on older SKILL.md frontmatter that has not been
  // migrated to the discrete verdicts yet. Current skills omit it.
  'audit_verdict',
];
...
function classifyAuditState(healthBlock, skillContract = {}) {
  ...
  // Assessment: derived from comprehension_verdict (the Behavior-Gate quality signal).
  //   UNVERIFIED / NA / missing → unassessed
  ...
  let assessment;
  if (!cv || cv === 'UNVERIFIED' || cv === 'NA') assessment = 'unassessed';
  else if (cv === 'PROVISIONAL') assessment = 'assessed_provisional';
  else assessment = 'assessed_graded';

  // Certification outcome: only meaningful when assessed. PASS is the certifying
  // outcome; SHALLOW / REDUNDANT are honest negative grader signals.
  let certification;
  if (assessment === 'unassessed') certification = 'pending — eligible only';
  else certification = cv;
  ...
  // v8: project-anchoring is carried by `project[]` (the retired `deployment_target`
  // enum was replaced by the boolean `public` publishability gate). Derive conceptScope
  // from project[] presence first, then `public`, with the retired enum kept as a
  // back-compat fallback for unmigrated skills.
  let conceptScope;
  if (cv === 'SKIPPED_BASELINE_HIGH') conceptScope = 'framework_concept';
  else if (cv === 'NA') conceptScope = 'no_comprehension_layer';
  else if (Array.isArray(skillContract.project) && skillContract.project.length > 0) conceptScope = 'project';
  else if (skillContract.public === true) conceptScope = 'portable';
  else if (skillContract.deployment_target === 'portable') conceptScope = 'portable'; // back-compat (retired enum)
  else if (skillContract.deployment_target === 'project') conceptScope = 'project';   // back-compat (retired enum)
...
/**
 * Detect "verdict present, artifact absent" (A4) — a graded comprehension
