...
const { collectSkillFiles, loadWorkspaceConfig } = require('../../scripts/lib/roots');
...
function extractHealthBlock(frontmatter) {
  // Health Block may live under audit_state, metadata, or top-level fields.
  const blocks = [frontmatter.audit_state, frontmatter.metadata, frontmatter].filter(Boolean);
...
function classifyAuditState(healthBlock) {
...
function formatTable(skillName, healthBlock) {
  ...
  const auditState = classifyAuditState(healthBlock);
...
module.exports = { extractHealthBlock, classifyAuditState, findSkillPath, findRepoRoot };
