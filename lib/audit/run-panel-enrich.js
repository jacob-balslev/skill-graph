...
function main(argv) {
  ...
  if (!skill || !skillDir) {
    console.error('Usage: node lib/audit/run-panel-enrich.js --skill <slug> --skill-dir <dir> [--cwd <skill-graph-root>] [--eval-mode application|comprehension] [--prior-verdict <V>] [--max-rounds N] [--no-advisory] [--dry-run] [--no-eval] [--status-file <path>] [--no-tui] [--quiet]');
    process.exit(1);
  }
  ...
  try {
    ...
    process.stderr.write(`panel-enrich FAILED: ${err.message}\n`);
    process.exit(1);
  }
  reporter.teardown();
