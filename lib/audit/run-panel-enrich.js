 ...
 */
function runConvergence({ skill, skillDir, proposals, deps, policy }) {
  const history = [];
...
function runPanelEnrich(opts = {}) {
  ...
  for (const model of advisoryRequested) {
    ...
    if (slot && slot.ok === false) { advisoryFailures.push({ model, phase: 'claim', error: slot.error }); continue; }
    let p;
  ...
  // ── Phase 2 — cross-review, iterate to convergence ──
  const convergenceState = runConvergence({ skill, skillDir, proposals, deps, policy });
