 ...
 * @param {object} cell        { skill, model, tier, phase, state, findings } — state is
 *                             'running' | 'done' | 'failed'. `findings` (optional) is the
 *                             merge-ledger contributions array; when supplied it is merged into
 *                             status.findings[] so the findings-review viewer sees in-session
 *                             findings live (e.g. curate-one.js passing the curated ledger).
 */
function updateCellStatus(statusFile, { skill, model, tier, phase, state, findings } = {}) {
  ...
  try {
    ...
    if (Array.isArray(findings)) status.findings = findings;
 ...
 */
function writeRunnerHeartbeat(statusFile, { skill, phase, total, done, failed, complete, agents, findings, findings_review } = {}) {
  ...
  try {
    ...
    if (Array.isArray(findings)) status.findings = findings;
    if (findings_review && typeof findings_review === 'object') status.findings_review = findings_review;
    const tmp = `${statusFile}.tmp-${process.pid}`;
    fs.mkdirSync(path.dirname(statusFile), { recursive: true });
    fs.writeFileSync(tmp, `${JSON.stringify(status, null, 2)}\n`);
    fs.renameSync(tmp, statusFile);
