...
function toSidecarReceipt(r) {
  ...
  // SKI-356: a receipt persisted into a (publishable) skill sidecar must never
  // carry an absolute home-dir path, and must declare whether the ledger it
  // references actually ships with the public skill. Relativize the ledger path
  // to the workspace root and classify its visibility — the in-sidecar receipt
  // summary below is the portable evidence when the ledger is monorepo-private.
  const { relativizeReceiptPath, receiptVisibilityForPath } = require('./log-paths');
  const rawLedgerRef = r.merge_ledger_ref === undefined ? null : r.merge_ledger_ref;
  const portableLedgerRef = rawLedgerRef ? relativizeReceiptPath(rawLedgerRef) : null;
  const receipt = {
    ...
    merge_ledger_ref: portableLedgerRef,
    receipt_visibility: receiptVisibilityForPath(rawLedgerRef),
    directions,
  };
