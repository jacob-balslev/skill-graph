...
function runBidirectionalEval(opts = {}) {
  ...
  // canonical FRONTIER_PAIR [opus, codex-current], the first direction's generator
  ...
  const common = { mode, skillDir, evalFile, workspace, baselineWorkspace: baselineWorkspace || executionProfile.cwd, executionProfile };
  ...
  // for codex-current until codex's resolved model is captured from its output
  ...
  // cross-family certifying AND both directions' models are resolved. Otherwise a
  // strong verdict is capped to PROVISIONAL — the in-code "never certify on an
  // invalid / biased / unprovable run".
  const certifyingClean = Boolean(parity.parity_ok) && bothCertifying && resolvedClean;
  ...
  const advisoryPanel = wantAdvisory
    ? runAdvisoryPanel({ mode, skillDir, evalFile, workspace, baselineWorkspace: baselineWorkspace || executionProfile.cwd, executionProfile, deps })
    : null;
  ...
  if (!certifyingClean && CERTIFYING_VERDICTS.has(synthesizedVerdict)) {
    ...
    capReason = !parity.parity_ok
      ...
      : !bothCertifying
        ? `a direction was not cross-family certifying (Claude=${certClaude.tier}, Codex=${certCodex.tier})`
        : `a direction's model is unresolved (${SENTINEL}) — cannot prove which concrete model ran`;
  }
...
function toSidecarReceipt(r) {
  ...
  const receipt = {
    ...
    directions: [mapDir(r.direction_claude, 'Claude'), mapDir(r.direction_codex, 'Codex')],
  };
  if (ep) {
...
function main(argv) {
  ...
  if (!skillDir && !args['eval-file']) {
    console.error('Usage: node lib/audit/run-bidirectional-eval.js --mode comprehension|application --skill-dir <dir> [--eval-file <f>] [--cwd <skill-graph-root>] [--skill <slug>] [--advisory]');
    process.exit(1);
  }
