...
function runBidirectionalEval(opts = {}) {
  ...
  const resolvedClean = !dirUnresolved(dirA) && !dirUnresolved(dirB);
 ...
 * native return carries extra working fields (direction_a/direction_b/parity object,
 * certification sub-objects); this projects ONLY the schema-allowed keys so a
...
function toSidecarReceipt(r) {
  ...
  const receipt = {
    ...
    directions: [mapDir(r.direction_a, 'A'), mapDir(r.direction_b, 'B')],
  };
  if (ep) {
...
// Runs the bidirectional eval guardrail end-to-end (Direction A + B, tools-ON
...
function main(argv) {
  ...
  if (!skillDir && !args['eval-file']) {
    console.error('Usage: node lib/audit/run-bidirectional-eval.js --mode comprehension|application --skill-dir <dir> [--eval-file <f>] [--cwd <skill-graph-root>] [--skill <slug>]');
    process.exit(1);
  }
