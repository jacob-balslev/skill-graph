...
function runBidirectionalEval(opts = {}) {
  ...
  if (!certifyingClean && CERTIFYING_VERDICTS.has(synthesizedVerdict)) {
    ...
    if (!parity.parity_ok) {
      ...
      capReason = 'a direction\'s resolved model family differs from its declared family — the cross-family certifying decision was computed from a stale/incorrect declared family';
