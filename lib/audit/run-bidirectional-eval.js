 ...
 * @param {Set<number>} [opts.caseIds]  Optional application-case filter (passed through to runDirection).
 * @param {number} [opts.trials]        Optional application trial count (passed through to runDirection).
...
function runBidirectionalEval(opts = {}) {
  const {
    ...
    // A10: held-out slicing for application mode. 'certification' grades the held-out slice
  ...
  const common = {
    ...
    caseIds,
    trials,
    slice, // A10: threaded to runEvalDirection → runApplicationEval (application mode only)
  };
  ...
  // certifying AND both directions' models are resolved. Application
  ...
  // direction now measures the SAME deployment-representative generator, judged by a
  // different frontier. Applicability is therefore for the representative population,
  // not for an Anthropic/OpenAI generator family.
  ...
  const applicableFor = (certifyingClean
    && dirClaude && dirClaude.verdict === certVerdict
    && dirCodex && dirCodex.verdict === certVerdict)
    ? 'representative'
    : 'neither';
...
// ─── CLI ──────────────────────────────────────────────────────────────────────
// node lib/audit/run-bidirectional-eval.js --mode comprehension|application --skill-dir <dir>
...
function main(argv) {
  ...
  if (!skillDir && !args['eval-file']) {
    console.error('Usage: node lib/audit/run-bidirectional-eval.js --mode comprehension|application --skill-dir <dir> [--eval-file <f>] [--cwd <skill-graph-root>] [--skill <slug>] [--case-id 1,2] [--trials N] [--advisory] [--calibration-receipt FILE]');
    process.exit(1);
  }
