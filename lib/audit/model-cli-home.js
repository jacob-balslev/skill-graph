...
function prepareModelCliHome(options = {}) {
  ...
  const envByCli = {
    // Claude Code OAuth/keychain state is bound to the real home. A copied ~/.claude.json is
    // metadata only; with HOME pointed at scratch, `claude -p` reports "Not logged in".
    claude: {},
    codex: env,
    gemini: env,
    opencode: env,
  ...
  const copied = {
    ...
    claudeHome: copyListedFiles({ srcDir: realHome, destDir: root, entries: CLAUDE_HOME_FILES, fsImpl }),
...
function envForCli(cli, baseEnv = process.env, modelEnv = {}) {
  if (modelEnv && modelEnv.byCli) {
    const cliEnv = Object.prototype.hasOwnProperty.call(modelEnv.byCli, cli)
      ? modelEnv.byCli[cli]
      : (modelEnv.default || {});
    return mergeModelEnv(baseEnv, cliEnv || {});
  }
  return mergeModelEnv(baseEnv, modelEnv || {});
}
