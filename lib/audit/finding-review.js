...
const DISPOSITIONS = new Set(['to-file']);
...
function normalizeDisposition(value) {
  ...
  if (!raw) return null;
  const lower = raw.toLowerCase();
  if (lower === 'file' || lower === 'filed' || lower === 'tofile' || lower === 'to-file') return 'to-file';
  return DISPOSITIONS.has(lower) ? lower : null;
}
...
function applyFindingDecision(state, findingId, decision, nowIso = new Date().toISOString(), note = undefined) {
  ...
  const existingDisposition = normalizeDisposition(existing.disposition);
  if (existingDisposition) {
    record.disposition = existingDisposition;
    if (existing.disposition_at) record.disposition_at = existing.disposition_at;
  }
...
function applyFindingDisposition(state, findingId, disposition, nowIso = new Date().toISOString(), note = undefined) {
  if (!findingId) return normalizeReviewState(state);
  const nextDisposition = normalizeDisposition(disposition);
  const next = normalizeReviewState(state);
  next.updated_at = nowIso;
  const existing = next.decisions[findingId] || {};
  const record = {
    ...
    disposition: nextDisposition || 'to-file',
...
function decisionRecord(finding, reviewState = {}) {
  ...
  return {
    ...
    disposition: normalizeDisposition(stored.disposition) || null,
    disposition_at: oneLine(stored.disposition_at, 80) || null,
