...
// A8: every pairwise trial is graded in BOTH presentation orders and required to agree
// (position-bias mitigation per graders/references/rubric-best-practices.md). One pairwise
// trial therefore costs this many grader calls.
const APPLICATION_PAIRWISE_ORDERS = 2;
...
// generator call. The generator must run on an explicit measured-subject model. By
// default it uses the representative-generator role; the bidirectional runner keeps
// that generator fixed and varies the frontier judges. This is NOT a no-lesser-models
// violation because that rule governs the GRADER, which remains top-tier.
const APPLICATION_GENERATOR_DEFAULT_MODEL = REPRESENTATIVE_GENERATOR_MODEL;
...
// A1l: a CERTIFYING run (the one that can stamp APPLICABLE) defaults to the higher end
// of the field-standard range so the verdict rests on ≥5 trials, not a 2-of-3 majority.
// Dev/improve-loop runs keep the cheaper 3-trial default for speed.
const APPLICATION_CERTIFYING_DEFAULT_TRIALS = 5;
...
// run. The audit-loop path uses a representative generator judged by both
// frontier judges; single-perspective or undeclared runs are structurally only
// PROVISIONAL. This is the in-code form of
  ...
  for (let attempt = 1; attempt <= APPLICATION_GRADER_MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      raw = runGraderPrompt(prompt, {
        ...
        telemetryTrialIndex: options.trialIndex,
      });
    } catch (callError) {
...
/**
 * A8: position-bias-neutralized pairwise grade — runs BOTH presentation orders of the
 * SAME trial and requires agreement, per `graders/references/rubric-best-practices.md`
 * § "Pairwise Judging: Neutralize Position Bias" (option 1, the strongest mitigation,
 * the one fusion-judge uses). The judge is strongly biased toward whichever response it
 * sees SECOND (win-rate flipped 2.5%→82.5% on reorder; EMNLP 2025 / SKI-49 Opus judge),
 * so the prior per-trial order ALTERNATION (`(id+trialIndex)%2`) only averaged the bias
 * ACROSS trials — a single trial's pairwise verdict could still be pure position artifact.
 *
 * Runs the grader with order ['baseline','with_skill'] AND ['with_skill','baseline'].
 * When the two orders agree on `preferred_run`, that is the clean winner (confidence =
 * the conservative MIN of the two). When they DISAGREE, the comparison is position-biased:
 * `preferred_run` → 'tie' and `confidence` → 0, which makes computeApplicationPerCaseVerdict
 * fall through to the independent absolute-axis path (split credit / no pairwise force) —
 * exactly the rubric's "otherwise split credit / call it a tie".
 *
 * @returns the normalized pairwise grade (same shape gradeApplicationPairwise returns)
 *          plus { both_orders:true, order_agreement:boolean, position_bias:boolean,
 *          orders:[gradeAB, gradeBA] }.
 */
function gradeApplicationPairwiseBothOrders(testCase, responses, options) {
  const gradeAB = gradeApplicationPairwise(testCase, responses, { ...options, order: ['baseline', 'with_skill'] });
  const gradeBA = gradeApplicationPairwise(testCase, responses, { ...options, order: ['with_skill', 'baseline'] });
  const agree = gradeAB.preferred_run === gradeBA.preferred_run;

  if (agree) {
    return {
      ...gradeAB,
      confidence: Math.min(gradeAB.confidence, gradeBA.confidence),
      both_orders: true,
  ...
  // Disagreement = position bias. Drop the pairwise signal (tie + confidence 0) so the
  // per-case verdict rests on the independent absolute axis scores, not a biased pick.
  return {
    ...gradeAB,
    preferred: 'tie',
    preferred_run: 'tie',
    ...
    comparative_reasoning:
      `position-bias tie: A-then-B preferred ${gradeAB.preferred_run}, B-then-A preferred ${gradeBA.preferred_run} — orders disagree, pairwise signal dropped`,
    both_orders: true,
...
// ─── Held-out certification slice (A10) ────────────────────────────────────────

// Fraction of cases reserved for the held-out CERTIFICATION slice — the rest form the
// ITERATION slice the improve/evolve loop optimizes against. A certifying verdict earned
// on the certification slice was NOT seen during iteration, so it cannot be Goodharted by
// the loop overfitting to the eval (the loop tuned the skill against the iteration slice only).
const APPLICATION_HOLDOUT_FRACTION = 1 / 3;

/**
 * A10: deterministically split application cases into an ITERATION slice (the
 * improve/evolve loop optimizes against this) and a held-out CERTIFICATION slice
 * (never seen during iteration; certifies without Goodhart overfitting).
 *
 * The split is DETERMINISTIC and STABLE (no Math.random — banned in these scripts and
 * would break resume): cases are ordered by id, then every Nth case (N = round(1/fraction),
 * default every 3rd) is held out. Interleaving by position keeps both real and red-herring
 * cases distributed across the two slices rather than clustering them. With too few cases to
 * fill both slices the certification slice may be empty — the caller decides whether that is
 * acceptable (a tiny suite simply cannot offer an honest held-out signal).
 *
 * @param {object[]} cases               The full case list (each with a numeric `id`).
 * @param {object}   [opts]
 * @param {number}   [opts.holdoutFraction]  Fraction reserved for certification (default 1/3).
 * @returns {{ iteration: object[], certification: object[] }}
 */
function splitApplicationCases(cases, { holdoutFraction = APPLICATION_HOLDOUT_FRACTION } = {}) {
  const list = Array.isArray(cases) ? cases.slice() : [];
  if (list.length === 0) return { iteration: [], certification: [] };
  // Stable order by id so the split is reproducible across runs.
  list.sort((a, b) => Number(a.id) - Number(b.id));
  const stride = Math.max(2, Math.round(1 / holdoutFraction)); // every `stride`-th case held out
  const iteration = [];
  const certification = [];
  list.forEach((testCase, index) => {
    if ((index + 1) % stride === 0) certification.push(testCase);
    else iteration.push(testCase);
  });
  return { iteration, certification };
}

const APPLICATION_EVAL_SLICES = new Set(['all', 'iteration', 'certification']);
 ...
 * @param {string}   [options.slice]      - A10 held-out slicing: 'all' (default, every case),
 *                  'iteration' (the slice the improve/evolve loop optimizes against), or
 *                  'certification' (the held-out slice — a certifying verdict here was NOT seen
 *                  during iteration, so it cannot be Goodharted). The chosen slice is recorded as
 *                  `eval_slice` on the summary. Applied AFTER caseIds (split of the filtered set).
 ...
 * @param {boolean}  [options.certifying] - Operator attests this is a certifying
 *                  configuration. ONLY a certifying run (asserted + representative generator
 *                  with a top-tier judge, or legacy cross-family declared families) can stamp
 *                  APPLICABLE; every other shape caps at PROVISIONAL.
...
function runApplicationEval(evalFile, options = {}) {
  ...
  if (!Array.isArray(data.cases)) {
    if (Array.isArray(data.evals)) {
      throw new Error(
        `Eval file ${evalFile} has 'evals[]' but no 'cases[]'. This looks like Comprehension — use --comprehension instead of --application.`,
      );
    }
  ...
  const filteredCases = caseIds
    ? allCases.filter((testCase) => caseIds.has(Number(testCase.id)))
    : allCases;

  // A10: held-out slicing. 'all' (default) keeps every filtered case; 'iteration' and
  // 'certification' take the deterministic split so a certifying verdict can be earned on
  // cases the improve/evolve loop never optimized against. Recorded as eval_slice.
  const evalSlice = APPLICATION_EVAL_SLICES.has(options.slice) ? options.slice : 'all';
  const cases = evalSlice === 'all'
    ? filteredCases
    : splitApplicationCases(filteredCases)[evalSlice];
  const realCaseCount = cases.filter((c) => !c.red_herring).length;
  const redHerringCaseCount = cases.filter((c) => c.red_herring).length;

  if (caseIds && filteredCases.length === 0) {
    throw new Error(`No application cases matched requested ids: ${Array.from(caseIds).join(', ')}`);
  }
  if (evalSlice !== 'all' && cases.length === 0) {
    throw new Error(`Application eval slice '${evalSlice}' is empty — the suite (${filteredCases.length} case(s)) is too small to reserve a held-out certification slice. Run slice:'all' or add cases.`);
  }
  ...
  // A11: with no explicit --trials, a certifying run defaults to 5, a dev run to 3.
  const trialsDefault = options.certifying ? APPLICATION_CERTIFYING_DEFAULT_TRIALS : APPLICATION_DEFAULT_TRIALS;
  ...
  // Certification tier (default provisional). Only a declared certifying run can
  // later stamp APPLICABLE; everything else is capped at PROVISIONAL in
  // stampApplicationVerdict. resolveCertificationTier never selects a model — it
  ...
  if (certification.tier !== 'certifying') {
    console.log(`  → APPLICABLE is capped to PROVISIONAL for this run (only a declared certifying run can earn APPLICABLE).`);
  }
  ...
  console.log(`Generator model: ${generatorModel}  (env APPLICATION_GENERATOR_MODEL — default representative generator; the measured agent — SKI-306)`);
  ...
  if (dryRun) {
    ...
    const pairwiseGraderCalls = gradingMode === 'pairwise' ? cases.length * trials * APPLICATION_PAIRWISE_ORDERS : 0;
  ...
  console.log('');
  console.log(`REAL RUN — about to invoke ${cases.length * 2 * trials} generator calls + ${cases.length * 2 * trials} pointwise grader calls${gradingMode === 'pairwise' ? ` + ${cases.length * trials * APPLICATION_PAIRWISE_ORDERS} pairwise grader calls (both orders)` : ''} (${cases.length} cases × ${trials} trials). ^C now to abort.`);
  console.log('');
    ...
    try {
      ...
      for (let trialIndex = 0; trialIndex < trials; trialIndex += 1) {
        ...
        const baselineResponse = getEvalResponse(baselinePrompt, {
          ...
          telemetryEvalId: testCase.id,
          telemetryTrialIndex: trialIndex + 1,
        ...
        // A8: run BOTH presentation orders of THIS trial and require agreement (the
        // strongest position-bias mitigation per rubric-best-practices.md § Pairwise
        // Judging). Replaces the prior cross-trial order ALTERNATION, which left a
        // single trial's pairwise verdict exposed to pure position artifact.
        const pairwiseGrade = gradingMode === 'pairwise'
          ? gradeApplicationPairwiseBothOrders(testCase, {
      ...
      for (const trialRecord of trialRecords) {
        for (const [runName, grade] of [['baseline', trialRecord.baseline], ['with_skill', trialRecord.with_skill]]) {
          const record = {
            ...
            grading_mode: gradingMode,
            eval_slice: evalSlice, // A10: which slice (all/iteration/certification) produced this record
            ...
            ...(trialRecord.pairwise ? {
              ...
              // A8: both-orders agreement provenance (true = clean winner; false = the
              // two presentation orders disagreed and the pairwise signal was dropped).
              pairwise_both_orders: trialRecord.pairwise.both_orders === true,
              pairwise_order_agreement: trialRecord.pairwise.order_agreement !== false,
              pairwise_position_bias: trialRecord.pairwise.position_bias === true,
            } : {}),
            verdict_consistency: trialAgg.verdict_consistency,
            verdict_stable: trialAgg.verdict_stable,
  ...
  const completedSubsetAggregateVerdict = computeApplicationAggregateVerdict(completed);
  const aggregateVerdict = errorCount > 0 ? 'unverified' : completedSubsetAggregateVerdict;
  ...
  const summary = {
    ...
    trials,
    grading_mode: gradingMode,
    eval_slice: evalSlice, // A10: all | iteration | certification
    generator_model: generatorModel,
    grader_model: graderModel,
    ...
    completed_subset_aggregate_verdict: completedSubsetAggregateVerdict,
  ...
  if (aggregateVerdict === 'harmful') {
    ...
    console.log('NO MEASURED LIFT: baseline had measurement headroom but the skill did not improve the measured generator behavior on this case set.');
  }
