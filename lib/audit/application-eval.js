...
function isBaselineSaturated({ baseline, redHerring }) {
  ...
  return APPLICATION_AXES.every((axis) => Number(baseline.axis_scores[axis]) >= 2);
}
...
function computeApplicationPerCaseVerdict({ baseline, withSkill, redHerring, pairwise }) {
  ...
  if (pairwise && pairwise.confidence > 0) {
    ...
    if (pairwise.preferred_run === 'with_skill') {
      if (fpDelta < 0 || pairwise.application_verdict === 'mixed') return 'mixed';
      return 'applicable';
    }
    if (pairwise.preferred_run === 'baseline') {
      return 'harmful';
  ...
  if (Math.abs(flagDelta) < 1 && Math.abs(fixDelta) < 1 && Math.abs(fpDelta) < 1 && Math.abs(primaryDelta) < 1) {
...
function runApplicationEval(evalFile, options = {}) {
  ...
  // Tools-ON execution profile (lockstep parity). When supplied (the bidirectional
  // runner always supplies one), the generator runs with FULL tools so it can
  // research repo+web. Without a profile, the legacy default stays tools-OFF.
    ...
    try {
      ...
      for (let trialIndex = 0; trialIndex < trials; trialIndex += 1) {
        ...
        const baselineResponse = getEvalResponse(baselinePrompt, {
          ...
          // Tools-ON when a bidirectional execution profile is supplied; tools-OFF
          // for the legacy default. Both runs use the same level (lockstep parity).
