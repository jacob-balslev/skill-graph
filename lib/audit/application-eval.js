      ...
      for (const trialRecord of trialRecords) {
        for (const [runName, grade] of [['baseline', trialRecord.baseline], ['with_skill', trialRecord.with_skill]]) {
          const record = {
            ...
            trials_total: trials,
            // No generator_model / grader_model fields are INVENTED by this
            // script. When the operator DECLARES the families (for the
            // certifying-tier check), they are recorded as declared_* provenance
            // so the receipt can show what the operator attested — never a model
            // name the script guessed.
