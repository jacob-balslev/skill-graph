...
// APPLICATION_GRADER_PROMPT_PATH is resolved via __dirname so this module works
// when installed as a standalone package (no REPO_ROOT required). Mirrors the
// SH-6304 concept-grader fix applied to lib/audit/evaluate-skill.js near line 70.
const APPLICATION_GRADER_PROMPT_PATH = path.join(__dirname, 'graders', 'application-grader-prompt.md');

// APPLICATION_HISTORY_LOG is resolved via REPO_ROOT (two levels up from lib/audit/).
// persistApplicationHistory() creates the directory if it does not exist.
const REPO_ROOT = path.resolve(__dirname, '../..');
const APPLICATION_HISTORY_LOG = path.join(REPO_ROOT, 'agent-orchestration', 'logs', 'application-history.jsonl');
 ...
 *
 * Creates the log directory if it does not exist.  History persistence must
 * never break eval runs — all I/O errors are silently swallowed.
 */
