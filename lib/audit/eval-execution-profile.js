...
//
// The bidirectional eval guardrail measures a fixed representative generator and
// asks both frontier judges to grade under IDENTICAL conditions. The single
// allowed variable is which frontier judge scores the evidence. Everything else
// — tool access, repo scope, web allowance, working directory — must be
// byte-identical across the two directions. This module is the single source of
// truth for that profile and for the assertion that a run actually honored it.
//
...
// directions, never equal-zero — a different per-direction permission set would
// mean we measured "who got to peek," not the skill.
//
...
// (PASS unreachable). The synthesizer and the verdict-stamper must
...
// profile applies to the measured GENERATOR runs (the representative agent that
// researches + answers the eval task). The GRADERS receive an IDENTICAL CLOSED evidence packet
...
// A6: fence strength on the eval generator's cwd. `os-isolated` means the cwd is a
// public-only temp checkout (isolated-eval-workspace.js) — the private Development tree
// is physically off the agent's path, so the repoScope/answer-key boundary is OS-enforced.
// `prompt-level` (the default for a real-repo-root cwd) means the boundary is the prompt
// instruction + the claim-filter only; an agent could path-traverse to a private sibling.
const FENCE_OS_ISOLATED = 'os-isolated';
const FENCE_PROMPT_LEVEL = 'prompt-level';
 ...
 *
 * The profile is role-AGNOSTIC by design — it is handed to BOTH directions
 * unchanged, and only the per-CLI translation (cliAccessForProfile) differs so
 * each CLI reaches the SAME effective access. Model identity is recorded in the
 * receipt, but execution access must not vary between judge directions.
 *
 * @param {object}  opts
 ...
 * @param {string} [opts.fence]     A6: the enforcement strength of the repoScope/answer-key
 *                                  fence. `os-isolated` = the cwd is a public-only temp checkout
 *                                  (isolated-eval-workspace.js): the private tree is physically
 *                                  absent from the agent's path, so the fence is OS-enforced.
 *                                  `prompt-level` (default) = the cwd is the real repo root inside
 *                                  the private workspace; the fence is the prompt instruction + the
 *                                  claim-filter only — an agent CAN path-traverse to `../sales-hub`.
 *                                  Recorded so a certifying receipt honestly states its fence strength.
 * @returns {{ tools: string, research: string, repoScope: string, cwd: string, fence: string }}
 */
function buildExecutionProfile({ cwd, tools, research, repoScope, fence } = {}) {
  ...
  return {
    ...
    fence: fence === FENCE_OS_ISOLATED ? FENCE_OS_ISOLATED : FENCE_PROMPT_LEVEL,
  };
}
