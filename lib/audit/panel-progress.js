...
//
// run-skill-audit-loop.js is pure orchestration; THIS is its CANONICAL, version-controlled
...
//      This shape is now schema'd at schemas/panel-heartbeat.schema.json — the shared
//      contract the workspace /boardmeeting writer (scripts/agent/boardmeeting-heartbeat.js)
//      also conforms to, so both producers render identically via watch-panel.js.
...
//
// The reporter is the injected `onProgress` sink for run-skill-audit-loop.js; the orchestrator
...
// Per-agent lifecycle glyph vocabulary, shared by the collected renderer, the
// native Task-panel bridge, and the Ink RunPanel.
 ...
 * @returns {string[]}
 */
function renderCollected(status, opts = {}) {
  const s = status || {};
  const agents = Array.isArray(s.agents) ? s.agents : [];
  const done = s.done || 0;
  const total = s.total != null ? s.total : agents.length;
  const failed = s.failed || 0;
  // Live counting timer: when the caller supplies a `nowMs` (the viewer passes Date.now()),
  // extrapolate elapsed past the last heartbeat (`status.ts`) so the header and any ACTIVE
  // agent row visibly count up *between* heartbeats instead of freezing on the snapshot.
  // Without `nowMs` (the runner's own TTY header, unit tests) the snapshot elapsed is used,
  // so renderCollected stays pure (no Date.now() inside) and backward-compatible.
  const tsMs = s.ts ? Date.parse(s.ts) : NaN;
  const liveExtraMs = (opts.nowMs && !Number.isNaN(tsMs)) ? Math.max(0, opts.nowMs - tsMs) : 0;
  const lines = [];
  const globalMs = (s.elapsed_s != null ? s.elapsed_s * 1000 : 0) + (s.complete ? 0 : liveExtraMs);
  lines.push(
    `Skill Audit Loop: (${s.skill || '?'}) · ${s.phase || 'starting'} · ${done}/${total}`
    + (failed ? ` · ${failed} failed` : '')
    + (s.elapsed_s != null ? ` · ${fmtElapsed(globalMs)}` : '')
    + (s.complete ? ' · DONE' : ''),
  );
  agents.forEach((a, i) => {
    const branch = i === agents.length - 1 ? '└' : '├';
    const glyph = agentGlyph(a.state);
    const ph = a.phase ? ` ${a.phase}` : '';
    const active = isActiveAgentState(a.state);
    const el = active ? ` ${fmtElapsed((a.elapsed_s || 0) * 1000 + liveExtraMs)}` : '';
    // Show WHY a terminal-failure row ended (the failure_reason category + the short detail
    // text the runner threads through), so a skipped/failed agent is never a bare "–/✗".
    const reason = ((a.state === 'skipped' || a.state === 'failed') && a.failure_reason)
      ? ` (${a.failure_reason}${a.failure_detail ? `: ${a.failure_detail}` : ''})`
      : '';
    lines.push(`${branch} ${glyph} ${resolveDisplayName(a.model)}${ph} · ${a.state}${el}${reason}`);
  });
  return lines;
}

/**
 * Build a progress reporter for one skill-audit-loop run.
 *
 * @param {object}   opts
...
function createProgressReporter(opts = {}) {
  ...
  let complete = false;
  // Reviewable findings (merge-ledger contributions) surfaced into the heartbeat once curate runs,
  // so the findings-review viewer can show them live DURING a run with no --findings-file. Set via
  // an onProgress({ kind: 'findings', findings }) event; empty until the runner emits it.
  let findings = [];
  let tick = null;
  let headerInit = false;
  const HEADER_ROWS = () => 1 + agents.size; // renderCollected: 1 summary line + 1 per agent
  ...
  function buildStatus() {
    ...
    for (const a of agents.values()) {
      ...
      if (isActiveAgentState(a.state)) running.push({ cell: `${a.model}:${a.phase || 'run'}`, elapsed_s });
      // F4: surface a distinct failure_reason (timeout vs error vs no-document) on failed/skipped
      // rows so a buried advisory failure is visible in the heartbeat, not collapsed to "✗/–".
      agentRows.push({ model: a.model, tier: a.tier, phase: a.phase, state: a.state, elapsed_s, failure_reason: a.failure_reason || null, failure_detail: a.failure_detail || null });
    }
    return {
  ...
  function onProgress(evt) {
    ...
    if (evt.kind === 'phase') {
      ...
      if (evt.state === 'start') {
        ...
        a.state = 'failed';
        a.startedAt = null;
        if (evt.failure_reason) a.failure_reason = evt.failure_reason; // F4
        if (evt.failure_detail) a.failure_detail = String(evt.failure_detail).slice(0, 80); // short cause text
