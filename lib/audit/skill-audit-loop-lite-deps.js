...
function buildCuratePrompt({ skill, proposals, currentSkillPath, mergedSkillPath, mergeLedgerPath, mergeProtocolRef }) {
  ...
  return [
    'You are the CURATOR for a two-frontier curate merge. Read the union-curate merge',
...
function buildOpencodeEnrichArgs(prompt, { modelId, agent = 'build', exploreDir } = {}) {
  ...
  // repo + web, then EMIT the curated SKILL.md as text (captured from stdout). Do NOT add --pure
...
function createSkillAuditLoopLiteDeps(options = {}) {
  ...
  function runClaim(subcommandArgs, { model, agentId } = {}) {
    const id = agentId || `curate-${sanitizeModelForFilename(model || 'slot')}`;
  ...
  function curate({ skill, skillDir, proposals, currentSkillPath }) {
    ...
    const curatorAgentId = 'skill-audit-loop-curator';
    let curatorRunDir;
    try {
