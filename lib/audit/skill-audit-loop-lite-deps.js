...
const { mergeModelEnv } = require('./model-cli-home');
...
function buildOpencodeEnrichArgs(prompt, { modelId, agent = 'plan', exploreDir } = {}) {
...
function createSkillAuditLoopLiteDeps(options = {}) {
  ...
  const dispatchModel = ({ cli, args, cwd, env }) => {
    ...
    return dispatch({ cli: w.cli, args: w.args, cwd: modelCwd || cwd, env: mergeModelEnv(env || process.env, modelEnv) });
  };
