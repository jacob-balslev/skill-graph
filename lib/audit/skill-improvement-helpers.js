...
function resolveModelExecutor(model) {
  ...
  // Free OpenCode models — explicit routing avoids fallthrough ambiguity
  if (model === 'minimax' || model === 'minimax-m2.5-free') {
    return { cli: OPENCODE_CLI, model: 'opencode/minimax-m2.5-free' };
  }
  if (model === 'nemotron' || model === 'nemotron-3-super-free') {
    return { cli: OPENCODE_CLI, model: 'opencode/nemotron-3-super-free' };
  }
...
function buildSkillInvocationPrompt(skillName, taskPrompt) {
  return [
...
// Stored as JSONL in agent-orchestration/logs/skill-health-ledger.jsonl.

const REPO_ROOT = path.resolve(__dirname, '../..');
const HEALTH_LEDGER_PATH = path.join(REPO_ROOT, 'agent-orchestration', 'logs', 'skill-health-ledger.jsonl');
...
const ARCHITECTURE_MAP_PATH = path.join(REPO_ROOT, 'sales-hub', 'ARCHITECTURE_MAP.md');
const DATAFLOW_PATH = path.join(REPO_ROOT, 'sales-hub', 'DATAFLOW.md');
const PAGE_CONTRACTS_DIR = path.join(REPO_ROOT, 'sales-hub', 'docs', 'design', 'page-contracts');
const SKILLS_DIR = path.join(REPO_ROOT, 'skills');
const SALES_HUB_SKILLS_DIR = path.join(REPO_ROOT, 'skills', 'sales-hub');

/**
 * Find a skill directory by name, checking both shared and sales-hub locations.
 */
function findSkillDirByName(skillName) {
  const candidates = [
    path.join(SKILLS_DIR, skillName),
    path.join(SALES_HUB_SKILLS_DIR, skillName),
  ];
