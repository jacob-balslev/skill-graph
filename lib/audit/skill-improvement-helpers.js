...
const { resolveModelDescriptor, REGISTRY_VERSION } = require('../audit-shared/model-provider');
...
function resolveModelExecutor(model) {
  // Role aliases (see ../audit-shared/model-provider.js § Model Identity).
  // The grader role always resolves to the newest Opus via the bare CLI alias.
  if (model === 'strongest-reasoning-grader') {
    return { cli: 'claude', model: 'opus' };
  }
  if (model === 'representative-generator') {
    return { cli: 'claude', model: 'sonnet' };
  }
  // `codex-current` = newest GPT the Codex/GPT app currently serves. The model
...
function buildSkillInvocationPrompt(skillName, taskPrompt, options = {}) {
  ...
  // The comprehension eval (single-direction default) and the tools-OFF application
