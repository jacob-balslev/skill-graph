...
// Shared prompt, evaluation, receipt, and ledger helpers for skill improvement
// and evaluation runners.
...
function resolveModelExecutor(model) {
  ...
  if (model === REPRESENTATIVE_GENERATOR_MODEL) {
    return { cli: 'claude', model: 'sonnet', role: REPRESENTATIVE_GENERATOR_MODEL };
  }
...
function resolveReceiptModelId(modelAlias, { capturedCodexModel = null } = {}) {
  if (modelAlias === REPRESENTATIVE_GENERATOR_MODEL) {
    return REPRESENTATIVE_GENERATOR_RECEIPT_ID;
  }
  let cli = null;
