...
// ─── Live production deps for the multi-agent PANEL IMPROVE orchestrator ───────
//
...
//     final text answer — captured from stdout (extractSkillDocument) and written by us, with a
...
// SKI-350: the captured-text size above which a dispatch that produced no parseable
// SKILL.md is classified 'non-document-output' (the model ran and emitted substantial text
// in the wrong shape — a delivery/prompt problem) rather than 'no-document' (a negligible/
// empty reply). 200 bytes is comfortably above an empty/one-line reply and below any real
// proposal; both observed misses (minimax 1074 b, deepseek-flash 28111 b) land above it.
const NON_DOCUMENT_MIN_BYTES = 200;

// SKI-350: find the index of the first line that is exactly `---` AND that opens a
// document whose frontmatter PARSES and carries a non-empty `name:`. A weak free model
// often wraps the document in a "Here is the skill:" preamble that contains its OWN `---`
// horizontal rule(s), so the naive "first `---`" anchor slices from a stray rule and the
// frontmatter then fails to parse — the exact reason a 28 KB deepseek-flash reply that
// DID contain a real SKILL.md was rejected as no-document. Probing each `---` for a
// parseable name finds the real document start even behind prose + stray rules. This only
// improves EXTRACTION; the strict looksLikeSkillDoc write-path gate is unchanged, so prose
// can never slip through — a better anchor can only help a genuine doc pass.
function findParseableFrontmatterStart(lines) {
  for (let i = 0; i < lines.length; i += 1) {
    if (lines[i].trim() !== '---') continue;
    try {
      const fm = parseFrontmatter(`${lines.slice(i).join('\n')}\n`);
      if (fm && typeof fm.name === 'string' && fm.name.trim().length > 0) return i;
    } catch (_) { /* not a valid frontmatter start at this `---` — try the next */ }
  }
  return -1;
}

function extractSkillDocument(raw) {
  ...
  const lines = t.split('\n');
  // Prefer the first `---` whose slice parses as frontmatter with a name (skips preamble
  // prose + stray `---` rules); fall back to the naive first-`---`, then the first heading.
  const parseableFmIdx = findParseableFrontmatterStart(lines);
  if (parseableFmIdx >= 0) return lines.slice(parseableFmIdx).join('\n').trim();
...
// stream → 0-byte capture). The existing text parsers (extractSkillDocument for propose/revise,
...
function createSkillAuditLoopDeps(options = {}) {
  ...
  const advisoryDispatch = options.advisoryDispatch || (({ backend, cli, args, cwd, runDirAbs }) => {
    ...
    const r = runCommandWithTimeoutSync({
      ...
      // Liveness heartbeat (best-effort): the wrapper's --child ticks this file on each
      // streamed chunk so a watcher can tell an alive-but-working dispatch from a dead one.
      // No-op when the caller did not pass a run dir.
      heartbeatFile: runDirAbs ? path.join(runDirAbs, 'dispatch-heartbeat.json') : null,
    });
    const tokenUsage = extractTokenUsage(r.stdout || '', r.stderr || '', r);
    let stdout = r.stdout || '';
    // opencode is dispatched with `--format json` (buildOpencodeAuditArgs): stdout is the JSONL
    ...
    if (backend === 'opencode') {
      ...
      // claude is dispatched with `--output-format stream-json` (buildClaudeAuditArgs): stdout is
      // the JSONL EVENT STREAM, not rendered text. Reconstruct the assistant's text reply so the
      // stdout-PARSING phases this dispatch feeds — cross-review/verify (parseLastJsonBlock) and
      // revise/advisory-propose (extractSkillDocument) — receive plain text and parse unchanged,
      // exactly as they did under `--output-format text`. Only replace when an event stream actually
      // parsed AND carried text; a non-event stdout (early CLI error) or an empty/error result is
      // left intact so the real failure (and any rate-limit signal) surfaces to the callers.
      const rec = reconstructClaudeText(stdout);
      if (rec.sawEvent && rec.text) stdout = rec.text;
      r.dispatchDiagnostics = {
        claude_stream_json_seen: rec.sawEvent,
        claude_raw_stdout_bytes: String(r.stdout || '').length,
        claude_reconstructed_text_bytes: String(stdout || '').length,
      };
    }
    const ok = r.ok === true;
    const failure_reason = ok ? null : r.timedOut ? 'timeout' : 'error';
    return { ok, stdout, stderr: r.stderr || '', failure_reason, error: r.error || null, diagnostics: r.dispatchDiagnostics || null, token_usage: tokenUsage, started_at: r.started_at, ended_at: r.ended_at, duration_ms: r.duration_ms };
  });
  ...
  function appendAgentTelemetry({ skill, skillDir, phase, model, backend, tier = 'advisory', ok = true, status, start, stdout, stderr, result, beforeText, afterText, artifactPath, receiptPath, error, extra }) {
    try {
  ...
  function buildArgsFor(model, prompt, { runDirAbs } = {}) {
    ...
    if (backend === 'gemini') {
      return { backend, cli: 'gemini', args: buildGeminiAuditArgs(prompt, { modelId: desc.modelId }) };
    }
    if (backend === 'opencode') {
      ...
      //     the artifact ("I'm in Plan Mode (read-only), so I cannot execute the audit-loop result" — verified
      ...
      return { backend, cli: 'opencode', exploreDir, args: buildOpencodeAuditArgs(prompt, { modelId: desc.modelId, exploreDir, agent: 'build' }) };
    }
    if (backend === 'codex' || cliForModel(model) === 'codex') {
      ...
      // buildCodexAuditArgs already branches correctly (SKI-169 Fix B); the panel just has to
      // pass the real fence state instead of a hardcoded false.
      return { backend: 'codex', cli: 'codex', args: buildCodexAuditArgs(prompt, { model: desc.modelId, writableRoots: runDirAbs ? [runDirAbs] : [], osFenceActive: advisoryOsFence.active }) };
    }
    return { backend: 'claude', cli: 'claude', args: buildClaudeAuditArgs(prompt, { model }) };
  }
...
- IMPROVE the skill: add capability, precision, grounding, edge cases. NEVER strip or shorten existing content.
...
${proposedSkillBody || '(missing)'}`;
  ...
  function collectAdvisoryIterationSuggestions({ skill, skillDir, model, brief, skillBody, proposalPath, runDirAbs }) {
    ...
    try {
      ...
      telemetryStart = startAgentTelemetry({ skill, operation: 'panel', phase: 'iteration-suggestions', model, backend, tier: 'advisory' });
      const res = advisoryDispatch({ backend, cli, args, cwd: modelCwd || skillGraphRoot, mode: 'text', runDirAbs });
      ...
      if (!res.ok) {
        ...
        appendAgentTelemetry({
          skill, skillDir, phase: 'iteration-suggestions', model, backend,
          tier: 'advisory', ok: false, status: 'failed', start: telemetryStart,
          stdout: res.stdout, stderr: res.stderr, result: res, artifactPath: outPath,
          receiptPath: rawPath, error: payload.error,
          extra: { suggestions_count: 0, parse_ok: false },
        });
        return outPath;
      }
      ...
      appendAgentTelemetry({
        skill, skillDir, phase: 'iteration-suggestions', model, backend,
        tier: 'advisory', ok: payload.ok === true, status: payload.ok ? 'completed' : 'failed',
        start: telemetryStart, stdout: res.stdout, stderr: res.stderr, result: res,
        artifactPath: outPath, receiptPath: rawPath, error: payload.error,
        extra: { suggestions_count: payload.suggestions.length, parse_ok: payload.parse_ok },
      });
      return outPath;
    } catch (e) {
      ...
      appendAgentTelemetry({
        skill, skillDir, phase: 'iteration-suggestions', model, backend: telemetryBackend,
        tier: 'advisory', ok: false, status: 'failed', start: telemetryStart,
        artifactPath: outPath, error: e.message,
        extra: { suggestions_count: 0, parse_ok: false },
      });
      return outPath;
    }
  ...
  function dispatchWriteProposal({ skill, skillDir, model, brief, runDirAbs, proposalPath, noveltyMemoPath }) {
    try {
      ...
      const finishSuccess = (result) => {
        ...
        const suggestionPath = collectAdvisoryIterationSuggestions({ skill, skillDir, model, brief, skillBody, proposalPath, runDirAbs });
      ...
      const runAttempt = (promptText, suffix) => {
        ...
        const telemetryStart = startAgentTelemetry({ skill, operation: 'panel', phase: 'propose', model, backend, tier: 'advisory' });
        const res = advisoryDispatch({ backend, cli, args, cwd, mode: 'text', runDirAbs });
        if (exploreCleanup) exploreCleanup();
        ...
        if (buildOutputPath && fs.existsSync(buildOutputPath)) {
          ...
          const builtDoc = extractSkillDocument(fileDoc);
          ...
          if (looksLikeSkillDoc(builtDoc, skill)) {
            ...
            appendAgentTelemetry({
              skill, skillDir, phase: 'propose', model, backend, tier: 'advisory',
              ok: true, status: 'completed', start: telemetryStart,
              stdout: res.stdout, stderr: res.stderr, result: res,
              beforeText: skillBody, afterText: builtDoc,
              artifactPath: proposalPath, receiptPath: diagnosticsPath,
              extra: { attempt: attempt.attempt, accepted_via: attempt.accepted_via },
            });
            return finishSuccess({ ok: true, proposalPath, noveltyMemoPath, via: attempt.accepted_via });
          }
        }
        const doc = extractSkillDocument(res.stdout);
        ...
        if (looksLikeSkillDoc(doc, skill)) {
          ...
          appendAgentTelemetry({
            ...
            ok: true, status: 'completed', start: telemetryStart,
            stdout: res.stdout, stderr: res.stderr, result: res,
            beforeText: skillBody, afterText: doc,
            artifactPath: proposalPath, receiptPath: diagnosticsPath,
            extra: { attempt: attempt.attempt, accepted_via: attempt.accepted_via },
          });
          return finishSuccess({ ok: true, proposalPath, noveltyMemoPath, via: attempt.accepted_via });
        ...
        if (wrote) {
          const writtenDoc = fs.readFileSync(proposalPath, 'utf8');
          ...
          appendAgentTelemetry({
            ...
            ok: true, status: 'completed', start: telemetryStart,
            stdout: res.stdout, stderr: res.stderr, result: res,
            beforeText: skillBody, afterText: writtenDoc,
            artifactPath: proposalPath, receiptPath: diagnosticsPath,
            extra: { attempt: attempt.attempt, accepted_via: attempt.accepted_via },
          });
          return finishSuccess({ ok: true, proposalPath, noveltyMemoPath, via: attempt.accepted_via });
        }
        // SKI-350: classify the not-accepted outcome distinctly so a buried advisory miss is
        // diagnosable. A dispatch failure keeps its 'timeout'/'error' (auth/launch/capacity).
        // A successful dispatch that produced text but no parseable SKILL.md splits by volume:
        // SUBSTANTIAL captured text (>= NON_DOCUMENT_MIN_BYTES) is 'non-document-output' — the
        // model RAN and said a lot but in the wrong shape (a delivery/prompt problem, salvageable
        // by the retry), distinct from a negligible/empty reply ('no-document'). Both are distinct
        // from the auth/capacity 'error' the task asked us to separate.
        const capturedBytes = (res.stdout || '').length;
        const failure_reason = res.failure_reason
          || (!res.ok ? 'error'
            : capturedBytes >= NON_DOCUMENT_MIN_BYTES ? 'non-document-output'
              : 'no-document');
        ...
        appendAgentTelemetry({
          ...
          extra: { attempt: attempt.attempt, failure_reason },
        });
        return {
          ok: false,
          error: `${model}: captured ${(res.stdout || '').length}b of text (not document-shaped) and no file written; dispatch ok=${res.ok}; reason=${failure_reason}`,
      ...
      // Retry on BOTH a negligible reply and a substantial-but-wrong-shape reply (SKI-350):
      // the FINAL DELIVERY RETRY prompt ("output ONLY the complete SKILL.md") addresses exactly
      // the prose-wrapped / preamble'd case that produced large non-document output.
      if (first.failure_reason === 'no-document' || first.failure_reason === 'non-document-output') {
      ...
      appendAgentTelemetry({
        skill, skillDir, phase: 'propose', model, backend: resolveBackend(model).backend,
        tier: 'advisory', ok: false, status: 'failed', error: e.message,
        extra: { failure_reason: 'error' },
      });
      return { ok: false, error: e.message, failure_reason: 'error' };
    }
  }
  ...
  function dispatchTextOnly({ model, prompt, runDirAbs }) {
    try {
      const { cli, args, backend, exploreCleanup } = buildArgsFor(model, prompt, { runDirAbs });
      const telemetryStart = startAgentTelemetry({ operation: 'panel', phase: 'text', model, backend });
      const res = advisoryDispatch({ backend, cli, args, cwd: modelCwd || skillGraphRoot, mode: 'text', runDirAbs });
      if (exploreCleanup) exploreCleanup();
      return { ok: res.ok, stdout: res.stdout || '', stderr: res.stderr || '', error: res.error, result: res, backend, telemetryStart };
    } catch (e) { return { ok: false, stdout: '', stderr: '', error: e.message, result: null, backend: null, telemetryStart: null }; }
  }
  ...
  function crossReview({ skill, skillDir, reviewerModel, reviewerTier, ownProposalPath, otherProposals, round, artifactsDir }) {
    try {
      ...
      if (!res.ok) {
        appendAgentTelemetry({
          skill, skillDir, phase: 'cross-review', model: reviewerModel, backend: res.backend,
          tier: reviewerTier, ok: false, status: 'failed', start: res.telemetryStart,
          stdout: res.stdout, stderr: res.stderr, result: res.result,
          error: res.error || 'cross-review dispatch failed',
          extra: { round, parse_ok: false, reviewed_targets: (otherProposals || []).map((o) => o.model) },
        });
        return { ok: false, error: res.error || 'cross-review dispatch failed' };
      }
      ...
      appendAgentTelemetry({
        skill, skillDir, phase: 'cross-review', model: reviewerModel, backend: res.backend,
        tier: reviewerTier, ok: parse_ok, status: parse_ok ? 'completed' : 'failed',
        start: res.telemetryStart, stdout: res.stdout, stderr: res.stderr, result: res.result,
        artifactPath: path.join(runDirAbs, `${skill}.${sanitizeModelForFilename(reviewerModel)}.review-r${round}.md`),
        error: parse_ok ? null : 'cross-review output did not contain parseable reviews JSON',
        extra: { round, parse_ok, reviewed_targets: (otherProposals || []).map((o) => o.model), feedback_count: feedback.reduce((sum, f) => sum + (Array.isArray(f.items) ? f.items.length : 0), 0) },
      });
  ...
  //     SKILL.md as its final text answer, which WE capture from stdout (extractSkillDocument /
  ...
  function reviseProposal({ skill, skillDir, reviserModel, reviserTier, ownProposalPath, feedbackForMe, round, artifactsDir }) {
    try {
      ...
      if (sandboxed) {
        ...
        const telemetryStart = startAgentTelemetry({ skill, operation: 'panel', phase: 'revise', model: reviserModel, backend: b, tier: reviserTier });
        const res = advisoryDispatch({ backend: b, cli, args, cwd: modelCwd || skillGraphRoot, mode: 'text', runDirAbs: writeDirAbs });
        if (exploreCleanup) exploreCleanup();
        ...
        const afterText = fs.existsSync(ownProposalPath) ? fs.readFileSync(ownProposalPath, 'utf8') : '';
        appendAgentTelemetry({
          skill, skillDir, phase: 'revise', model: reviserModel, backend: b, tier: reviserTier,
          ok: res.ok === true, status: res.ok ? 'completed' : 'failed', start: telemetryStart,
          stdout: res.stdout, stderr: res.stderr, result: res,
          beforeText: own, afterText,
          artifactPath: ownProposalPath,
          error: res.ok ? null : (res.error || 'revise dispatch failed'),
          extra: { round, changed: after !== before, feedback_count: feedbackForMe.length, document_captured: looksLikeSkillDoc(doc) },
        });
      ...
      const telemetryStart = startAgentTelemetry({ skill, operation: 'panel', phase: 'revise', model: reviserModel, backend, tier: reviserTier });
      const res = advisoryDispatch({ backend, cli, args, cwd: modelCwd || skillGraphRoot, mode: 'write', runDirAbs: writeDirAbs });
      if (exploreCleanup) exploreCleanup();
      ...
      appendAgentTelemetry({
        skill, skillDir, phase: 'revise', model: reviserModel, backend, tier: reviserTier,
        ok: res.ok === true, status: res.ok ? 'completed' : 'failed', start: telemetryStart,
        stdout: res.stdout, stderr: res.stderr, result: res,
        beforeText: own, afterText,
        artifactPath: ownProposalPath,
        error: res.ok ? null : (res.error || 'revise dispatch failed'),
        extra: { round, changed: after !== before || meta.changed === true, feedback_count: feedbackForMe.length, parse_ok: Boolean(meta && Object.keys(meta).length) },
      });
  ...
  function verifyMerge({ skill, skillDir, verifierModel, mergedSkillPath, mergeLedgerPath, ownProposalPath, artifactsDir, round }) {
    try {
      ...
      if (!res.ok) {
        appendAgentTelemetry({
          skill, skillDir, phase: 'verify',
          model: verifierModel, backend: res.backend, tier: 'mandatory',
          ok: false, status: 'failed', start: res.telemetryStart,
          stdout: res.stdout, stderr: res.stderr, result: res.result,
          error: res.error || 'verify dispatch failed',
          extra: { round: round || 1, parse_ok: false },
        });
        return { ok: false, error: res.error || 'verify dispatch failed' };
      }
      ...
      const verifyTranscript = path.join(runDirAbs, `${skill}.${sanitizeModelForFilename(verifierModel)}.verify-r${round || 1}.md`);
      appendAgentTelemetry({
        skill, skillDir, phase: 'verify',
        model: verifierModel, backend: res.backend, tier: 'mandatory',
        ok: parse_ok, status: parse_ok ? 'completed' : 'failed', start: res.telemetryStart,
        stdout: res.stdout, stderr: res.stderr, result: res.result,
        artifactPath: verifyTranscript,
        error: parse_ok ? null : 'verifyMerge: no parseable { approved, gaps } JSON block in verifier output',
        extra: { round: round || 1, parse_ok, approved: parsed && parsed.approved === true, gaps_count: parsed && Array.isArray(parsed.gaps) ? parsed.gaps.length : 0 },
      });
  ...
  return {
    ...
    curate: base.curate,
    prepareCandidateEval: base.prepareCandidateEval,
