...
function createSkillAuditLoopDeps(options = {}) {
  ...
  const advisoryDispatch = options.advisoryDispatch || (({ backend, cli, args, cwd }) => {
    ...
    return { ok, stdout, stderr: r.stderr || '', failure_reason, error: r.error || null };
  });
  ...
  function buildArgsFor(model, prompt, { runDirAbs } = {}) {
    ...
    if (backend === 'opencode') {
      ...
      // SKI-404 (Option A): opencode 1.16.2's project resolution walks `--dir` UP to the workspace
      // root, which the OS fence DENIES → cold-start EPERM at project-instance creation. Point `--dir`
      // at the public skills COPY (built OUTSIDE ws by preparePublicWorkspace) so the walk-up reaches
      // the OS tmp root, never the fence-denied workspace root. The copy mirrors the corpus, so
      // research breadth is preserved. Fall back to the real skillsRoot only if no public copy exists
      // (e.g. opencode not in the advisory roster / build skipped) — preserving the prior path.
      const exploreDir = advisoryOsFence.active
        ? (publicWorkspaceSkillsRoot || skillsRoot)
    ...
    try {
      ...
      const runAttempt = (promptText, suffix) => {
        ...
        if (looksLikeSkillDoc(doc, skill)) {
          ...
          return { ok: true, proposalPath, noveltyMemoPath, via: suffix ? 'stdout-capture-retry' : 'stdout-capture' };
        }
        const wrote = fs.existsSync(proposalPath) && fs.statSync(proposalPath).size > 0;
        if (wrote) return { ok: true, proposalPath, noveltyMemoPath, via: suffix ? 'file-write-retry' : 'file-write' };
