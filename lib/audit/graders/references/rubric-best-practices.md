...
**Compliance (audit-loop pairwise judges).** `fusion-judge` (both-orders + agreement) ✓; `structure-ab` (per-trial randomized coin) ✓; `experiment-judge` (independent absolute before/after scoring) ✓; the **application comparative grader** (`lib/audit/application-eval.js`) was removed in the 2026-06-15 application_verdict→three-verdict collapse and is no longer a wired pairwise judge (it had run both orders per trial with agreement required). Any new pairwise judge MUST satisfy this section before it is wired into the loop.
