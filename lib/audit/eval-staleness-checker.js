 ...
 *
 * Usage:
 *   node lib/audit/eval-staleness-checker.js                  # check all evals
 *   node lib/audit/eval-staleness-checker.js --skill graph-audit  # single skill
 *   node lib/audit/eval-staleness-checker.js --json            # JSON output only
 *   node lib/audit/eval-staleness-checker.js --all             # check all evals, not just substance:"domain"
 */

const fs = require('fs');
const path = require('path');
const { execFileSync } = require('child_process');
const { loadWorkspaceConfig, resolveSkillRoots, resolveTruthSourcePath, workspaceRoot } = require('../../scripts/lib/roots');

const ROOT_DIR = workspaceRoot();
const WORKSPACE_CONFIG = loadWorkspaceConfig(ROOT_DIR, msg => process.stderr.write(`WARN ${msg}\n`));
const SKILL_ROOTS = resolveSkillRoots(ROOT_DIR, WORKSPACE_CONFIG);
...
if (showHelp) {
  console.log(`Usage: node lib/audit/eval-staleness-checker.js [options]
...
Examples:
  ...
  node lib/audit/eval-staleness-checker.js --skill graph-audit
  node lib/audit/eval-staleness-checker.js --json
  node lib/audit/eval-staleness-checker.js --all --json`);
  process.exit(0);
}
...
const EVALS_DIR = path.join(ROOT_DIR, 'examples', 'evals');

/**
 * Find all consolidated eval artifacts under examples/evals/.
 * Returns array of { skillName, evalPath, skillDir }.
 */
function discoverEvalFiles() {
  const results = [];
  if (!fs.existsSync(EVALS_DIR)) return results;

  for (const entry of fs.readdirSync(EVALS_DIR, { withFileTypes: true })) {
    if (!entry.isFile() || !entry.name.endsWith('.json')) continue;
    const evalPath = path.join(EVALS_DIR, entry.name);
    let skillName = entry.name.replace(/\.routing\.json$/, '').replace(/\.json$/, '');
    try {
      const parsed = JSON.parse(fs.readFileSync(evalPath, 'utf8'));
      if (typeof parsed.skill_name === 'string' && parsed.skill_name.trim()) {
        skillName = parsed.skill_name.trim();
      }
    } catch {
      // Main pass reports malformed JSON as a warning; discovery keeps it visible.
    }
    if (!filterSkill || skillName.includes(filterSkill) || entry.name.includes(filterSkill)) {
      const skillDir = SKILL_ROOTS
        .map(root => path.join(root.absPath, skillName))
        .find(candidate => fs.existsSync(path.join(candidate, 'SKILL.md'))) || null;
      results.push({ skillName, evalPath, skillDir });
    }
  }
...
const PATH_PREFIXES = [
  ROOT_DIR,
  ...SKILL_ROOTS.map(root => root.absPath),
  ...SKILL_ROOTS.map(root => path.dirname(root.absPath)),
];
...
function extractClaims(evalEntry) {
  ...
  addTruthSourceClaims(claims, seen, evalEntry);
  ...
  for (const { field, text } of textFields) {
    ...
    while ((match = PATH_LINE_REGEX.exec(text)) !== null) {
      ...
      addClaim(claims, seen, { type: 'path', value: filePath, field });
    ...
    while ((match = SYMBOL_REGEX.exec(text)) !== null) {
      ...
      if (!isGenericSymbol(symbol)) {
        addClaim(claims, seen, { type: 'symbol', value: symbol, field });
      }
    }
...
function isLikelyPath(str) {
  ...
  // API endpoints are not filesystem paths.
...
function parseTruthSourceReference(value) {
  if (typeof value !== 'string') return null;
  if (/^https?:\/\//i.test(value)) return null;
  const withoutAnchor = value.split('#')[0];
  const match = withoutAnchor.match(/^(.+?)(?::\d+(?:-\d+)?)?$/);
  if (!match || !match[1]) return null;
  return match[1];
}

function addClaim(claims, seen, claim) {
  const key = claim.type === 'line_number'
    ? `line:${claim.value.file}:${claim.value.lineStart}:${claim.value.lineEnd || ''}`
...
function addTruthSourceClaims(claims, seen, evalEntry) {
  if (!Array.isArray(evalEntry.truth_sources)) return;
  for (const source of evalEntry.truth_sources) {
    const relPath = parseTruthSourceReference(source);
    if (!relPath || !isLikelyPath(relPath)) continue;
    addClaim(claims, seen, { type: 'path', value: relPath, field: 'truth_sources' });
  }
}
...
function resolveFilePath(filePath) {
  ...
  const truthSourcePath = resolveTruthSourcePath(filePath, ROOT_DIR, SKILL_ROOTS);
  if (fs.existsSync(truthSourcePath)) return truthSourcePath;
  ...
  const SEARCH_DIRS = uniqueExistingPaths([
    ROOT_DIR,
    ...SKILL_ROOTS.map(root => root.absPath),
    ...SKILL_ROOTS.map(root => path.dirname(root.absPath)),
...
function symbolExists(symbol, contextFile) {
  ...
  const searchDirs = uniqueExistingPaths([
    ROOT_DIR,
    ...SKILL_ROOTS.map(root => root.absPath),
    ...SKILL_ROOTS.map(root => path.dirname(root.absPath)),
    path.join(ROOT_DIR, 'scripts'),
