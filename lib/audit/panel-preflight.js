...
// Preflight checks for the multi-model panel runner: CLI availability, auth,
// budget locks, writable state dirs, and public-content scope.

const fs = require('fs');
const path = require('path');
const { spawnSync } = require('child_process');

const { findExhaustedLock } = require('./panel-budget');
const { canWriteDir, envForCli } = require('./model-cli-home');
...
function authFailureMessage(cli, res, { modelCliHome, hint } = {}) {
  const scratch = modelCliHome && modelCliHome.active && modelCliHome.mode === 'scratch';
  const detail = res && res.detail ? ` [${res.detail}]` : '';
  if (scratch) {
    return `${cli} CLI auth probe FAILED in scratch CLI home — this does NOT prove the real ${cli} CLI is logged out; the scratch auth bridge did not provide usable auth${detail}`;
  }
  return `${cli} CLI auth probe FAILED — ${hint || AUTH_HINT[cli] || `authenticate the ${cli} CLI`}${detail}`;
}

function authFailureHint(cli, { modelCliHome } = {}) {
  const scratch = modelCliHome && modelCliHome.active && modelCliHome.mode === 'scratch';
  if (scratch) {
    return `fix the scratch CLI home auth bridge for ${cli}, or rerun with --model-cli-home real when the real CLI state dirs are writable`;
  }
  return AUTH_HINT[cli] || `authenticate the ${cli} CLI`;
}
...
function runPanelPreflight(options = {}) {
  ...
  const modelEnv = options.envByCli ? { byCli: options.envByCli, default: options.env || {} } : (options.env || {});
  ...
  for (const cli of mandatoryClis) {
    const cliEnv = envForCli(cli, process.env, modelEnv);
    const dir = modelCliStateDir(cli, cliEnv);
    if (!canWriteDir(dir, fsImpl)) {
      const realHomeClaudeInScratch = cli === 'claude'
        && options.modelCliHome
        && options.modelCliHome.active
        && options.modelCliHome.mode === 'scratch'
        && options.envByCli
        && Object.prototype.hasOwnProperty.call(options.envByCli, 'claude');
      if (realHomeClaudeInScratch) {
        warnings.push(`${cli} state dir is not writable: ${dir}; using real HOME for keychain-bound auth while scratch mode covers the other CLIs`);
      } else {
  ...
  for (const model of mandatoryModels) {
    const found = findExhaustedLock({ model, budgetDir: options.budgetDir, fsImpl });
    if (found.active) errors.push(`${model} budget lock is present: ${found.lockPath}`);
  }
  for (const model of advisoryModels) {
    const found = findExhaustedLock({ model, budgetDir: options.budgetDir, fsImpl });
    if (found.active) warnings.push(`${model} advisory budget lock is present: ${found.lockPath}`);
  }
  ...
  if (options.authProbe) {
    ...
    for (const cli of allClis) {
      const tier = mandatoryClis.includes(cli) ? 'mandatory' : 'advisory';
      const cliEnv = envForCli(cli, process.env, modelEnv);
      const res = authProbe(cli, { env: cliEnv, spawn, timeoutMs });
      const hint = res.ok === false ? authFailureHint(cli, { modelCliHome: options.modelCliHome }) : null;
      auth[cli] = {
        tier,
        ok: res.ok,
        detail: res.detail || null,
        hint,
        scratch_bridge: Boolean(options.modelCliHome && options.modelCliHome.active && options.modelCliHome.mode === 'scratch'),
      };
      if (res.ok === false) {
        const msg = authFailureMessage(cli, { detail: res.detail }, { modelCliHome: options.modelCliHome, hint });
        if (tier === 'mandatory') errors.push(msg);
        else warnings.push(msg);
      }
    }
