...
//   4. Build the ENRICHED eval target (a temp copy whose SKILL.md is the merged
//      output) so the guardrail grades the ENRICHED skill, not the canonical
//      pre-enrich version. The canonical skill is NOT mutated here.
//   5. Run the bidirectional EVAL guardrail (run-bidirectional-eval.js) on the
//      enriched copy: tools-ON parity, cross-family, conservative reconciliation.
//   6. Keep-or-revert (GUARDRAIL ONLY) + apply-on-keep: keep the enriched skill;
 ...
 * failed to credit it (skill-audit-loop-philosophy.md § "WHY enrich, never strip
 * to a delta").
 *
 ...
 *   @param {Function} [opts.deps.prepareEnrichedEval] ({ skill, skillDir, mergedSkillPath }) => { evalSkillDir, cleanup? }. Returns a temp skill dir whose SKILL.md is the enriched merge so the guardrail grades the ENRICHED skill (SH-6686). If omitted, the eval falls back to the canonical skillDir.
 *   @param {Function} [opts.deps.applyMerge]        ({ skill, skillDir, mergedSkillPath }) => void. Applies the enriched SKILL.md to the canonical working tree. Called ONLY when keep-or-revert decides KEEP — the single point the canonical skill is mutated. A REVERT never applies (canonical stays original), so there is no git-revert-HEAD.
...
function runSkillAuditLoopLite(opts = {}) {
  ...
  // 4. Build the ENRICHED eval target (SH-6686). The guardrail must grade the
  //    ENRICHED skill, NOT the canonical pre-enrich version — otherwise keep-or-revert
  //    decides on a skill that was never changed. `prepareEnrichedEval` returns a temp
  //    skill dir whose SKILL.md is the curator's merged output; the canonical skill is
  //    NOT mutated here. When the dep is absent (enrich-only / back-compat), the eval
  ...
  // 5. Bidirectional EVAL guardrail (tools-ON parity, cross-family) on the ENRICHED
  //    skill. Skipped when (a) no direction runner is injected (enrich-only dry run),
  ...
  //    cannot be graded is KEPT (enrich succeeded), not crashed. Before this guard, a
  //    missing evals/<mode>.json threw ENOENT deep in runApplicationEval and discarded
  //    the entire (successful) enrichment (SKI: pilot best-practice 2026-06-05). The
  //    enriched eval dir is a recursive copy of the canonical skillDir, so the canonical
  ...
  // when the artifact is absent the guardrail is skipped (NOT a regression — keep).
  ...
  //    enriched SKILL.md to the working tree; the caller (CLI/operator) reviews + commits
...
function main(argv) {
  ...
  if (!skill || !skillDir) {
    console.error('Usage: node lib/audit/run-skill-audit-loop-lite.js --skill <slug> --skill-dir <dir> [--cwd <skill-graph-root>] [--eval-mode application|comprehension] [--prior-verdict <V>] [--curator <model>] [--dry-run] [--no-eval]');
    process.exit(1);
  }
