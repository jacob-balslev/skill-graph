...
function main(argv) {
  ...
  // APPLICABLE gate PARKED (2026-06-14T, owner directive) — see run-skill-audit-loop.js for the
  // full rationale (application eval not yet discriminating + expensive; the test is the problem,
  // not the skills). Application mode is OFF by default; opt in per-run with `--eval`, or revert
  // the parking commit to re-enable permanently. Comprehension mode is unaffected (cheap, default-on).
