...
// A skill whose synthesized verdict is measurably worse than the prior recorded
// verdict may be reverted. "Didn't move the score" / UNVERIFIED / a narrow
// non-improvement is NOT a regression.

// Confidence/quality ordering used to decide "measurably worse than prior". Mirrors
// docs/verdict-semantics.md (PASS > PROVISIONAL > UNVERIFIED, negatives
// below). Higher = better.
 ...
 * @param {object} [opts]
 * @param {string} [opts.priorVerdict]  The skill's prior comprehension verdict.
...
function decideKeepOrRevert(evalReceipt, { priorVerdict } = {}) {
  ...
  // 1. Measurably worse than the prior recorded verdict ⇒ revert. Only when prior
  ...
  // 2. Everything else ⇒ KEEP. Crucially: a non-improving or UNVERIFIED guardrail
...
function runSkillAuditLoopLite(opts = {}) {
  ...
  //    missing evals/comprehension.json threw ENOENT deep in the eval runner and discarded
