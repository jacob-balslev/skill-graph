  ...
  for (const c of contributions) {
    ...
    if (disposition === 'dropped') {
      ...
      if (!reason) {
        ...
        violations.push({ id: c.id, reason: `dropped for an unscored/delta reason ("${reason}") — banned by curate-not-strip; only wrong/redundant/harmful justifies a drop` });
      }
      continue;
    }
 ...
 *   @param {Function} [opts.deps.prepareEnrichedEval] ({ skill, skillDir, mergedSkillPath }) => { evalSkillDir, cleanup? }. Returns a temp skill dir whose SKILL.md is the curated merge so the guardrail grades the curated skill (SH-6686). If omitted, the eval falls back to the canonical skillDir.
