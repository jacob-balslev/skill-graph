...
// `run-panel-enrich.js` orchestrator (a Node CLI cannot call the Agent tool; only the session
// can). It REUSES the tested live deps (`panel-enrich-live-deps.js` → `crossReview`).
//
...
function main(argv) {
  ...
  finish({ ok: true, feedback, reviewed: others.map((o) => o.model) }, 0);
}
