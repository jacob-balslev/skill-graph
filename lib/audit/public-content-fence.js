...
function defaultPublicRoots({ skillGraphRoot, skillsRoot, workspaceRoot } = {}) {
  ...
  // workspace audit-artifacts root (skill-audit-claim's AUDIT_ROOT = <workspace>/
  // .opencode/progress/skill-audits). That is the loop's OWN derived-artifact area —
  // NOT private workspace data — and the skill-audit-claim private-skill filter already
  // refuses to claim a private skill, so no private-skill run dir is ever created here.
  // It is therefore a legitimate public-scope WRITE target; without it the fence would
  // reject the audit loop's own claim run dirs. The narrow `.opencode/progress/
  // skill-audits` path (not the whole workspace) keeps sibling private trees out.
  const auditArtifacts = path.join(ws, '.opencode', 'progress', 'skill-audits');
