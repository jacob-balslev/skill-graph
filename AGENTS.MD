...
- Current public release checkpoint in docs: `0.5.7` on `2026-05-18`.
- Current skill contract: `schema_version: 6` (closed-enum category, browse-facet framing; landed across skill-metadata-protocol `af6fd7f` / skill-graph `f489641` / skills `568647a` on 2026-05-16, then advanced to v6 with marketplace_tier / secondary_categories fields). The v4→v5 migration is complete across the 145-skill canonical workspace; see `docs/migrations/v4-to-v5.md` for that breaking-change matrix and `docs/migrations/v5-to-v6.md` for the v5→v6 follow-on.
- This repo is the **canonical consolidated implementation** post-2026-05-18 (commit `654b4df`; see [ADR 0009](docs/adr/0009-sibling-repo-deprecation.md) and SH-6137). Schemas, audit scripts, graders, eval fixtures, examples, and the protocol/audit canonical docs all live here. The canonical skill library (145 SKILL.md files) lives at `/Users/jacobbalslev/Development/skills/`. The sibling `skill-metadata-protocol` and `skill-audit-loop` repos are preserved as docs-only mirrors with deprecation banners — they are not archived on GitHub but no longer carry active source code. `examples/` still holds specimen projects and per-skill comprehension evals.
...
- `github.com/jacob-balslev/skill-graph` (this repo) **IS** the canonical Skill Graph **tooling repo** post-2026-05-16 monorepo split. It owns the lint, manifest compiler, router, drift sentinel, export pipeline, and per-skill comprehension evals. The protocol contract (v5 frontmatter, JSON Schemas) lives in the sibling `skill-metadata-protocol` repo; the canonical `skills/<name>/SKILL.md` source files live in the sibling `skills` repo.
