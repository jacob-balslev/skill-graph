#!/usr/bin/env bash
# run-panel-loop.sh — supervised BATCH/DRAIN driver for the multi-agent panel audit loop.
#
# Per skill: runs the official panel audit loop (lib/audit/run-skill-audit-loop.js — Opus 4.8 +
...
# KEPT audit-loop authority, surviving a wiped local ledger) OR it has a terminal local-ledger
...
#
# Resilience: a per-skill WATCHDOG (--timeout, default 5400s/90m) kills a hung audit run (whole
...
# updated skills — those land in the separate ~/Development/skills repo under AUDIT_LOOP=1.
#
# Usage:
...
#   0 = audit-loop change KEPT/applied  ->  commit SKILL.md (+ audit-state.json); release status=completed
...
AUDIT_RUNNER="$SG/lib/audit/run-skill-audit-loop.js"
...
LOOP_ID="skill-panel-audit"          # distinct from the OpenCode "skill-audit" loop checkpoint
...
DRY_FLAG=""                            # --dry-run: pass through to audit AND skip the CONTENT commit
TIMEOUT=5400                           # per-skill watchdog ceiling (s); ~90m, generous over ~68m observed
WORK_ROOT="${WORK_ROOT:-/tmp/skill-audit-loop}"
...
[ -f "$AUDIT_RUNNER" ] || { echo "run-panel-loop: audit runner not found: $AUDIT_RUNNER" >&2; exit 69; }
...
committed_already() { # slug -> 0 if a KEPT audit-loop commit exists in the skills repo (durable authority)
...
#       audit-loop commit, which survives a wiped local ledger (git history is permanent); OR
#   (2) it has a terminal LOCAL-ledger line (applied|reverted|noop|missing) in the persistent
#       ledger ($WORK_ROOT=/tmp/skill-audit-loop) — covering non-committed terminal outcomes so neither
...
# Per-skill watchdog: hard-kill the audit run tree if it outruns $TIMEOUT. Liveness via OWNED pid.
WATCHDOG_PID=""
start_watchdog() {
  ...
  (
    ...
    while [ "$elapsed" -lt "$timeout" ]; do
      ...
      kill -0 "$pid" 2>/dev/null || exit 0   # audit run exited on its own — nothing to kill
    done
...
configure_frontier_budget() {
  ...
  if [ -n "$exhausted" ]; then
    if [ "$DEGRADE_ON_BUDGET" -eq 1 ]; then
      AUDIT_FRONTIER_ARGS=(--degrade-on-budget)
...
# counters (globals, mutated inside audit_one_skill)
applied=0; reverted=0; failed=0; missing=0; skipped=0
CODE=0; REL_STATUS="aborted"
# SKI: set when a MANDATORY frontier hits the Claude MAX *session* limit (which the per-model
# budget-monitor does NOT track, so configure_frontier_budget cannot preempt it). When set, the
# drain loop STOPS cleanly (BUDGET-PAUSED) instead of cascade-failing every remaining skill.
RATE_LIMITED=0

# Run ONE skill through the panel audit loop. Sets globals CODE + REL_STATUS; commits on keep.
audit_one_skill() { # $1=slug $2=dir
  local slug="$1" dir="$2"
  ...
  AUDIT_LOOP=1 node "$AUDIT_RUNNER" \
    --skill "$slug" --skill-dir "$dir" --cwd "$SG" \
    --max-rounds "$MAX_ROUNDS" $ADV_FLAG $DRY_FLAG "${AUDIT_FRONTIER_ARGS[@]}" \
    --status-file "$STATUS" --no-tui \
    >"$RESULT" 2>"$LOG" &
  local AUDIT_PID=$!
  start_watchdog "$AUDIT_PID" "$TIMEOUT" "$WD_SENTINEL"; local WD="$WATCHDOG_PID"
  ...
  # MANDATORY-frontier Claude-MAX / session rate-limit → PAUSE the whole drain (recoverable). The
  # per-model budget-monitor does NOT track the MAX session ceiling, so configure_frontier_budget
  # cannot preempt this — detect it from the runner's own output and stop before the cascade marks
  # every remaining skill failed. The skill stays eligible (no terminal ledger line, no counter bump).
  if grep -qiE "hit your session limit|rate-limited[^.]*retry after the reset|MANDATORY (opus|gpt[^ ]*)[^.]*rate-limited" "$LOG" "$RESULT" 2>/dev/null; then
    echo "    $slug — MANDATORY frontier RATE-LIMITED (Claude MAX session limit) — pausing drain; '$slug' stays eligible for resume" >&2
    RATE_LIMITED=1; REL_STATUS="aborted"; return
  fi
  ...
  case "$CODE" in
    0)
      ...
      local frontier_label; frontier_label=$(node -e 'try{const r=require(process.argv[1]);const m=(r.mandatory_models||[]).join(", ")||"n/a";if(r.degraded_frontier&&r.degraded_frontier.enabled){console.log(`single-available-frontier ${m} (PROVISIONAL-capped; re-grade required)`)}else{console.log(`multi-frontier panel ${m}`)}}catch(e){console.log("multi-frontier panel")}' "$RESULT" 2>/dev/null || echo "multi-frontier panel")
      {
        ...
        printf 'Updated via the multi-agent panel loop (run-panel-loop drain).\n'
    ...
    143|137)
      ...
      # working tree must be restored so the next drain reruns from the committed
...
# top skill forever and NEVER advances (verified bug 2026-06-06: api-design reran on
...
drain_worklist() {
  ...
  for slug in "${SLUGS[@]}"; do
    ...
    if [ -n "$DRY_FLAG" ]; then
      echo "[$nn/$TOTAL] $slug — DRY preview (offline audit; no claim/release/commit)" >&2
      audit_one_skill "$slug" "$dir"
    ...
    echo "[$nn/$TOTAL] $slug — auditing -> ${dir#$SKILLS_REPO/}" >&2
    ...
    if [ "$RATE_LIMITED" -eq 1 ]; then
      : > "$WORK_ROOT/.rate-limit-paused"   # sentinel the resume-supervisor polls
      echo "run-panel-loop: BUDGET-PAUSED (Claude MAX rate limit) after ${processed} processed skill(s) — clean stop; resume after the MAX reset window" >&2
      break
    fi
...
drain_file() {
  ...
  for slug in "${SLUGS[@]}"; do
    ...
    echo "[$i/$TOTAL] $slug — auditing -> ${dir#$SKILLS_REPO/}" >&2
    audit_one_skill "$slug" "$dir"
    processed=$((processed+1))
    if [ "$RATE_LIMITED" -eq 1 ]; then
      : > "$WORK_ROOT/.rate-limit-paused"
      echo "run-panel-loop: BUDGET-PAUSED (Claude MAX rate limit) after ${processed} processed skill(s) — clean stop; resume after the MAX reset window" >&2
      break
    fi
  done
